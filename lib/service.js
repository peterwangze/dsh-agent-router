 ...
 * （403 restricted_client）。`cloud-platform` 是 gcloud CLI 自身同款
 * client+scope 组合，已实测通过授权端点校验，且 token 可调用
 ...
 *    则用 cloud-platform；
 *  - 自建 client：仅迁移已被 Google 拒绝的旧 generativelanguage。 */
...
export class RouterService extends TypertRemoteService {
  ...
  async oauthDiscover(request) {
    ...
    if (!response.ok) {
      ...
      return { ok: false, message: `模型列表请求失败 HTTP ${response.status}${detail ? `：${detail}` : ''}`, models: [] }
    }
    let payload
    try {
      payload = await response.json()
