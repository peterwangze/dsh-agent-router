 ...
 * LlmRuntime（宿主 @deepseek-ai/dsh-llm 的类定义）——registerAdapter /
 * registration / listModels 三方法齐备；会话选择面 =
 * sessionController.selectModel（宿主 dsh-api-session-controller）。
 *
 ...
 * 归一）+ 1 组判别测试（tests/preset-defaults.mjs N 节负向守卫届时翻转为
 * 正向）。**锚形 = 函数名 + 文件**（FIX-040）：删除前实现 = 本仓库 git 历史
 * 中 `lib/preset-defaults.js` 的本地实现 `sessionSelectFaceOf`（B3 迁移前该
 * 函数即含 apiProxy 回落分支——用 `git log -p -- lib/preset-defaults.js`
 * 按函数名检索即可定位；不再写死会漂移的行号区间）。
 *
...
/**
 * 会话选择面解析（§4.3 域 2 唯一实现；**锚形 = 函数名 + 文件**（FIX-040）：
 * 迁移自 `lib/preset-defaults.js` 的本地实现 `sessionSelectFaceOf`，
 ...
 *
 * **锚形 = 函数名 + 文件**（FIX-039 S-2 / FIX-040）：迁移自 `lib/preset-defaults.js`
 * 子代理 fixup 分支的本地实现 `inheritedRouteOf`（该实现已抽为本域单点，原位
 * 只余消费调用；原注释写死行号区间，B3 迁移时点虽准确，但随 v0.4.5 之后该文件
 * 演进已漂移，故不再写死行号——按函数名可经 `git log -p -- lib/preset-defaults.js`
 * 检索定位）。
 *
 * 语义：父最近请求头路由优先（`parent.session.requestHeader()?.config`——新宿主
 * 「The latest request header owns provider, model…」语义），无 header 回落
 * `parent.options`（旧宿主/父未发请求形态）。返回 null = 基线不可读（父缺失/
 * 形态漂移——调用方保护降级为 fixup 继续，EVO-013 F-4 台账域既有边缘）。
 */
...
/**
 * 会话空白判别（FIX-025；**锚形 = 函数名 + 文件**（FIX-040）：迁移自
 * `lib/preset-defaults.js` 的本地实现 `sessionNeverProduced` 原样）：与宿主
 * sessionBlank **完全同构**（宿主 dsh-host-apiproxy 的空白判据：
