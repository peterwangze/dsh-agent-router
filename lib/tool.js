...
export function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    description: [
      ...
      '`task` 必须自包含：专业 agent 看不到本会话完整上下文，需把全部必要信息写入（agent 类型处理文件时写明工作区文件路径）。',
      '附件按需显式派发：`attachments` 传附件序号（0 起，按最近一条含附件的用户消息中的出现顺序）；`includeImages: true` 表示把该消息的全部图片一并转发；两者都不给 = 不携带任何附件。',
      '`attachmentIds`：对话框图片通路——用户消息中带 "[router:image:…]" 图片标记时，逐个原样复制标记整串传给该参数（与 attachments/includeImages 取并集）。',
      '`files`：工作区文件路径或 http(s) URL 列表，一次可传多个不同类型。chat 类型按内容分发（图片文件内联注入，需该 agent 声明 image 能力；文本文件内联进 task；其余二进制请改传 agent 类型）；agent 类型把路径注入子代理由其用 fs 工具读取；cli 类型同样把路径注入无头 CLI 子代理（由其自身工具读取）；URL 由宿主下载到工作区 .router-files/ 后按同样规则分发。',
    ...
    parameters: {
      ...
      attachmentIds: {
        type: 'array',
        items: { type: 'string' },
        description: '显式图片引用列表：用户消息中 "[router:image:…]" 图片标记的原文（逐个原样复制整串，含方括号），或附件 ref JSON 字符串。与 attachments/includeImages 取并集（按 attachmentId 去重）。',
      },
      files: {
        type: 'array',
        items: { type: 'string' },
    ...
    async execute(args, exec) {
      ...
      // attachmentIds 显式引用（对话框图片通路）；三者取并集、按 attachmentId 去重。
      const picked = service.selectAttachments(exec.agent, {
        indices: Array.isArray(args.attachments) ? args.attachments : undefined,
        includeImages: args.includeImages === true,
      })
      const explicitRefs = typeof service.parseImageRefs === 'function'
        ? service.parseImageRefs(Array.isArray(args.attachmentIds) ? args.attachmentIds : [])
        : []
      const seen = new Set(picked.map((ref) => String(ref.attachmentId)))
      for (const ref of explicitRefs) if (!seen.has(String(ref.attachmentId))) picked.push(ref)
      const images = picked
