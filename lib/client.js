...
`
    ...
    function AgentsPage(props) {
      ...
      for (const entry of providers) {
        ...
        // FIX-019：llm-pi-ai 目录里的插件侧 provider（oauth 面统计键 + 宿主
        // 路由面 openai-codex 真目录条目）是插件自注册条目而非可配置账号——
        // 与统计键同判据（isPluginSelfRegisteredProvider 单点），不得以账号行
        // 形态出现在统计账号级卡片（用户实证幽灵卡）。
        if (isPluginSelfRegisteredProvider(entry.provider)) continue
      ...
      for (const total of stats ? stats.accountTotals ?? [] : []) {
        // FIX-015 + FIX-019：与 addedAccounts 同判据（isPluginSelfRegisteredProvider
        // 单点）——插件自注册 provider 键不得以账号行形态出现在统计账号级
        // 卡片（用户实证幽灵卡：假编辑器 + 统计行双重出现）。
        if (isPluginSelfRegisteredProvider(total.provider)) continue
      ...
      const statsBody = [
        ...
        // 总用量汇总 / 每日用量（按天聚合）/ 实时调用（最近调用记录）三类
        // 三级内容；默认折叠，summary 含全局计次）。
        el(CategoryCard, {
          ...
          children: [
            el('details', { open: true, className: 'dshrouter-card', style: { padding: '8px 10px' } },
              el('summary', { className: 'dshrouter-subtitle', style: { cursor: 'pointer' } }, t('statsPresetTotals')),
              el('div', { className: 'dshrouter-row', style: { marginTop: 6 } },
            ...
            el('details', { className: 'dshrouter-card', style: { padding: '8px 10px' } },
              el('summary', { className: 'dshrouter-subtitle', style: { cursor: 'pointer' } }, t('statsPresetDaily')),
              Object.keys(statsDays).length === 0 ? el('p', { className: 'dshrouter-hint', style: { marginTop: 6 } }, t('statsNoCalls')) : el('table', { className: 'dshrouter-table', style: { marginTop: 6 } },
                el('thead', null, el('tr', null,
            ...
            el('details', { className: 'dshrouter-card', style: { padding: '8px 10px' } },
              el('summary', { className: 'dshrouter-subtitle', style: { cursor: 'pointer' } }, t('statsPresetRecent')),
              recentCalls.length === 0 ? el('p', { className: 'dshrouter-hint', style: { marginTop: 6 } }, t('statsNoCalls')) : el('table', { className: 'dshrouter-table', style: { marginTop: 6 } },
                el('thead', null, el('tr', null,
