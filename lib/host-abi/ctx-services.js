 ...
 * typeof 守卫（**函数名 + 文件**式锚，禁行号式锚：B4→B5 间八项锚点已随
 * 行漂移失效六项，函数名锚不随行号腐化）：agentDefaultModel = lib/service.js
 * 的 agentDefaultModelOf 消费点 + lib/host-abi/llm-selection.js 的
 * liveDefaultSelection；sessionController = lib/host-abi/llm-selection.js 的
 * sessionSelectFaceOf；agents = lib/preset-defaults.js 的 agentsRegistryOf
 * 消费点（播种 + 预设切换）；agentPresets = lib/preset-defaults.js 的
 * agentPresetsServiceOf 消费点（livePresetOf + 请求遥测）；sessionProjections
 * = lib/prestep.js 的会话投影读取点；llm = lib/host-abi/llm-selection.js 的
 * llmFaceOf（wrapper/oauth-llm 消费）+ lib/service.js 的 safeListModels /
 * resolveModelInfo 消费点；settings = lib/host-route.js 的 readProvidersEntry /
 * syncHostRoute 消费点。credentials/fs/attachments/subagents 为存在性探测
...
/**
 * Node 侧 ctx 服务面 probe 清单（§5.2 启动自检 + §4.3 域 3 能力探测的装配源；
 * EVO-023 B5 的 R-5 接线——B1 落 registerFaceProbes 机制但注册表为空）：
 * 由 CTX_SERVICES 单点派生（禁手抄清单——新服务登记即自动入自检面），面名
 * 前缀 `ctx:` 与客户端面（remote.* 五命名空间 / fiber inject 面）区分——
 * HostHealthCard 按名去重时两侧同名会互相遮蔽，前缀保证 Node 面与浏览器面
 * 同时可见。返回 [{ name, probe }]——probe 供 registerFaceProbes 注册。
 */
export function ctxServiceFaceProbes() {
  return Object.keys(CTX_SERVICES).map((name) => ({
    name: `ctx:${name}`,
    probe: (ctx) => serviceFaceOf(ctx, name, CTX_SERVICES[name]).probe,
  }))
}
