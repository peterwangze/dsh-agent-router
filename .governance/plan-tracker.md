...
| MIG-001 | development | v3 迁移实施 Step 0-10（DEC-012） | 按架构 v3 §8 迁移路径逐步实施：Step 0 基线测试 → Step 1 移除整轮路由 → … → Step 10；每步独立提交+测试全绿；验收门 = D-1 五条指标（DEC-012） | 进行中——Step 0-7 完成（7cb2024/b7261d5/a23b338/374edfa/f89b8bd/f294c3c/98f04a3/2c4b194/1f17ea8 + Step 7 本提交；R1-R9 审查链全通过，EV-011~020；**核心通路功能闭环达成**——attachmentIds + 模态矩阵落地）；下一步 Step 8（F11 输入入口——V-DSH-2 实现前验证；含 R7-F-01/R8-F-02 断言补齐）；P2 遗留清零（R9 无新增 P2）；P3 遗留：R9 F-11~F-18 + R4 F-1/F-2（顺延六轮） | P0 |
