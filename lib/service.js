...
import { CHATGPT_PRESET, OauthCredentialStore, resolveCredentialPath, accountIdFromJwt, CREDENTIAL_ERROR_CODES, REFRESH_TIMEOUT_MS, startDeviceAuthorization, pollDeviceAuthorizationToken, DEVICE_FLOW_TIMEOUT_SECONDS, DEVICE_SLOW_DOWN_ADD_MS, DEVICE_TRANSPORT_RETRY_ADD_MS, DEVICE_MIN_POLL_INTERVAL_MS } from './oauth-credentials.js'
import { StatsStore } from './stats.js'
import { sourceAcceptsModality } from './wrapper.js'
// FIX-030-C：预设诊断快照（presetDiagnostics RPC 的数据源——preset-defaults
// 模块级有界环形，纯数据无宿主引用）。
import { presetDiagnostics as presetDiagnosticsOf } from './preset-defaults.js'
import {
  HOST_ROUTE_NS, HOST_ROUTE_REF, HOST_ROUTE_TICK_MS,
  createHostRouteState, syncHostRoute, hostRouteStatusOf, injectHostRouteToken, selectHostAccount,
} from './host-route.js'
// EVO-022 B4（ARCH-004 §10 B4 / §4.3 域 3）：17 处高危面散点 this.ctx.get
// 切换 ctx-services 域访问器（fs/settings 低危面直用按设计 §10 B4 分级
// 白名单保留——tests/host-abi-health.mjs §8a 快照看护）+ ②（EVO-019
// P3-5）hostFaceDiagnostics 三合一供数自 rpc.js 原型挂载平移为本类方法。
// EVO-023 B5（§4.3 域 4）：settings/updated 订阅收敛 events 域（与 index.js
// 统计持久化共享单宿主 listener）+ RPC 面读对非全绿快照做一次失败驱动复检
// （§7.1「健康面板打开」时机——boot 期面未就绪的陈旧 missing 自愈）。
import { llmOf, credentialsOf, attachmentsOf, subagentsOf, agentDefaultModelOf, hostVersionsOf, faceHealthSnapshot, hostDiagnostics, runFaceProbes, subscribeEvents } from './host-abi/index.js'
...
/** EVO-009 复用缝：codex-responses 端点归一导出（oauth-llm.js 主 agent 通路共用）。 */
export function resolveCodexResponsesUrl(baseURL) {
...
/**
 * EVO-011（RES-004）：codex images 端点归一——resolveCodexResponsesUrl 同构
 * 变换（空 baseURL → 官方默认 `https://chatgpt.com/backend-api`；已含
 * `/codex/images/generations` 原样；止于 `/codex` 补 `/images/generations`；
 * 其余补 `/codex/images/generations`——账号 baseURL 覆盖语义与 chat 分支
 * 保持一致）。ChatGPT 订阅生图专用（dsh-codex-connect transport.ts:22 同源
 * 常量；codex-rs endpoint/images.rs 官方路由）。
 */
export function resolveCodexImagesUrl(baseURL) {
  const raw = typeof baseURL === 'string' && baseURL.trim() ? baseURL.trim() : ''
  const normalized = (raw || CODEX_RESPONSES_DEFAULT_BASE_URL).replace(/\/+$/, '')
  if (normalized.endsWith('/codex/images/generations')) return normalized
  if (normalized.endsWith('/codex')) return `${normalized}/images/generations`
  return `${normalized}/codex/images/generations`
}
 ...
 * EVO-011（RES-004）：codex-responses 增补 'image'——ChatGPT 订阅账号经
 * chatgpt.com/backend-api/codex/images/generations 直连生图（JSON 非 SSE），
 * 该协议专属 image 分支；其余协议（含未知）保持 ['chat']。
 */
export function oauthCapabilities(protocol) {
  if (protocol === 'codex-responses') return ['chat', 'image']
  return ['chat']
}
 ...
 */
/** EVO-009 复用缝：SSE 事件流解析导出（oauth-llm.js 主 agent 通路共用）。 */
...
export class RouterService extends TypertRemoteService {
  constructor(ctx, base = {}, statsOptions = {}) {
    ...
    this.oauthEvents = []
    /** EVO-010：宿主官方 openai-codex 路由维护状态（host-route.js 状态面：
     *  maintained/accountId/tokenInjected/failures/degraded/notice——catalog
     *  下发数据源；永不携带 token 值）。 */
    this.hostRouteState = createHostRouteState()
    /** EVO-010：路由维护事件环形缓冲（独立于 oauthEvents 登录旅程环——
     *  C-9 环语义保持「登录旅程专用」，既有 journey-order 契约不被维护事件
     *  污染；上限 100 条，P7 永不携带 token 值）。 */
    this.hostRouteEvents = []
    /** EVO-010：路由维护串行队列（settings/updated 与 tick 并发到达时按序
     *  执行，避免并发 mutate/探活交错——stats transition 串行化先例）。 */
    this.hostRouteQueue = Promise.resolve()
    /** EVO-010 测试提速注入（实例级覆盖，oauthDeviceSlowDownAddMs 先例）：
     *  0 = 用 HOST_ROUTE_TICK_MS 默认值。 */
    this.hostRouteTickMs = 0
    /** FIX-031 D8（微批 5）：宿主路由维护完成 → 统计安全重载的一次性旗标
     *  （启动时序竞态：stats 启动即 load() 时 resolver 未就绪 → 历史
     *  host-route 行固化回落实体；accountId 就绪后 reload() 重放归并）。 */
    this.hostRouteStatsReloaded = false
    ...
    this.stats = new StatsStore({
      ...
      // FIX-031 D6（返工批 3）：宿主官方路由（openai-codex）与 oauth 通路共用
      // 同一份 ChatGPT 订阅凭据（host-route selectHostAccount 选首个启用账号
      // 注入同一凭据库）——归因单点经此 resolver 把 host-route 用量归并到路由
      // 实际选中的账号身份键（动态：随配置变化，读侧归一化）。归并方向
      // host-route → oauth，不改 oauth 键本身（accountHealth 白盒
      // 'oauth:'+accountId 寻址不受影响）。
      // D9（微批 7，第二轮复验）：身份与路由健康度解耦（DEC-029——账号身份
      // ≠ 健康度）。两级：①运行态优先（维护健康时用状态面 accountId——多账号
      // 时与 selectHostAccount 同源）；②配置推导兜底——维护降级/探活失败
      // （maintained=false，token 注入/parity 探活任一失败即达）不抹去身份，
      // selectHostAccount 从配置（启用 + preset/协议匹配 + host transport）
      // 确定性推导同一身份，历史行照常归并；③真未知（无任何启用的
      // host-transport 账号）→ null（诚实回落独立实体，不伪装成已知账号）。
  ...
  defaults() {
    const model = agentDefaultModelOf(this.ctx)
  ...
  /**
   * EVO-013：预设默认模型配置字典（settings scope 热读取——每次调用重读，
   * 设置页保存后无需重启即对后续 agent/created / agent-preset/selected
   * 事件播种生效）。未配置/形态异常
   * 返回空对象（lib/preset-defaults.js 消费点按「无配置 = 直通」处理）。
   */
  presetDefaults() {
    const presets = this.getState().presets
  ...
  async resolveAgent(id) {
    ...
    if (accountId) {
      ...
      // kill-switch ②层账号开关（EVO-006 / DEC-026 C2 转正后账号级关闭能力
      // 的调用侧单点——与池候选过滤同语义，P5）：停用账号明确报错（P8 非沉
      // 默），调用不发网络、不触碰凭据。
      if (account.enabled === false) {
        return { id, agent, mode: 'oauth', accountId, provider: `oauth:${accountId}`, model: '', source: 'account', error: `OAuth 账号 "${accountId}"（${account.name || accountId}）已停用（enabled=false）；请在账号卡片中启用后再调用` }
      }
   ...
   * qwen3.7-plus 可看图但会被误拒，README「常见问题」节「视觉 agent 用什么
   * 模型？」条 / 同型 D9）。旧预检只信宿主 `resolveModelInfo`，宿主判定缺失
   * 时无自证路径、无观测，静默拒绝同型 R2-F3 吞错零观测。
   *
  ...
  /**
   * 主模型（默认选择）image 能力判定（FIX-018 单点消费面，客户端零探测）。
   *
   * 为什么判定在服务端：客户端拿不到当前模型的模态信息（宿主取证 2026-08-31）
   * ——buildModelCatalog 的 group.models 条目仅 {id,name,description?,reasoning?}
   * （dsh-host-apiproxy lib/index.js:1010-1054，resolveModelInfo 拿到的
   * inputModalities 在 catalog 组装面被剥离不下发）；session.models 的 current
   * 仅 {provider,model,reasoningEffort?}（:2582-2594）；llm.providers 仅 provider
   * 视图（id/name）。浏览器端无任何模态判定路径，故由本插件 catalog RPC 下发。
   *
   * **FIX-043 批 C 判定（保留原文 + 失效登记；不静默改指）**：本句两处行号锚
   * （`dsh-host-apiproxy` 的 `lib/index.js` 区间锚一项 + 紧随其后的裸行号锚一项，
   * 均见上句）的宿主对象均为
   * **dsh-host-apiproxy**——该包在本机 DSH 0.1.5-rc.2 装态中**不存在**（靶子
   * `_npx/1e7f6d9597241db0/node_modules/@deepseek-ai/` 实读测试为假、包名
   * 枚举无此项）⇒ **对象已消失、无实读对象**。按 P10-④「不得虚构宿主符号」，
   * 本批不引入替代包名、不做语义改指（无同源实证），原锚与其承载的判据语义
   * 一并保留；宿主升级后按 `tests/host-contract.mjs` 头部刷新程序重核宿主锚
   * 的入口条件适用 ⇒ 登记为待判定（随宿主装态实读复核）。
   *
   * 判定复用 decideImagePrecheck（P5：适配器级能力探测不复制逻辑——宿主声明
   * 优先，缺失/不确定时 wrapper sourceAcceptsModality 自证，60s TTL 缓存），
   * 与 route_agent 带图调用的预检同一判定源（全链路一致，无第二套口径）。
   *
   * 判定对象 = agentDefaultModel.currentSelection()（与宿主 selectionFor 第三层
   * 同源；GUI selectModel 会 saveDefaultModelSelection，会话当前选择与默认选择
   * 在常规流一致）。漂移窗口内由客户端按 provider+model 匹配门控——不匹配 =
   * 不判定 = 客户端保守回落既有接管（宁多切不漏图）。
   *
   * @returns {Promise<{acceptsImage: boolean, source: string, provider: string, model: string}>}
   */
  async mainModelImageCapability() {
    const defaults = this.defaults()
    const base = { provider: defaults.provider, model: defaults.model }
    if (!defaults.provider || !defaults.model) return { acceptsImage: false, source: 'no-default-selection', ...base }
    const llm = llmOf(this.ctx)
    if (!llm || typeof llm.resolveModelInfo !== 'function') return { acceptsImage: false, source: 'llm-unavailable', ...base }
    try {
      const decision = await this.decideImagePrecheck({ provider: defaults.provider, model: defaults.model }, llm)
      return { acceptsImage: decision.accepted === true, source: decision.source, ...base }
    } catch (error) {
      // P8 可观测：判定异常不静默——source 携带原因；方向恒 fail-safe false
      // （客户端按「不支持」处理 = 保留既有贴图接管，图经 twin 必可达）。
      return { acceptsImage: false, source: `probe-failed: ${error && error.message ? error.message : String(error)}`, ...base }
    }
  }
  ...
  async runChat(resolved, input) {
    ...
    return {
      kind: 'chat',
      text: output || '（空响应）',
      // EVO-012 批二（用户反馈 EV-114：识别场景卡片不复读输入图——纯噪音）：
      // 转发图不回显进工具结果。图片仍在请求中转发（上文 messages 内容块），
      // 跨轮记忆由 rememberDispatchedImages/imageMemory 机制承担（M6 回写不变）。
      usage: usage ? {
  ...
  async runAgentDelegation(resolved, input) {
    const subagents = subagentsOf(this.ctx)
   ...
   *
   * `platform` 为可选**平台判据注入点**（默认 = 实机平台，行为零变化；P2-1 先例见
   * resolveCliSpec）——使 Windows 专属分支（`.cmd`/`.bat` shim、`.ps1`、无扩展名
   * PATH 探测）在 POSIX CI 亦可判别（FIX-040 P2-2：此前该分支只能由 win32 真机覆盖）。
   * 注：无扩展名分支的 PATH 扫描仍按实机分隔符（`;`）与实机 `ComSpec`/PATH 进行——
   * 注入平台只切换**平台判据**，不虚构宿主环境。
   */
  resolveCliInvocation(command, args, platform = globalThis.process?.platform ?? '') {
  ...
  async runImage(resolved, input) {
    ...
    let key
    const credentials = credentialsOf(this.ctx)
    if (apiKeyEnv) {
      if (credentials) {
      ...
      if (missingMeta) {
        ...
        if (data) {
          ...
          this.recordCapabilityEvent('attachment_ref_degraded', {
            ...
            host: attachmentsOf(this.ctx) ? 'unreadable' : 'unavailable',
          })
        }
      }
      refs.push({
  ...
  /** 把完整附件引用序列化为纯文本图片标记（EVO-012：内容寻址 id 附同源资产
   *  url——浏览器侧 <img src> 直达，绕开宿主 imageData RPC 网关；无 url 的
   *  旧标记/旧产物仍走 imageData 回退，attachmentId 跨轮指代语义不变）。 */
  imageMarkerOf(ref) {
    const safe = { ...ref }
    if (typeof safe.name === 'string') safe.name = RouterService.sanitizeImageRefName(safe.name)
    const url = RouterService.imageAssetUrl(typeof safe.attachmentId === 'string' ? safe.attachmentId : '')
    if (url) safe.url = url
    return `${RouterService.IMAGE_MARKER_PREFIX}${JSON.stringify(safe)}]`
  }

  /** EVO-012：内容寻址附件 id → 同源图片资产 URL 路径。白名单单点：仅
   *  `sha256:64hex`（ATTACHMENT_ID_RE）产 url——非内容寻址 id（旧产物/测试桩
   *  id）不产，客户端回落 imageData RPC（旧产物兼容零行为变化）。 */
  static imageAssetUrl(id) {
    return isAttachmentId(id) ? `/router-assets/${id}` : undefined
  }

  /**
   * EVO-012（FIX-020 承载）：按内容寻址附件 id 读取图片资产字节——同源
   *  /router-assets/ HTTP 路由的只读后端（绕开宿主 imageData RPC 网关通道）。
   * 读取复用 registry.readStoredImage（FIX-007 F-1 单点：条目完整 ref →
   * 宿主 readImage → 对象文件自取证 + 内容哈希兜底），id 已在入口过
   *  ATTACHMENT_ID_RE 白名单；只读，不触碰任何状态。未注册且宿主不可读
   * （含 id 非法）返回 null → 调用方 404。
   */
  async readImageAsset(id) {
    if (typeof id !== 'string' || !isAttachmentId(id)) return null
    let stored
    try {
    ...
    const ref = stored.ref && typeof stored.ref === 'object' ? stored.ref : {}
    const mediaType = typeof ref.mediaType === 'string' && ref.mediaType ? ref.mediaType : ''
    return {
      data: stored.data,
      ...(mediaType ? { mediaType } : {}),
      ...(typeof ref.width === 'number' ? { width: ref.width } : {}),
   ...
   *
   * v0.3.1 EVO-006（DEC-026 C2 转正）：入口原 §3.6 第③层 kill-switch 调用期
   * 检查（R5-F1）随实验通道转正移除——通道关闭能力由 ① router.enabled 总开关、
   * ② 账号级 enabled（resolveAgent 直连判别 + oauthBegin 发起判别）、
   * ③ 登出删除（W-5 恒可用）承担。校验序保持（未知 preset → 资源触碰）。
   */
  async resolvePresetCredential(account) {
    ...
    let fresh
    try {
      fresh = await store.ensureFresh(cred)
    } catch (error) {
    ...
    // EVO-010（唯一刷新者纪律）：任何刷新路径（请求路径/后台 tick）产出新
    // access 都同步宿主凭据 ref——插件是唯一刷新者，宿主官方路由每请求活读
    // ref 即得新 token（绝无 pi-ai grant-record 双写者）。钩子内部自吞错
    // （注入失败不阻断插件通路请求——P8 经事件/warn/失败计数观测）。
    if (fresh.access !== cred.access) await this.afterPresetCredentialRefresh(account, fresh)
    return fresh
  }

  /**
   * EVO-010：凭据刷新后的宿主 ref 注入钩子。仅 host 通路账号参与（plugin
   * 账号被排除出路由维护——transport 语义）；失败计入 host 路由失败链路
   * （P8 可观测），绝不向调用方抛错（请求通路不受注入失败阻断）。
   */
  async afterPresetCredentialRefresh(account, fresh) {
    try {
      if (!account || normalizeTransport(account.transport) !== 'host') return
      const access = fresh && typeof fresh.access === 'string' ? fresh.access : ''
      if (!access) return
      if (await injectHostRouteToken(this.ctx, HOST_ROUTE_REF, access)) {
        this.hostRouteState.tokenInjected = 'ok'
        this.recordHostRouteEvent('host_route_token_injected', {})
      }
    } catch (error) {
      this.hostRouteState.tokenInjected = 'failed'
      this.hostRouteState.failures += 1
      try {
        this.recordHostRouteEvent('host_route_token_inject_fail', { reason: 'refresh_hook' })
      } catch { /* 事件面尽力而为 */ }
      this.ctx.logger?.warn?.(`dsh-agent-router: openai-codex host ref injection failed after refresh: ${errorMessage(error)}`)
    }
  }

  /** EVO-010：路由维护事件埋点（kind/detail 同 oauthEvents 形状；独立环）。
   *  detail 仅携带 accountId/reason/action 等非敏感字段（P7）。 */
  recordHostRouteEvent(kind, detail = {}) {
    this.hostRouteEvents.unshift({ at: Date.now(), kind, ...detail })
    if (this.hostRouteEvents.length > 100) this.hostRouteEvents.length = 100
  }

  /**
   * EVO-010：宿主路由维护入口（串行队列包装 syncHostRoute）——settings/
   * updated、登录联动与后台 tick 共用；异常不外泄（计入失败链路并 warn）。
   * @param trigger - 触发源透传（F-1 回环抑制：'llm' = llm-pi-ai ns 事件，
   *   失败态下 gate 写入；'tick'/'user'/'boot' 不 gate——见 syncHostRoute）。
   */
  queueHostRouteSync(trigger = 'user') {
    this.hostRouteQueue = this.hostRouteQueue
      .catch(() => {})
      .then(() => syncHostRoute(this.ctx, this, trigger))
      // FIX-031 D8（微批 5）：维护完成后探测 accountId 就绪迁移——一次性
      // 触发统计安全重载（maybeReloadStatsAfterHostRouteSync 内幂等去抖）。
      .then((result) => { this.maybeReloadStatsAfterHostRouteSync(); return result })
      .catch((error) => {
        // F-2/F-1 语义保持：队列化 pass 异常计入失败链路（原 runHostRouteTick
        // catch 口径——tick 异常不中断循环）。
        if (this.hostRouteState) this.hostRouteState.failures += 1
        this.ctx.logger?.warn?.(`dsh-agent-router: host route sync failed: ${errorMessage(error)}`)
      })
    return this.hostRouteQueue
  }

  /**
   * FIX-031 D8（微批 5）：宿主路由维护完成 → 统计一次性安全重载。
   *
   * D9（微批 7，第二轮复验）：触发门槛 = **身份就绪**（resolver 现在能给出
   * 身份键）而非路由健康度（maintained）——身份 ≠ 健康度（DEC-029）：维护
   * 降级/探活失败不应阻止历史行归并重放（身份经配置推导恒可知，
   * hostRouteAccountKeyOf 两级 resolver）。每次维护 pass 后探测（boot/
   * settings/tick 同链），首次满足即触发并置一次性旗标（重复触发零副作用）；
   * 真未知（resolver null）不触发——回落独立实体语义保持诚实。stats 未启用
   * 持久化时不消费旗标（无盘可重放；待 persist 启用后的下一次维护 pass 再
   * 触发）。reload 失败零影响（stats 内部兜底，回落实体保持现状——可观测
   * 降级而非中断）。
   */
  maybeReloadStatsAfterHostRouteSync() {
    try {
      if (this.hostRouteStatsReloaded) return
      if (!this.stats || this.stats.persist !== true) return
      if (typeof this.stats.hostRouteAccountKeyOf !== 'function') return
      if (!this.stats.hostRouteAccountKeyOf()) return
      this.hostRouteStatsReloaded = true
      void this.stats.reload()
    } catch { /* 探测失败零影响维护链（P7） */ }
  }

  /**
   * EVO-010：启动宿主官方路由维护生命周期（index.js ctx.effect 装配）：
   * ① 立即首跑（启动自证 + PoC 条目接管）；② settings/updated 监听
   * （router ns = 账号/开关变化——user 触发；llm-pi-ai ns = 条目被外部改动
   * 或**本模块自身写入**——llm 触发；undefined = 全量广播——user 触发）；
   * ③ 周期 tick（token 临期刷新注入 + parity 守卫自愈——**走同一串行队列**
   * （F-2：tick 不再旁路，登出→复活窗口 ≤30s 语义保持））。
   * 返回卸载器（清监听 + 清定时器）。
   *
   * EVO-023 B5（§4.3 域 4 / D1-9）：两个 ns 消费者迁 events 域同一次订阅
   * ——与 index.js 统计持久化共享单宿主 listener（跨模块聚合），按 ns 各自
   * 过滤（迁移前单 handler 的 if/else 语义逐字保持：router/undefined→user、
   * llm-pi-ai→llm）。**不声明域 gate**：F-1 回环门是**写级**部分抑制
   * （gateWrites 分支在 syncHostRoute 内——失败态下事件 pass 照跑只读状态/
   * user-modified 检查），整 handler 抑制会丢该只读面（F1-2 契约回退）。
   */
  startHostRouteMaintenance() {
    const disposeEvents = subscribeEvents(this.ctx, [
      {
        event: 'settings/updated',
        consumer: 'host-route-router-ns',
        handler: (ns) => { if (ns === ROUTER_NS || ns === undefined) this.queueHostRouteSync('user') },
      },
      {
        event: 'settings/updated',
        consumer: 'host-route-entry-ns',
        handler: (ns) => { if (ns === HOST_ROUTE_NS) this.queueHostRouteSync('llm') },
      },
    ])
    const tickMs = this.hostRouteTickMs > 0 ? this.hostRouteTickMs : HOST_ROUTE_TICK_MS
    const timer = setInterval(() => { void this.queueHostRouteSync('tick') }, tickMs)
    timer.unref?.()
    this.queueHostRouteSync('boot')
    return () => {
      disposeEvents()
      clearInterval(timer)
    }
   ...
   * EVO-011（RES-004）：codex-responses 协议（ChatGPT 订阅）的 image 类型
   * 走专属 runCodexResponsesImage 直连（/codex/images/generations，JSON 非
   * SSE）——能力矩阵增补后仅此处展开分发（P5 单点）。
   */
  async runOauthDispatch(resolved, input) {
    ...
    if (type === 'image' && protocol === 'codex-responses') return this.runCodexResponsesImage(resolved, input)
  ...
  async runOauthChat(resolved, input) {
    ...
    // EVO-012 批二（EV-114）：输入转发图不回显进工具结果（识别场景卡片不复读
    // 用户发的图）；跨轮记忆由 rememberDispatchedImages/imageMemory 承担。
  ...
  async runCodexResponsesChat(resolved, input) {
    ...
    const body = {
      ...
      // FIX-013：不发送 max_output_tokens——ChatGPT backend-api 拒绝该参数本身
      //（HTTP 400 "Unsupported parameter max output tokens"）；router/test 路径的
      // 强制 maxTokens 保留，供其它协议（openai max_tokens / gemini maxOutputTokens）消费。
      ...(temperature !== undefined ? { temperature } : {}),
    }
    let response
    try {
    ...
    if (!output) throw new Error('OAuth 调用返回中没有文本内容')
    // EVO-012 批二（EV-114）：输入转发图不回显进工具结果（codex-responses 与
    // runOauthChat/runChat 同口径）；跨轮记忆由 rememberDispatchedImages 承担。
    return { kind: 'chat', text: output, usage }
  }

  /**
   * EVO-011（RES-004）：codex-responses 协议专属 image 分支——ChatGPT 订阅
   * preset 账号直连 chatgpt.com/backend-api/codex/images/generations 生图
   * （OpenAI Images API 兼容 JSON，非 SSE——dsh-codex-connect transport.ts:22
   * 同一端点；codex-rs endpoint/images.rs 官方路由）。
   *
   * - 请求：preset 四元组头部（Bearer + chatgpt-account-id + originator
   *   dsh-agent-router 诚实自标识 + 诚实 User-Agent）与 runCodexResponsesChat
   *   同构；仅 chatgpt.com 目标经代理 dispatcher（resolveOauthProxy 单点）；
   *   redirect:'manual'——301/302 拒绝跟随（dsh-codex-connect 先例，防端点
   *   漂移到非预期目标）。
   * - body：{ model, prompt, n:1, size?, quality? }——model 策略：agent 模型
   *   匹配 /^gpt-image/ 透传，否则（dall-e-3 等平台 API 旧默认）默认
   *   'gpt-image-2' 并在结果文本给出换型指引（不静默替换——提示说明）；
   *   size 仅透传 gpt-image 合法值（1024x1024/1024x1536/1536x1024/auto），
   *   非法值省略并提示；quality 存在则透传（agent 配置面暂未暴露，预留）。
   * - 响应：JSON 无 SSE——data[0].b64_json 缺失报明确错误（端点形状校验，
   *   P-v2-8/9 防守非公开契约）；成功走 runImage 同款链路 decodeBase64→
   *   sniffMediaType→attachments.saveImage→{ kind:'image', image }
   *   （tool.js result.image → marker 渲染零改动）。
   * - 错误：401/403 重登、429/usage_limit_* resets_at 分钟数——H3-14 同款
   *   解析；recordCapabilityEvent 埋点（P8：降级/失败路径可观测）。
   */
  async runCodexResponsesImage(resolved, input) {
    const account = resolved.account
    const preset = typeof account.preset === 'string' && account.preset.trim() ? account.preset.trim() : ''
    if (!preset) {
      throw new Error('codex-responses 协议仅支持 ChatGPT 预设账号（preset 凭据四元组含 accountId，H3-7）；请在账号卡片使用 ChatGPT 预设登录')
    ...
    const agent = resolved.agent
    const prompt = this.composeTask(input.task, input.extra)
    // 模型策略：gpt-image 系透传；其余（dall-e-3 等平台 API 旧默认）→ 默认
    // gpt-image-2 + 换型指引（不静默替换——错误/提示信息说明）。
    const originalModel = typeof resolved.model === 'string' && resolved.model.trim() ? resolved.model.trim() : ''
    let model = originalModel
    let modelHint = ''
    if (!/^gpt-image/i.test(model)) {
      model = 'gpt-image-2'
      modelHint = `【模型指引】原模型 ${originalModel || '（空）'} 非 gpt-image 系，已改用 ChatGPT 订阅生图默认模型 gpt-image-2；如需多档质量请在模型字段填写 gpt-image-*（如 gpt-image-2）`
      this.recordCapabilityEvent('codex_image_model_fallback', { provider: resolved.provider, model: originalModel || '', resolvedModel: model })
    }
    const body = { model, prompt, n: 1 }
    // size 透传：仅 gpt-image 合法值；非法值（如 1792x1024）省略并提示；
    // 未配置 → 默认 1024x1024（runImage 同款默认语义）。
    const rawSize = typeof agent.imageSize === 'string' && agent.imageSize.trim() ? agent.imageSize.trim() : ''
    const gptImageSizes = new Set(['1024x1024', '1024x1536', '1536x1024', 'auto'])
    let sizeHint = ''
    if (rawSize) {
      if (gptImageSizes.has(rawSize)) body.size = rawSize
      else {
        sizeHint = `【尺寸提示】${rawSize} 非 gpt-image 合法值（1024x1024 / 1024x1536 / 1536x1024 / auto），本次已省略 size（默认尺寸）`
        this.recordCapabilityEvent('codex_image_size_omitted', { provider: resolved.provider, model, size: rawSize })
      }
    } else {
      body.size = '1024x1024'
    }
    const quality = typeof agent.quality === 'string' && agent.quality.trim() ? agent.quality.trim() : ''
    if (quality) body.quality = quality
    const url = resolveCodexImagesUrl(account.baseURL)
    const platform = globalThis.process?.platform ?? 'unknown'
    const arch = globalThis.process?.arch ?? 'unknown'
    // H3-9 同款 preset 四元组（Bearer + chatgpt-account-id + originator
    // dsh-agent-router 诚实自标识 + 诚实 User-Agent）；accept 为 JSON（非 SSE）。
    const headers = {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${cred.access}`,
      'chatgpt-account-id': cred.accountId,
    ...
    try {
      const init = {
        ...
        headers,
        body: JSON.stringify(body),
        // 301/302 拒绝跟随（dsh-codex-connect 先例——端点漂移防护）。
        redirect: 'manual',
        ...(input.signal ? { signal: input.signal } : {}),
      }
      const proxy = resolveOauthProxy(this.getState())
      ...
      if (error && typeof error.message === 'string' && error.message.includes('undici 代理支持')) throw error
      throw new Error(`OAuth 账号 "${resolved.accountId}" 生图端点不可达（${url}）：${errorMessage(error)}`)
    }
    if (!response.ok) {
      // H3-14 同款：301/302 → 重定向拒绝；401/403 → 重登；429/usage_limit_*
      // → resets_at 分钟数；其余透传 status 与错误体（截断 400 字符）。
      const raw = await response.text().catch(() => '')
      let err = null
      try { err = JSON.parse(raw)?.error ?? null } catch { /* 非 JSON 错误体按原文 */ }
      const code = typeof err?.code === 'string' ? err.code : ''
      ...
      if (response.status === 301 || response.status === 302) {
        throw new Error(`ChatGPT 生图端点返回重定向（HTTP ${response.status}），已拒绝跟随；请检查账号 baseURL 指向的网关/代理形态`)
      }
      if (response.status === 401 || response.status === 403) {
        this.recordCapabilityEvent('codex_image_error', { provider: resolved.provider, model, status: response.status })
        throw new Error(`OAuth access token 无效或已过期（HTTP ${response.status}）${detail ? `：${detail}` : ''}；请在账号卡片中重新登录`)
      }
      if (response.status === 429 || /usage_limit_reached|usage_not_included|rate_limit_exceeded/i.test(code)) {
        let when = ''
        ...
        const plan = typeof err?.plan_type === 'string' && err.plan_type ? `，${err.plan_type} plan` : ''
        this.recordCapabilityEvent('codex_image_error', { provider: resolved.provider, model, status: response.status, code: code || 'rate_limit' })
        throw new Error(`ChatGPT 生图用量限制（HTTP 429${plan}）${detail ? `：${detail}` : ''}${when}`)
      }
      this.recordCapabilityEvent('codex_image_error', { provider: resolved.provider, model, status: response.status, code: code || '' })
      throw new Error(`ChatGPT 生图调用失败 HTTP ${response.status}${detail ? `：${detail}` : ''}`)
    }
    let payload
    try {
      payload = await response.json()
    } catch (error) {
      throw new Error(`ChatGPT 生图端点返回非 JSON：${errorMessage(error)}`)
    }
    // 响应形状校验（P-v2-8/9）：data[].b64_json 缺失 → 明确错误（非静默吞掉）。
    const b64 = payload?.data?.[0]?.b64_json
    if (typeof b64 !== 'string' || !b64) {
      throw new Error('ChatGPT 生图端点未返回 b64_json 图片数据（端点形状校验：data[0].b64_json 缺失；如订阅额度已耗尽请稍后重试或查看 usage_limit 提示）')
    }
    const data = decodeBase64(b64)
    const attachments = attachmentsOf(this.ctx)
    if (!attachments || typeof attachments.saveImage !== 'function') throw new Error('附件服务不可用，无法保存生成的图片')
    const saved = await attachments.saveImage({ data, mediaType: sniffMediaType(data), name: `router-${resolved.id}.png` })
    const hints = [modelHint, sizeHint].filter(Boolean).join(' ')
    return { kind: 'image', text: `已生成图片（${saved.width}x${saved.height}）${hints ? `\n${hints}` : ''}`, image: saved }
  }
  ...
  /**
   * FIX-030-C：预设播种/修正诊断快照（RPC presetDiagnostics）——有界环形
   * 最近 N 条（kind/applied/target/skip 原因/preset/session 时间戳）。观测
   * 面：设置页预设卡最近事件行 + 报障排障依据；数据源 = preset-defaults
   * notePresetDiag（纯数据无宿主引用）。ok/enabled 包装同 statsSnapshot。
   */
  presetDiagnostics() {
    return { ok: true, enabled: this.isEnabled(), ...presetDiagnosticsOf() }
  }

  /**
   * ARCH-004 B1→B4（§6.2；EVO-019 P3-5 平移收口）：宿主面健康三合一观测面
   * （RPC hostFaceDiagnostics 数据源——EVO-019 期经 rpc.js 原型挂载的过渡
   * 形态已删除，本类方法为正式装配；wire 形状与 method 名不变，客户端零
   * 感知）：hostVersions（域 6 版本遥测）+ faces（FaceHealth 快照）+ diag
   * （noteHostDiag 环形最近 64 条）。
   *
   * EVO-023 B5（R-5 后续面 / §7.1 探针三时机）：全绿快照走**纯缓存读**（零
   * probe——渲染路径与设置页常驻轮询纪律）；快照为空或含非 ok 面时，本次读
   * 触发一次失败驱动复检（「健康面板打开」时机）——boot apply 自检时宿主服务
   * 尚未就绪的陈旧 missing 于面板打开时自愈，徽章不再长期误报（P1 事实原则
   * 的观测面落地）。复检只发生在存在降级态时，全绿后回到纯读（探针不进
   * 渲染路径、不进 2s 轮询——tests/host-abi-health.mjs §9f 判别锁定）。
   */
  hostFaceDiagnostics() {
    const cached = faceHealthSnapshot()
    const faces = cached.length === 0 || cached.some((face) => face.state !== 'ok')
      ? runFaceProbes(this.ctx)
      : cached
    return {
  ...
  async catalog() {
    ...
    return {
      ...
      // FIX-018：主模型 image 能力判定（服务端单点）——客户端 ModelTakeover
      // 贴图接管门控消费（客户端拿不到模态信息，见 mainModelImageCapability
      // 取证注释）。判定失败安全回落 false = 客户端保留既有贴图接管。
      mainModelImage: await this.mainModelImageCapability(),
      // EVO-010：宿主官方 openai-codex 路由维护状态（订阅卡 UI 状态行数据源）。
      openaiCodexRoute: hostRouteStatusOf(this),
      ...
      oauthAccounts: (await Promise.all(Object.entries(this.getState().oauthAccounts ?? {}).map(async ([id, account]) => {
        ...
        return {
          ...
          // EVO-010：调用通路镜像（归一后恒 'host'|'plugin'——订阅卡开关数据源）。
          transport: normalizeTransport(account.transport),
        }
  ...
  async oauthBegin(request) {
    ...
    // kill-switch ②层账号开关（EVO-006 / DEC-026 C2）：停用账号不得发起新
    // 登录（与调用侧 resolveAgent 判别同语义；登出删除不设此门——W-5 恒可用）。
    // 拒绝留痕（REL-004 / R0 P2-b）：reason='account_disabled'（C-9 观测面
    // 补「发起被拒」缺口，与 unknown_preset 同型；实验语义 reason
    // kill_switch/tos 已随转正消亡，不复活）。
    if (account.enabled === false) {
      this.recordOauthEvent('preset_begin_fail', { accountId: id, reason: 'account_disabled' })
      return { ok: false, message: `OAuth 账号 "${id}"（${account.name || id}）已停用（enabled=false）；启用后再发起授权` }
    }
    const preset = typeof account.preset === 'string' && account.preset.trim() ? account.preset.trim() : ''
   ...
   * 未就绪明确报错）。v0.3.1 EVO-006（DEC-026 C2 转正）：原 §3.6 第③层
   * kill-switch 与 ToS 门随实验通道转正移除——通道转正式登录路径，关闭
   * 能力由 ① router.enabled、② 账号级 enabled（oauthBegin 入口统一判别）、
   * ③ 登出删除承担；1455 惰性启动语义不变（未发起登录零端口占用，DEC-021）。
   * PKCE/state 生成与 oauthPending 会话（10 分钟过期）
  ...
  async persistPresetLogin(id, payload) {
    ...
    try {
      // AUDIT-001 P0-B：登录落盘与在途刷新（ensureFresh 锁内 read-modify-write）
      // 串行化——无锁 write 可插在刷新读盘与写回之间，随后旧链刷新产物覆盖
      // 刚落盘的新登录凭据（rotating refresh 链被丢弃、accountId 回跳）。
      // 锁预算 = 刷新最坏时长（REFRESH_TIMEOUT_MS=25s）+ 余量：等锁对象是
      // 一笔有界刷新，而非无界操作。ensureFresh 侧另有写回前盘上 CAS 守卫
      // 双保险（oauth-credentials.js ensureFresh 注释）。
      const store = this.credentialStoreFor(account)
      await store.withLock(REFRESH_TIMEOUT_MS + 10_000, () => store.write({ type: 'oauth', access: token, refresh, expires: Date.now() + expiresIn * 1000, accountId }))
    } catch (error) {
    ...
    // EVO-010：登录改变路由维护资格（未登录 → 清理；登录 → 建条目 + 注入新
    // token）——异步联动（不阻塞登录返回；失败经路由失败链路观测）。
    this.queueHostRouteSync()
   ...
   * 删除路径恒可用（EVO-006 转正后不受 ① router.enabled 与 ② 账号 enabled
   * 任何开关门控）：用户始终可以移除已落盘的订阅凭据
  ...
  async oauthLogout(request) {
    ...
    const preset = typeof account.preset === 'string' && account.preset.trim() ? account.preset.trim() : ''
    if (!OAUTH_PRESET_VALUES.includes(preset)) {
      // F-2（EVO-007 R0 返工）：原文案「通用账号请在账号卡片删除 token」指向
      // 已删除的 OAuth 账号卡片（死文案）——改指 F-1 恢复的删除入口（账号池行
      // 「删除账号」或高级扩展区「未入池的 OAuth 账号」列表行）。
      return { ok: false, message: '登出仅适用于 ChatGPT 预设账号（preset 独立凭据文件）；通用 OAuth 账号请在「多模态账号 → 高级扩展」的账号池行或未入池列表中删除（删除账号按钮）' }
    }
    ...
    try {
      // AUDIT-001 P0-B：登出删除与在途刷新串行化——无锁 delete 后，刷新完成
      // 的锁内 write 会把用户已显式删除的凭据文件**复活**（合规删除 W-5 被
      // 击穿；`exchangeDeviceCode` 的落盘前 cancelled 复查只覆盖兑换流，
      // 挡不住已过读点的刷新）。
      // 锁预算同 persistPresetLogin（等锁对象 = ≤25s 有界刷新）。ensureFresh
      // 侧另有写回前盘上 CAS 守卫双保险（deleted → 不写回）。
      const store = this.credentialStoreFor(account)
      await store.withLock(REFRESH_TIMEOUT_MS + 10_000, () => store.delete())
    } catch (error) {
    ...
    // EVO-010：登出后凭据文件已删——宿主官方路由条目随之清理（异步联动，
    // 不阻塞登出返回；幂等）。
    this.queueHostRouteSync()
