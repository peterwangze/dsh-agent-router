...
export class RouterService extends TypertRemoteService {
  ...
  // UNSUPPORTED_CONTENT。因此插件自建一条工作区路径通路：
  // - 客户端把对话框图片经 router/imagePrompt 交给宿主，落盘到会话工作区
  //   .router-files/，把「用户附带图片 + 路径清单」以纯文本注入用户消息；
  // - 主模型按提示调用 route_agent（files 填这些路径——与既有 files 通路
  //   完全一致）交给视觉 agent；带图片的调用自动附带主会话最近上下文，
  //   视觉 agent 结合上下文分析，截图真正成为对话上下文的一部分；
  // - 生成图片（draw）以纯文本标记渲染进工具结果（绝不把图片块写入历史，
  //   避免文本模型被击穿），浏览器侧 toolview 解析标记后经
  ...
  /** 组装注入会话的用户消息文本：需求 + 图片工作区路径 + 对主模型的 route_agent 引导。 */
  composeImagePromptText(text, paths, agentId) {
    const lines = []
    if (typeof text === 'string' && text.trim()) lines.push(text.trim())
    lines.push('', '[用户附带图片]')
    for (const path of paths) lines.push(`- ${path}`)
    lines.push('', [
      `用户通过对话框上传了 ${paths.length} 张图片（截图已由插件保存到上方工作区路径）。你（主模型）无法直接读取图片内容：`,
      `请调用 route_agent 工具把图片交给视觉 agent 分析——agent 填 "${agentId}"，files 填这些路径，task 填用户需求（当前会话的最近上下文会随调用自动附带）。`,
  ...
  /**
   * router/imagePrompt：对话框图片发送通路（工作区路径形态）。
   * - 校验总开关、视觉 agent、图片数量/类型/大小（与 api-proxy 准入同源
   *   的 attachments.imageLimits）；
   * - 把图片落盘到会话工作区 .router-files/（真实路径，主模型按 files
   *   参数交给视觉 agent——与既有 files 通路完全一致，无新协议）；
   * - 把文本 + 路径清单注入会话（agent.followup），主模型随后调用
   *   route_agent。失败一律返回 { ok: false, message }，绝不半写入会话。
   */
  async imagePrompt(request) {
    ...
    if (!cwd || typeof cwd !== 'string') return { ok: false, message: `会话 "${sessionId}" 没有工作目录，无法保存附件图片` }
    // 落盘会话工作区（与 files URL 下载同目录、同直写方式；真实路径即可
    // 被 route_agent 的 files 参数直接引用）。
    const dir = join(cwd, '.router-files')
    const stamp = `${Date.now().toString(36)}-${randomBytes(4).toString('hex')}`
    const extOf = { 'image/png': 'png', 'image/jpeg': 'jpg', 'image/webp': 'webp', 'image/gif': 'gif' }
    const paths = []
    try {
      ...
      decoded.forEach((image, index) => {
        const target = join(dir, `router-img-${stamp}-${index}.${extOf[image.mediaType] ?? 'png'}`)
        writeFileSync(target, image.data)
        paths.push(target)
      })
    } catch (error) {
      return { ok: false, message: `附件图片落盘失败：${errorMessage(error)}` }
    }
    const text = this.composeImagePromptText(typeof request?.text === 'string' ? request.text : '', paths, finalId)
    try {
      agent.followup(createUserMessage({ content: [{ type: 'text', text }], source: { kind: 'user' } }))
    } catch (error) {
      return { ok: false, message: `消息注入会话失败：${errorMessage(error)}` }
    }
    return { ok: true, message: `已发送 ${paths.length} 张图片：将由视觉 agent "${finalId}" 分析`, agentId: finalId }
  }
