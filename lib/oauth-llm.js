 ...
 *  imageRequestPricing（FIX-034 补齐，宿主 lib/index.js:1645 原型新增）/ listModels /
...
export function createOauthAdapter(ctx, service) {
  const adapter = {
    ...
    /**
     * FIX-034：宿主 0.1.5-rc.2 LlmAdapter 原型新增契约方法（宿主
     * lib/index.js:1645，语义与 FIX-033 twin wrapper.js 镜像实现同构）——
     * 签名 imageRequestPricing(_provider, _model)，**同步零 I/O**（宿主注释
     * :1639 "must answer synchronously without I/O; the token meter resolves
     * this per measurement"），默认返回 undefined = "本路由未声明图片定价，
     * 消费者回落自身中性估算"。运行时消费点 LlmRuntime.imageRequestPricing
     * （宿主 lib/index.js:1996-1998）经 adapters.get(provider).adapter.
     ...
     * 宿主基类默认语义（:1645 空体 → "declares none"，宿主 LlmImageRequest
     * Pricing schema :171-178 的 priceImages 实形由声明方适配器承载）。纯
