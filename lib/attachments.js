...
export class AttachmentRegistry {
  ...
  async materialize(id, options = {}) {
    ...
    const entry = await this.byId(id, { sessionId })
    if (!entry) throw attachmentError(ATTACHMENT_ERROR_CODES.ATTACHMENT_UNKNOWN, `附件不可解析（${id}）：未注册且宿主无法读取`)
    let path
    if (typeof entry.workspacePath === 'string' && entry.workspacePath) {
  ...
  async read(id, options = {}) {
    ...
    if (entry && typeof entry.workspacePath === 'string' && entry.workspacePath) {
      const fs = this.fsService
      if (!fs || typeof fs.readBytes !== 'function') throw attachmentError(ATTACHMENT_ERROR_CODES.STORE_UNAVAILABLE, '文件服务不可用，无法读取附件文件')
      let bytes
      try {
        bytes = await fs.readBytes(entry.workspacePath, signal, ATTACHMENT_FILE_MAX_BYTES)
      } catch (error) {
