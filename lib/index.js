...
export function apply(ctx) {
  ...
  }), 'dsh-router: oauth callback route')
  ...
  ctx.effect(() => {
    const loopback = createServer((req, res) => {
      handleOauthCallback(service, req, res).catch((error) => {
        ctx.logger.warn(`dsh-router: oauth loopback callback failed: ${errorMessage(error)}`)
        if (!res.headersSent) {
    ...
    loopback.on('error', (error) => {
      if (error && error.code === 'EADDRINUSE') {
        ctx.logger.warn('dsh-router: loopback OAuth callback port 8085 is occupied (e.g. gcloud CLI); built-in public client disabled')
        return
      }
      ctx.logger.warn(`dsh-router: loopback OAuth callback server error: ${errorMessage(error)}`)
    })
  ...
  }, 'dsh-router: builtin oauth loopback server')
