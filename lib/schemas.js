...
/**
 * EVO-010：账号调用通路（transport）枚举——
 * - `host`（默认）：宿主官方 openai-codex 路由（pi-ai 内置 codex 目录实现，
 *   插件只做凭据桥 + 路由条目维护，ARCH-003 实施要点）；
 * - `plugin`：插件手写协议层（OAUTH_PROVIDER 'chatgpt-oauth' 适配器）降级
 *   通路——账号被排除出官方路由维护，可随时手切，零重启。
 */
export const OAUTH_TRANSPORT_VALUES = Object.freeze(['host', 'plugin'])

/**
 * transport 归一（消费点校验先例——preset/protocol 同构）：'plugin' 之外
 * 一律按 'host'（含缺省/未知值——host 为推荐默认）。
 */
export function normalizeTransport(value) {
  return value === 'plugin' ? 'plugin' : 'host'
}
...
export const oauthAccountSchema = z.object({
  ...
  /** EVO-010 调用通路：'host'（默认）= 宿主官方 openai-codex 路由（推荐）；
   *  'plugin' = 插件手写协议层（降级）。自由字符串先例，消费点经
   *  normalizeTransport 归一（未知值按 host）。 */
  transport: z.string().default('host'),
})
...
/**
 * EVO-013：按 DSH 预设为粒度的默认模型配置（router.presets 字典的条目形状；
 * 权威单点——wire 侧无逐字段镜像：config/save 通道对 `value` 全量透传
 * （configResult.value 为自由 object codec、saveRequest ops.value 为 json），
 * 保存即随 config 下发，无字段白名单缺口）。
 *
 * 语义（与 agentSchema 的空串语义同源）：
 * - `main.provider` / `main.model` 都非空 = 该预设的主 agent 默认模型——仅当
 *   新会话无更显式选择（picked / 日志 header 层）时由 lib/preset-defaults.js
 *   换入；未设置 = 完全遵循 DSH 现行规则（零行为变化）。
 * - `subagent.*` 都非空 = 该预设派生的 subagent 默认模型；未设置 = 继承
 *   main；main 也未设置 = 不介入。显式指定模型的子代理（插件 agent 类专业
 *   agent 的 agentOptions / workflow model 覆盖）永不被覆盖。
 * - `enabled`：条目级开关（false = 该预设条目整体不生效）。
 * - 键 = DSH 预设 id（自由字符串）；预设已删除的残留键自然不命中任何会话
 *   （消费点语义：配置存在但预设已不存在 = 不介入，UI 提示可删）。
 */
export const presetDefaultSchema = z.object({
  /** 条目级开关（false = 该预设不应用默认模型）。 */
  enabled: z.boolean().default(true),
  /** 主 agent 默认模型（provider/model 同时非空才生效；空 = 遵循 DSH 规则）。 */
  main: z.object({
    provider: z.string().default(''),
    model: z.string().default(''),
  }).default({}),
  /** subagent 默认模型（空 = 继承 main；main 亦空 = 不介入）。 */
  subagent: z.object({
    provider: z.string().default(''),
    model: z.string().default(''),
  }).default({}),
...
export const routerSchema = z.object({
  ...
  /** EVO-013：以 DSH 预设 id 为键的默认模型配置字典（形状见 presetDefaultSchema
   *  权威单点；未知预设键放行——消费点按「配置存在但预设已不存在」处理）。 */
  presets: z.dict(presetDefaultSchema).default({}),
})
...
/** FIX-030-C：预设播种/修正诊断条目的 wire 形状（有界环形最近 N 条）。 */
export const presetDiagEntryCodec = v.object({
  at: v.number(),
  kind: v.string(),
  applied: v.boolean(),
  preset: v.string(),
  session: v.string(),
  target: v.object({ provider: v.string(), model: v.string() }, true),
  skip: v.string(true),
  detail: v.string(true),
})

/** FIX-030-C：router/presetDiagnostics 结果。 */
export const presetDiagnosticsResult = v.object({
  entries: v.array(presetDiagEntryCodec),
})

/** ARCH-004 B1（§4.3 域 1/§6.1）：FaceHealth wire 形状——面名 + 状态
 *  （ok|degraded|missing）+ 可选原因短码（host-face-missing /
 *  host-face-shape: <方法表> / probe-threw 等 §4.3 错误码族）。 */
export const faceHealthCodec = v.object({
  name: v.string(),
  state: v.string(),
  detail: v.string(true),
})

/** ARCH-004 B1（§6.1）：宿主面诊断条目 wire 形状（noteHostDiag 环形——
 *  字段白名单同 lib/host-abi/health.js noteHostDiag：kind 事件短码 /
 *  face·consumer·code·detail 可选截断串）。 */
export const hostDiagEntryCodec = v.object({
  at: v.number(),
  kind: v.string(),
  face: v.string(true),
  consumer: v.string(true),
  code: v.string(true),
  detail: v.string(true),
})

/** ARCH-004 B1（§6.2）：router/hostFaceDiagnostics 结果——版本（域 6）+
 *  能力（FaceHealth 缓存快照）+ 事件（诊断环形）三合一。 */
export const hostFaceDiagnosticsResult = v.object({
  hostVersions: v.object({
    llm: v.string(),
    tools: v.string(),
    typertProtocol: v.string(),
  }),
  faces: v.array(faceHealthCodec),
  diag: v.array(hostDiagEntryCodec),
})

/** 校验器导出：宿主 typert 注册与客户端 $mount 共用同一形状。 */
export const wireCodecs = {
  /** FIX-030-C：router/presetDiagnostics 结果（预设播种/修正观测面）。 */
  presetDiagnosticsResult,
  /** ARCH-004 B1：router/hostFaceDiagnostics 结果（宿主面健康三合一观测面）。 */
  hostFaceDiagnosticsResult,
  /** router/catalog 结果。 */
  catalogResult: v.object({
    ...
    oauthAccounts: v.array(v.object({
      ...
      /** EVO-010：调用通路镜像（'host'|'plugin'，归一后恒两值之一）。 */
      transport: v.string(true),
    })),
    /** EVO-010：宿主官方 openai-codex 路由维护状态（可选——旧服务端缺省兼容；
     *  状态面永不携带 token 值，P7）。 */
    openaiCodexRoute: v.object({
      maintained: v.boolean(),
      ref: v.string(),
      accountId: v.string(),
      tokenInjected: v.string(),
      degraded: v.boolean(),
      failures: v.number(),
      notice: v.string(true),
    }, true),
