...
- 💬 **对话框图片能力**：启用视觉类专业 agent（能力标签含 `image`）后，输入框出现「添加图片」按钮——附件图片直接进原生草稿栏，经插件发送通路落盘为工作区文件，主模型拿到「用户附带图片 + 路径」信息后按 `files` 参数交给视觉 agent（自动附带主会话最近上下文，截图真正成为对话上下文的一部分）；生成图片以缩略图显示在 route_agent 工具卡片中（纯插件机制：图片以工作区路径进入会话，纯文本主模型全程不受影响）
...
- **对话框图片**：启用带 `image` 能力标签的视觉类专业 agent（chat / agent / cli 类型）后，对话输入框出现「添加图片」按钮——选中图片进入原生附件栏，输入框上方出现发送条（可下拉选择视觉 agent）；点「发送给专业 agent」后，插件把截图落盘到会话工作区 `.router-files/`，以「用户附带图片 + 路径」注入消息，主 agent 按 `files` 参数调用 route_agent 交给视觉 agent 分析（**自动附带主会话最近上下文**——截图是对话上下文的一部分，视觉 agent 结合上下文作答）。生成图片以缩略图显示在 route_agent 工具卡片里（点击查看原图），纯文本主模型全程不接触图片字节
