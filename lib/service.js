...
import { CHATGPT_PRESET, OauthCredentialStore, resolveCredentialPath, accountIdFromJwt, CREDENTIAL_ERROR_CODES, startDeviceAuthorization, pollDeviceAuthorizationToken, DEVICE_FLOW_TIMEOUT_SECONDS, DEVICE_SLOW_DOWN_ADD_MS } from './oauth-credentials.js'
...
export class RouterService extends TypertRemoteService {
  ...
  async oauthBeginDeviceFallback(id, reason) {
    ...
    const intervalMs = Math.max(1, started.intervalSeconds * 1000)
    ...
    return {
      ...
      expiresIn: DEVICE_FLOW_TIMEOUT_SECONDS,
    }
  }
   ...
   * 分支同路径）；failed 终态；超时（expiresAt，15 分钟）终态 expired；
   * cancelled（登出取消）终态。任何终态：写 session.status、清 Map、resolve
   ...
   * @returns {'ok'|'exchange_failed'} 循环终态码。
   */
