...
`
    ...
    function AddPresetCard(props) {
      ...
      return el('div', { className: 'dshrouter-card' },
        ...
        el('div', { className: 'dshrouter-row' },
          el('button', {
            type: 'button', className: 'dshrouter-button',
            disabled: !selected || selected.broken === true || busy || !writable,
            onClick: onSave,
          }, t('add'))),
    ...
    function AgentsPage(props) {
      ...
      const load = useCallback(async () => {
        ...
        // EVO-013：宿主预设罗盘（agentPreset.list）——独立失败面：罗盘不可达
        // 不阻塞整页（卡片显示错误提示 + 空列表，添加下拉为空），P8 可观测。
        try {
          const presetResponse = await api.agentPreset.list({})
