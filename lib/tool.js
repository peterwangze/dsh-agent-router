...
export function apply(ctx) {
  ...
  // EVO-012：内容寻址 id 附同源资产 url（与 service.imageMarkerOf 同构——
  // 服务未挂载时工具行无法注册 /router-assets/，但标记语义一致）。
  const markerOf = (ref) => {
    ...
    if (typeof safe.attachmentId === 'string' && isAttachmentId(safe.attachmentId)) safe.url = `/router-assets/${safe.attachmentId}`
  ...
  ctx.tools.register(defineTool({
    ...
    description: [
      ...
      '附件按需显式派发：`attachments` 传附件序号（0 起，按最近一条含附件的用户消息中的出现顺序）；`includeImages: true` 表示把该消息的全部图片一并转发；`attachmentIds` 传内容寻址附件 id 数组（`sha256:` 前缀，来自 imageMemory 记忆段或图片标记）直接派发；三者都不给 = 不携带任何附件。',
      '`attachmentIds`：内容寻址附件 id 数组（如 `sha256:<64位hex>`，来自 imageMemory 记忆段或 `[router:image:]` 标记中的 id）。跨轮指代：主 agent 从记忆段拿到 id 可直接精确再看原图。经 M2 附件统一编址解析（含懒注册降级）；非法/未知 id 明确报错；与 `attachments`/`includeImages` 正交可组合（按附件去重）。',
      '`files`：工作区文件路径或 http(s) URL 列表，一次可传多个不同类型。chat 类型按内容分发（图片文件内联注入，需该 agent 声明 image 能力；文本文件内联进 task；其余二进制请改传 agent 类型）；agent 类型把路径注入子代理由其用 fs 工具读取；cli 类型同样把路径注入无头 CLI 子代理（由其自身工具读取）；URL 由宿主下载到工作区 .router-files/ 后按同样规则分发。对话框上传的图片走 `includeImages`/`attachments` 附件通道，不经 `files`。',
      '`filePath`：语音识别（speech）类 agent 需要——工作区内音频文件的绝对或相对路径。',
      '普通看图无需路由：当前轮图片已由主模型原生查看时，无需调用本工具；仅在需要专业深度分析（截图/图表/OCR 精度）、跨轮旧图再查（`attachmentIds` 来自记忆段或图片标记）或用户显式要求的图像任务时才路由。',
    ...
    parameters: {
      ...
      attachmentIds: {
        type: 'array',
        items: { type: 'string' },
        description: '内容寻址附件 id 数组（如 `sha256:<64位hex>`；来自 imageMemory 记忆段或 `[router:image:]` 标记）。经 M2 附件统一编址解析（含懒注册降级），非法/未知 id 明确报错；与 attachments/includeImages 正交可组合（按附件去重）。',
      },
      files: {
        type: 'array',
        items: { type: 'string' },
    ...
    async execute(args, exec) {
      ...
      // attachmentIds（v3 Step 7 / N-8）：内容寻址附件 id 直接派发——跨轮指代
      // 记忆段 id 精确再看原图；经 M2 统一编址解析（懒注册降级 W-2），非法/
      // 未知 id 明确报错。与 attachments/includeImages 正交可组合：按附件 id
      // 去重并集（同一附件多通道显式指定只派发一次）。
      let dispatchImages = images
      if (Array.isArray(args.attachmentIds) && args.attachmentIds.length > 0) {
        const byId = await service.resolveAttachmentIds(args.attachmentIds, exec)
        const seen = new Set()
        dispatchImages = [...images, ...byId].filter((ref) => {
          const key = String(ref?.attachmentId ?? ref?.id ?? '')
      ...
      try {
        ...
        service.record({
          ...
          ok: true,
          // FIX-031 F-1 对齐：call 行统一归因到请求起点（专业 agent 行单源——
          // 不经 agent/request 遥测、无 scope 配对行，跨午夜无双计面；此处对齐
          // 主模型站点的同规则，保持全部 call 行同语义）。
          at: started,
          ms: Date.now() - started,
