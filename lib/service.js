...
import { AttachmentRegistry, isAttachmentId } from './attachments.js'
...
export class RouterService extends TypertRemoteService {
  ...
  /**
   * 可接收图片输入的视觉识别类 agent，按 id 排序。
   * 仅 chat / agent 类型可接收图片并返回文本分析；生图 agent 以 image 类型
   * （生成端点）或 cli 类型（生图 CLI 子代理，如 codex）出现，capabilities
   * 同样含 image 但语义是"产出图片"，必须排除——否则改写标记会把图片误交给
   * 生图 CLI、识别流程卡死。
   */
  listImageVisionAgents() {
    ...
    for (const [id, agent] of this.listEnabledAgents()) {
      ...
      if (type !== 'chat' && type !== 'agent') continue
  ...
  listImageGenerationAgents() {
    ...
    for (const [id, agent] of this.listEnabledAgents()) {
      const type = this.normalizeType(agent.type)
      const capabilities = Array.isArray(agent.capabilities) ? agent.capabilities : []
      const isGeneration = type === 'image' || (type === 'cli' && capabilities.includes('image'))
      if (!isGeneration) continue
      out.push([id, agent])
    }
    out.sort((a, b) => (a[0] < b[0] ? -1 : 1))
    return out
