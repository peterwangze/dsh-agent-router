...
export function apply(ctx) {
  ...
  // ── 带图轮确定性整轮路由 ────────────────────────────────────────────
  // 用户带图是确定性信号，不应依赖大脑"自觉"调 route_agent（实测大脑
  // 从不自觉）。本轮待回答的 user 输入含图片块时，直接把 provider/model
  // 切到视觉 agent 的模型：图片原生直传（不经包装路由改写），视觉模型
  // 直接看图回答；文本轮与视觉 agent 缺失时保持默认配置不动。
  ctx.on('agent/request', async ({ agent }, next) => {
    const config = await next()
    try {
      const service = ctx.get('router')
      if (!service || !service.isEnabled()) return config
      const visionAgents = service.listImageVisionAgents()
      if (visionAgents.length === 0) return config
      // 判据：从消息尾部找"最近一条未被回答的 user 输入"——尾部先出现
      // assistant/tool 结果说明本轮输入已由视觉模型回答（同轮后续步不
      // 重复路由），user 消息含 image 块才触发。
      ...
      if (!lastUser || !Array.isArray(lastUser.content) || !lastUser.content.some((block) => block && block.type === 'image')) return config
      const [agentId] = visionAgents[0]
      const resolved = await service.resolveAgent(agentId)
      if (resolved.error) return config
