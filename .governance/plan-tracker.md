...
| — | 下一轮 | **暂停（用户裁决 2026-08-31，P0/P1 修复插队不受影响）**——①用户重启复验 FIX-018（贴图不跳组）②EVO-010 迁移裁决（PoC 绿——插件自动维护 openai-codex 官方路由+token 热注入+手写层降级）③v0.4.0 发布链（含发布前小修批：FIX-017 P2×2 + FIX-018 P2-1 + reasoning 必验项）；后续待用户需求（FIX-008 / C-4+C-5 / CI RISK-001 / 插件仓 FIX-281） | — | 0.4.0 | 暂停待指令（两项用户动作待回） |
