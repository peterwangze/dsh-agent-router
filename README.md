...
1. **原生插件形态 + 开关**：普通 cordis 组合行（可 `disabled: true` 整体关闭），
   ...
   提示段清空、统计暂停，无需重启。
2. **扩展设置界面自定义 agent（分级分类卡片 + 展开配置）**：除总开关外全部配置
   组织为三个顶层分类卡片，头部整行点击展开/收起（层级：分类卡片 → 分区
   子卡片）——**专业 Agent**（核心区，前置且默认展开）、**多模态账号**
   （API Key 登录 + OAuth 官方登录 + 账号池三个子区，默认折叠）、
   **统计信息**（Agent 级与账号级两级明细 + 最近调用，默认折叠）。
