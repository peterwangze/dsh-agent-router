...
export class StatsStore {
  ...
  #foldScope(rec) {
    ...
    arm.calls += 1
    arm.lastAt = rec.at
    const modelCount = arm.models.get(`${rec.provider}/${rec.model}`) ?? { provider: rec.provider, model: rec.model, calls: 0 }
    modelCount.calls += 1
    ...
    day.calls += 1
    const dayModel = day.models.get(`${rec.provider}/${rec.model}`) ?? { provider: rec.provider, model: rec.model, calls: 0 }
    dayModel.calls += 1
    day.models.set(`${rec.provider}/${rec.model}`, dayModel)
    arm.days.set(date, day)
    arm.recent.unshift({ at: rec.at, provider: rec.provider, model: rec.model })
  ...
  #fold(rec) {
    ...
    const accountDay = accountDayMap.get(date) ?? { calls: 0, errors: 0, inputTokens: 0, outputTokens: 0, ms: 0, cost: 0, models: new Map() }
    accountDay.calls += 1
    ...
    const accountDayModel = accountDay.models.get(rec.model) ?? { model: rec.model, calls: 0 }
    ...
    agentRecent.unshift({ at: rec.at, provider: rec.provider, model: rec.model, ok: rec.ok, ms: rec.ms, inputTokens: rec.inputTokens, outputTokens: rec.outputTokens })
    ...
    accountRecent.unshift({ at: rec.at, agentId: rec.agentId, model: rec.model, ok: rec.ok, ms: rec.ms, inputTokens: rec.inputTokens, outputTokens: rec.outputTokens })
  ...
  snapshot() {
    ...
    const accountDays = dayMapSnapshot(this.accountDays)
    const recentMapSnapshot = (map) => [...map.entries()].map(([key, entries]) => ({ key, recent: [...entries] }))
    const recentByAgent = recentMapSnapshot(this.recentByAgent)
    const recentByAccount = recentMapSnapshot(this.recentByAccount)
    return {
      totals,
      recent: [...this.recent],
      series,
