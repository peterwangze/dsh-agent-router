...
| — | 下一轮 | ①**用户重启 DSH 一次验收**：FIX-018（贴图不跳组直传）+ EVO-010（openai-codex 组自动维护+token 注入状态+通路开关切换）②v0.4.0 发布链（含发布前小修批：FIX-017 P2×2 + FIX-018 P2-1 + EVO-010 P3×4 + reasoning 必验项）；后续待用户需求（FIX-008 / C-4+C-5 / CI RISK-001 / 插件仓 FIX-281） | — | 0.4.0 | 待用户重启验收 |
