...
import { installOauthLlmAdapters } from './oauth-llm.js'
import { installPreStep } from './prestep.js'
import { installPresetDefaults, installRequestTelemetry } from './preset-defaults.js'
import { clearImageMemory } from './memory.js'
import { CHATGPT_PRESET } from './oauth-credentials.js'
// ARCH-004 B5（§4.3 域 4 / §5.2）：宿主行 apply —— 域管事件订阅经 events 域
// （共享 listener + 卸载聚合）+ Node 侧 host 面注册表接线与启动自检
// （R-5：11 ctx 服务面 + 2 llm-selection 面，faces 生产非空）。
import { ctxServiceFaceProbes, probeLlmAdapterFace, probeSessionSelectFace, registerFaceProbes, runFaceProbes, subscribeEvents } from './host-abi/index.js'
...
/** 图片资产路由前缀（EVO-012 / FIX-020：插件同源 HTTP 直达，绕开宿主附件 RPC 通道）。 */
const ASSETS_ROUTE_PREFIX = '/router-assets'

/**
 * EVO-012（FIX-020 承载）：/router-assets/<attachmentId> 图片资产 handler。
 *
 * 宿主路由匹配器能力（dsh-host-webserver 源码实证）：webServer.register 除
 * `kind: 'exact'` 外支持前缀路由——`register(route)` 内按
 * `route.kind === 'exact' ? this.exact : this.prefixes` 择表；
 * `match(pathname)` 对 exact 未命中作最长前缀优先，匹配条件等价于
 * `pathname === prefix || pathname.startsWith(prefix + '/')`（宿主源码以
 * `pathname !== prefix && !pathname.startsWith(...)` 作反向短路）。
 * 因此单个 prefix 路由承载全部资产 id，无需为每个产物注册独立 exact 路由。
 * （FIX-043 批 C：两处原行号式锚实读均已漂移到他对象 ⇒ 改符号/代码串式。）
 *
 * 安全（P7 / FIX-020）：路径段只接受内容寻址附件 id（ATTACHMENT_ID_RE =
 * `sha256:64hex` 白名单，lib/attachments.js 的 ATTACHMENT_ID_RE）——`../`、嵌入穿越段与任何
 * 非内容寻址 id 一律 404；字节读取经 service.readImageAsset 的
 * registry.readStoredImage 单点（完整 ref → 宿主 readImage → 对象文件自取证 +
 * 内容哈希兜底），本通道只读且不暴露任意文件读。未知 id → 404。
 */
async function handleRouterAsset(service, req, res) {
  let rawPath
  try {
  ...
  if (!rawPath.startsWith(`${ASSETS_ROUTE_PREFIX}/`)) {
    res.writeHead(404)
    res.end('not found')
    return
  }
  let id
  try {
    id = decodeURIComponent(rawPath.slice(ASSETS_ROUTE_PREFIX.length + 1))
  } catch {
    id = ''
  }
  ...
  const asset = await service.readImageAsset(id)
  if (!asset) {
    res.writeHead(404)
    res.end('not found')
    return
  ...
  res.writeHead(200, {
    ...
    // 内容寻址：id 即内容哈希，字节不可变——强缓存安全（缩略图重复直达不再
    // 每轮 RPC/传输）。
...
export function apply(ctx) {
  ...
  // ARCH-004 B5（§5.2 启动自检 / R-5）：Node 侧 host 面 probe 清单注册 + 一次自检
  // ——B1 落的 registerFaceProbes 机制自此有生产供数（此前注册表为空，RPC
  // `router/hostFaceDiagnostics`.faces 恒 []）。面名带 `ctx:`/域前缀与客户端面
  // 区分（HostHealthCard 按名去重防互相遮蔽）。apply 期缺口面记 missing 属诚实
  // 快照——设置页打开时 RPC 读对非全绿快照做一次失败驱动复检（service.js
  // hostFaceDiagnostics，§7.1 三时机之「健康面板打开」），boot 期未就绪的陈旧
  // missing 于打开时自愈。零新增常驻 timer（§7.1 轮询纪律）。
  registerFaceProbes([
  ...
  // EVO-009：ChatGPT 订阅 OAuth 账号注册为宿主 llm provider（主 agent 模型
  // 选择器直接选用）——注册/注销随账号与开关状态热同步（oauth-llm.js 内
  // settings/updated + llm/adapters-updated 双事件联动）。
  ctx.effect(() => installOauthLlmAdapters(ctx, service), 'dsh-agent-router: oauth main-model provider')

  // EVO-010：宿主官方 openai-codex 路由维护生命周期（凭据桥 + 路由维护者）：
  // 启动首跑自证（PoC 条目接管）+ settings/updated 热同步 + 周期 tick
  // （token 临期刷新注入 + P9 parity 守卫自愈）。随宿主行 fiber 卸载。
  ctx.effect(() => service.startHostRouteMaintenance(), 'dsh-agent-router: host openai-codex route maintenance')
  ...
  ctx.effect(() => {
    ...
    // ARCH-004 B5（§4.3 域 4 / D1-9）：settings/updated 订阅收敛 events 域——
    // 原独立 ctx.on listener 与 service.js 宿主路由维护的订阅共享单宿主
    // listener（跨模块聚合，census 4→3）；ns 过滤语义逐字保持（监听
    // settings/updated 而非 settings/document-updated：settings 服务在 resolved
    // 提交后发出，读到的一定是新值——仅响应本 namespace 或全量广播）。
    const disposeEvents = subscribeEvents(ctx, [{
      event: 'settings/updated',
      consumer: 'stats-persistence',
      handler: (ns) => { if (ns === ROUTER_NS || ns === void 0) service.applyStatsSettings() },
    }])
    return () => {
      disposeEvents()
  ...
  // 两个预设事件监听：agent/created（新 agent 注册，含 resume 恢复路径）+
  // agent-preset/selected（空白会话切换预设）；EVO-013 的 agent/request 请求
  // 层监听已彻底移除（会话过程零插件开销）。主会话播种 = selectModel 显示层
  // picked 写入 + 全局默认瞬态写立即恢复（用户裁决接受的唯一豁免副作用）；
  // subagent 纯 options 修正零副作用；用户主权结构化（无模型变更监听，手动
  // 选择 = 宿主原生 picked 永不被碰）。随宿主行 fiber 卸载。
  ctx.effect(() => installPresetDefaults(ctx, service), 'dsh-agent-router: preset default model')

  // EVO-017：预设作用域请求遥测（agent/request 只读旁路——scope 统计数据源；
  // 用户统计分级需求授权的观测豁免：不改任何请求字段，仅记次）。随宿主行
  // fiber 卸载。
  ctx.effect(() => installRequestTelemetry(ctx, service), 'dsh-agent-router: preset request telemetry')
  ...
  // EVO-012（FIX-020 承载）：插件同源图片资产路由（/router-assets/<id>，前缀
  // 路由——宿主 webServer 支持 kind:'prefix'，最长前缀优先）。生图产物标记
  // 携带 url，浏览器侧 RouteImage 以 <img src> 同源直达（绕开被宿主网关
  // rejected 的 imageData RPC）；无 url 旧产物继续走 imageData RPC 回退。
  ctx.effect(() => ctx.webServer.register({
    kind: 'prefix',
    path: ASSETS_ROUTE_PREFIX,
    handler: (req, res) => handleRouterAsset(service, req, res),
  }), 'dsh-agent-router: image asset route')
