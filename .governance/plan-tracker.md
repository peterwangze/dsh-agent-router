...
| P0 | FIX-022 | EVO-013 复验失败修复：roster RPC 域名误用 `api.agentPreset.list`（客户端 api 面真实域名 = `api.agentPresets` 复数）→ TypeError「Cannot read properties of undefined (reading 'list')」，预设下拉不可用 | EVO-013（复验失败 EV-123）✅ | 0.4.2 | **开发+审查终态（待用户刷新复验）**——RCA 实锤（宿主命名不一致陷阱：JS 组名复数 agentPresets / wire 方法名单数 agentPreset.list，dsh-client-connection L6318-6324）+ 740f95a（域名对齐 + P9 三层防御：复数优先/单数回落/缺域中文诊断 throw 走既有可观测降级 + fixture 对齐宿主真实形状 {presets,authorable,hasDocument}——消断言假绿）；RED 6 FAIL 精确圈定→GREEN 全绿（smoke ALL PASSED 零回退）；R0 APPROVED_WITH_NOTES/0（机录；P2×2 台账：fixture 补 isDefault 同构 + 缺域 throw 测试场景；P3×2）；apiMock 五域 parity 审查复核无其它不一致；用户复验 = **刷新设置页**（client HMR 热推）→「预设 Agent」卡片 roster 正常加载 |
