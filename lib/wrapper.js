  ...
  const sync = () => {
    ...
    if (active.length > 0) {
      for (const provider of wanted) {
        ...
        try {
          // EVO-017 R2：twin 调用统计回调——预设会话主/subagent 的 LLM 流经
          // twin（账号级用量此前零记录 → 账号统计恒 0 根因）；agentId='twin'
          // 计入专业 Agent 统计的独立分组行 + provider=twin 路由计入账号级。
          const onCall = typeof service.record === 'function'
            ? (event) => service.record({ agentId: 'twin', ...event })
            : undefined
