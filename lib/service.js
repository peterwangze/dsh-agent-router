...
import {
  ...
  createHostRouteState, syncHostRoute, runHostRouteTick, hostRouteStatusOf, injectHostRouteToken,
} from './host-route.js'
...
export class RouterService extends TypertRemoteService {
   ...
   * updated、登录联动与后台 tick 共用；异常不外泄（tick 自吞 + 失败计数）。
   */
  queueHostRouteSync() {
    this.hostRouteQueue = this.hostRouteQueue
      .catch(() => {})
      .then(() => syncHostRoute(this.ctx, this))
      .catch((error) => {
  ...
  /** EVO-010 测试观测口：等待队列排空（串行化断言用）。 */
  async hostRouteSettled() {
    await this.hostRouteQueue.catch(() => {})
  }

  /**
   * EVO-010：启动宿主官方路由维护生命周期（index.js ctx.effect 装配）：
   * ① 立即首跑（启动自证 + PoC 条目接管）；② settings/updated 监听
   * （router ns = 账号/开关变化；llm-pi-ai ns = 条目被外部改动——自愈重建；
   * undefined = 全量广播）；③ 周期 tick（token 临期刷新注入 + parity 守卫
   * 自愈）。返回卸载器（清监听 + 清定时器）。
   */
  startHostRouteMaintenance() {
    const offSettings = this.ctx.on('settings/updated', (ns) => {
      if (ns === ROUTER_NS || ns === HOST_ROUTE_NS || ns === undefined) this.queueHostRouteSync()
    })
    const tickMs = this.hostRouteTickMs > 0 ? this.hostRouteTickMs : HOST_ROUTE_TICK_MS
    const timer = setInterval(() => { void runHostRouteTick(this.ctx, this) }, tickMs)
    timer.unref?.()
