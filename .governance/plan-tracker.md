...
| **v0.3.0** | 规划完成——M-0 裁决已定（DEC-025，2026-08-27），M-1 打包待 EVO-005 | 待定 | ChatGPT 订阅接入（C-1）：preset 账号 + 独立凭据文件 + 1455 回调 + codex-responses 协议分支 + Q2 per-protocol 能力接口 + 合规三层 kill-switch + C-9 埋点；**W-5/S-3 绑定**；**DEC-025 M-0 裁决追加**：C-3 统计（EVO-003/004）并入双主题 + 设备码流补实现（EVO-005）+ peerDeps ^0.1.0-rc.8 + RISK-001 带披露发布（CHANGELOG 用户面 A-3a + stats.persist 落盘显著披露） | EVO-001✅, EVO-002✅, EVO-003✅, EVO-004✅, FIX-006✅, EVO-005（设备码，待实施）, REL-002（规划✅） | 一键登录端到端 + 出口条件六项（§2.1）+ version-plan-v0.3.0.md（双审通过） |
