...
| MIG-001 | development | v3 迁移实施 Step 0-10（DEC-012） | 按架构 v3 §8 迁移路径逐步实施：Step 0 基线测试 → Step 1 移除整轮路由 → … → Step 10；每步独立提交+测试全绿；验收门 = D-1 五条指标（DEC-012） | 进行中——Step 0-6 完成（7cb2024/b7261d5/a23b338/374edfa/f89b8bd/f294c3c/98f04a3/2c4b194 + Step 6 本提交；R1-R8 审查链全通过，EV-011~019；V-DSH-1 经 R8 宿主源码印证成立）；下一步 Step 7（模态矩阵 + attachmentIds 参数——依赖 Step 4+5b 已满足）；P2 遗留：R8-F-01（modalityState 门控前性能，两行级，Step 7 同批）/R8-F-02（context 追加映射等测试缺口）+ R7-F-01 + R6-F-2/F-3/F-4 + R4 F-1/F-2（顺延） | P0 |
