...
| — | 下一轮 | **v0.4.0 发布裁决待用户**（全链就位：EVO-009/010 + FIX-015~019 + 用户验收已过——⑤ 控件在位/FIX-018 贴图直传/幽灵卡修复待重启后目验）；发布前小修批清单：FIX-017 P2×2 + FIX-018 P2-1 + EVO-010 P3×4 + FIX-019 P3×2 + reasoning 回传必验项；后续待用户需求（FIX-008 / C-4+C-5 / CI RISK-001 / 插件仓 FIX-281） | — | 0.4.0 | 待发布裁决 |
