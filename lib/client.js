...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const wOauthExchangeRequest = wv.object({
      accountId: wv.string(), code: wv.string(), codeVerifier: wv.string(), redirectUri: wv.string(),
    })
...
`
    ...
    const OAUTH_PRESETS = [
      { id: 'gemini', label: 'Gemini · Google', draft: { name: 'Gemini', protocol: 'gemini', baseURL: 'https://generativelanguage.googleapis.com/v1beta', authUrl: 'https://accounts.google.com/o/oauth2/v2/auth', tokenUrl: 'https://oauth2.googleapis.com/token', scope: 'https://www.googleapis.com/auth/generativelanguage' } },
    ...
    function OAuthAccountCard(props) {
      const { id, entry, tokenState, total, buckets, expanded, draft, busy, notice, t, writable, onToggle, onDraftField, onSave, onPasteToken, onAuthorize, onExchange, onDiscover, onDelete } = props
        ...
        expanded ? el('div', { className: 'dshrouter-stats' },
          ...
          el('div', { className: 'dshrouter-row' },
            ...
            el('label', { className: 'dshrouter-switch' },
              ...
              t('oauthModeCode'))),
          loginMode === 'paste' ? el('div', { className: 'dshrouter-row' },
            el('input', {
              className: 'dshrouter-input', type: 'password', autoComplete: 'off',
            ...
            }, t('oauthPaste'))) : el('div', { className: 'dshrouter-row', style: { alignItems: 'flex-end' } },
            el('div', { className: 'dshrouter-field' },
              el('span', { className: 'dshrouter-field-label' }, t('oauthClientId')),
              el('input', { className: 'dshrouter-input', value: draft.clientId ?? '', onChange: (event) => onDraftField('clientId', event.target.value) })),
            el('div', { className: 'dshrouter-field' },
              el('span', { className: 'dshrouter-field-label' }, t('oauthClientSecret')),
              el('input', { className: 'dshrouter-input', type: 'password', autoComplete: 'off', value: draft.clientSecret ?? '', onChange: (event) => onDraftField('clientSecret', event.target.value) })),
            el('div', { className: 'dshrouter-field' },
    ...
    function AddOAuthCard(props) {
      const { t, adding, setAdding, oauthDraftOf, setOauthDraft, writable, onSave } = props
      ...
      const draft = oauthDraftOf(newId)
      return el('div', { className: 'dshrouter-card' },
        el('div', { className: 'dshrouter-head' },
        ...
        el('div', { className: 'dshrouter-row' },
          ...
          el('button', {
            type: 'button', className: 'dshrouter-button',
            disabled: !newId || !ID_PATTERN.test(newId) || !writable,
        ...
        el('div', { className: 'dshrouter-row' },
          ...
          ...OAUTH_PRESETS.map((preset) => el('button', {
            ...
            disabled: !newId,
            onClick: () => setOauthDraft(newId, { ...preset.draft }),
          }, preset.label))),
        el('div', { className: 'dshrouter-field' },
          el('span', { className: 'dshrouter-field-label' }, t('fieldName')),
          el('input', { className: 'dshrouter-input', value: draft.name ?? '', onChange: (event) => setOauthDraft(newId, { name: event.target.value }) })),
        el('div', { className: 'dshrouter-row' },
          el('div', { className: 'dshrouter-field' },
            el('span', { className: 'dshrouter-field-label' }, t('oauthProtocol')),
            el('select', { className: 'dshrouter-select', value: draft.protocol ?? 'openai-completions', onChange: (event) => setOauthDraft(newId, { protocol: event.target.value }) },
              el('option', { value: 'openai-completions' }, 'openai-completions'),
              el('option', { value: 'anthropic' }, 'anthropic'),
              el('option', { value: 'gemini' }, 'gemini'))),
          el('div', { className: 'dshrouter-field' },
            el('span', { className: 'dshrouter-field-label' }, t('oauthBaseUrl')),
            el('input', { className: 'dshrouter-input', value: draft.baseURL ?? '', onChange: (event) => setOauthDraft(newId, { baseURL: event.target.value }) }))),
      )
    }
    ...
    function AgentsPage(props) {
      ...
      const defaultOauthDraft = () => ({
        ...
        clientId: '', clientSecret: '', authUrl: '', tokenUrl: '', scope: '', models: [],
      })
      const oauthDraftOf = (id) => {
        const entry = oauthById.get(id)
        const base = entry ? { name: entry.name, enabled: entry.enabled, protocol: entry.protocol, baseURL: entry.baseURL, clientId: entry.clientId, authUrl: entry.authUrl, tokenUrl: entry.tokenUrl, scope: entry.scope, models: entry.models ?? [] } : defaultOauthDraft()
      ...
      const saveOauthAccount = async (id, isNew) => {
        ...
        if (isNew) {
          ...
          ops = ['name', 'enabled', 'protocol', 'baseURL', 'clientId', 'authUrl', 'tokenUrl', 'scope'].map((field) => ({ op: 'set', path: ['oauthAccounts', id, field], value: draft[field] ?? '' }))
      ...
      return el('section', { className: 'dshrouter-section', 'aria-label': t('title') },
        ...
        el(AddOAuthCard, {
          ...
          oauthDraftOf, setOauthDraft, writable: state.writable,
          onSave: (id) => saveOauthAccount(id, true),
        }),
        ...
        ...agentIds.map((id) => {
          ...
          return el(AgentCard, {
            ...
            providers, models: state.models ?? [], catalog, oauthAccounts: oauthEntries, testResult,
    ...
    function AgentCard(props) {
      const { id, draft, t, writable, busy, notice, providers, models, catalog, oauthAccounts, testResult, stat, buckets, expanded, onToggle, onField, onSave, onDelete, onTest, onDiscover } = props
      const groups = models ?? []
      const group = groups.find((entry) => entry.provider === (draft.provider || ''))
      const oauthEntry = (oauthAccounts ?? []).find((entry) => entry.id === (draft.account || ''))
      ...
      const sourceLabel = catalog ? ({ agent: t('sourceAgent'), main: t('sourceMain'), 'provider-default': t('sourceProvider'), account: t('fieldAccount') }[catalog.source] ?? t('sourceUnknown')) : ''
      ...
      return el('div', { className: 'dshrouter-card' + (draft.enabled === false ? ' disabled' : '') },
        ...
        (oauthAccounts ?? []).length > 0 ? el('div', { className: 'dshrouter-field' },
