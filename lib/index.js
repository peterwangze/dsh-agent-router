 ...
 * `kind: 'exact'` 外支持前缀路由——`kind !== 'exact'` 注册进 prefixes 表
 * （register，lib/index.js:128-135），match 对 exact 未命中作最长前缀优先，
 * 匹配条件 `pathname === prefix || pathname.startsWith(prefix + '/')`
 * （lib/index.js:269-279）。因此单个 prefix 路由承载全部资产 id，无需为
 * 每个产物注册独立 exact 路由。
 *
 * 安全（P7 / FIX-020）：路径段只接受内容寻址附件 id（ATTACHMENT_ID_RE =
 * `sha256:64hex` 白名单，lib/attachments.js:38）——`../`、嵌入穿越段与任何
