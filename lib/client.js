...
`
    ...
    function AgentsPage(props) {
      ...
      // R8-F1：判据统一——通用 OAuth 区只排除**成员** preset 账号（专职账号卡
      // 管理）；非成员 preset（未知值）回落到通用卡（可删，非成员无独立凭据
      // 文件——通用删除安全，不再 UI 黑箱/删除死锁）。
      const oauthIds = oauthEntries.filter((entry) => !isPresetAccount(entry)).map((entry) => entry.id).sort()
      ...
      const deleteOauthAccount = async (id) => {
        ...
        // （明确告知凭据删除），与预设卡删除语义一致。
        // R8-F1：判据统一——仅**成员** preset 账号分流至 W-5 联动删除；非成员
        // preset（未知值）走通用删除（无独立凭据文件，unset 条目安全）。
        const entry = oauthById.get(id)
