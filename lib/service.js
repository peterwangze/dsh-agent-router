...
import {
  ...
  createHostRouteState, syncHostRoute, hostRouteStatusOf, injectHostRouteToken,
} from './host-route.js'
...
export class RouterService extends TypertRemoteService {
  constructor(ctx, base = {}, statsOptions = {}) {
    ...
    this.stats = new StatsStore({
      ...
      // 实际选中的账号身份键（动态：随配置/路由激活变化，读侧归一化）。
      // hostRouteStatusOf 既有状态面直读（maintained + accountId），不新增
      // host-route 读取面；路由未维护/账号未知 → null（回落独立实体 + 诚实
      // 标签，不伪装成已知账号）。归并方向 host-route → oauth，不改 oauth 键
      // 本身（accountHealth 白盒 'oauth:'+accountId 寻址不受影响）。
      hostRouteAccountKeyOf: () => {
        const status = hostRouteStatusOf(this)
        return status.maintained && status.accountId ? `oauth:${status.accountId}` : null
      },
    })
  }
   ...
   *
   * 触发条件（accountId ''→id 迁移到就绪态）：maintained && accountId——
   * 每次维护 pass 后探测（boot/settings/tick 同链），首次满足即触发并置
   * 一次性旗标（重复触发零副作用）；路由关闭/账号未知（maintained=false
   * 或 accountId=''）不触发——回落独立实体语义保持诚实（历史行不伪装）。
   * stats 未启用持久化时**不消费旗标**（无盘可重放；待 persist 启用后的
   * 下一次维护 pass 再触发）。reload 失败零影响（stats 内部兜底，回落实
   * 体保持现状——可观测降级而非中断）。
   */
  maybeReloadStatsAfterHostRouteSync() {
    try {
      ...
      const status = hostRouteStatusOf(this)
      if (!status.maintained || !status.accountId) return
