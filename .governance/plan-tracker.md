...
| MIG-001 | development | v3 迁移实施 Step 0-10（DEC-012） | 按架构 v3 §8 迁移路径逐步实施：Step 0 基线测试 → Step 1 移除整轮路由 → … → Step 10；每步独立提交+测试全绿；验收门 = D-1 五条指标（DEC-012） | 进行中——Step 0-5c 完成（7cb2024/b7261d5/a23b338/374edfa/f89b8bd/f294c3c/98f04a3 + Step 5c 本提交；R1-R7 审查链全通过，EV-011~018）；下一步 Step 6（pre-step reminder + 逃生组兜底——依赖 Step 3+5a/5b 已满足；V-DSH-1 关联）；P2 遗留：R7-F-01（result codec 错误形状断言，Step 8 补）+ R6-F-2/F-3/R6-F-4 + R4 F-1/F-2（顺延） | P0 |
