 ...
 * 折叠头**（FIX-010——宿主会话级模型选择 = picked → 日志 header → 默认，api-proxy
 * selectionFor 同序；日志 header 是"会话实际路由"的持久证据，wrapper 接管后
 * header 即以 `-router` 结尾），回落 agent.options（创建/恢复时的默认选择快照，
 * 仅新会话 header 尚未落盘时代表路由）。
 *
...
/** 会话当前模型（与 sessionProvider 同源，FIX-010 同序：header 优先）。 */
