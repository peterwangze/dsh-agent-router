...
| REL-001 | release（v0.2.1） | v0.2.1 热修版本发布（承载 FIX-001/001b/002/002b） | 用户指令"发布版本承载修改"：P0 修复不等 v0.3.0。范围 = v0.2.0 以来 main 全部；含 metrics 夹具 prepareCall 补齐（发布门禁发现，D-1-2 恢复 100%）；CHANGELOG/bump/README/tag/tarball 离线验证/归档检测 | **已完成（本地）**——EV-037；tag v0.2.1（a1ab717）；tarball 1460KB 离线验证 OK；**待办：git push + GitHub Release 上传 tarball**（需用户执行或授权） | P0 |
