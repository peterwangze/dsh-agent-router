...
| P1 | EVO-007 | 账号面板 UX 调整（用户截图红字指认 sha256:75eea8e2）——A 移除「OAuth 账号（官方登录）」不可用入口区块 + B ChatGPT 订阅登录与子代理（无头 CLI）位置交换（正式通道上移醒目位） | — | 0.3.2 | 进行中——TRIAGE-EVO-007 机录 2026-08-30（vision 识别确认需求）；Developer 实现中（a7cbd3d1） |
