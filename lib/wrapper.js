 ...
 * 为文本证据。改写器注册表（active 条目 = `{ modality, agentId, rewrite }`）
 * 决定"哪种模态激活、由哪个 agent 承接、如何改写"——多模态并存时多个
 * 条目各改各的块，聚合进同一包装路由。
 */
export function rewriteContentDeep(content, active) {
  ...
  for (const block of content ?? []) {
    ...
    if (handler) {
      changed = true
      out.push(handler.rewrite(block, handler.agentId))
      continue
    }
...
export const MODALITY_ENTRIES = [
  {
    modality: 'image',
    /** 视觉 agent 存在且总开关开启时激活，返回承接 agent id。 */
    agentOf(service) {
      if (!service.isEnabled()) return ''
      const list = service.listImageVisionAgents()
...
export function installAdmissionWrapper(ctx, service) {
  ...
  const sync = () => {
    // 已启用模态的激活条目：agentId 非空者进入聚合声明与改写分发。
    const active = MODALITY_ENTRIES
      .map((entry) => ({ modality: entry.modality, agentId: entry.agentOf(service), rewrite: entry.rewrite }))
