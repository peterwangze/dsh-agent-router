...
| MIG-001 | development | v3 迁移实施 Step 0-10（DEC-012） | 按架构 v3 §8 迁移路径逐步实施：Step 0 基线测试 → Step 1 移除整轮路由 → … → Step 10；每步独立提交+测试全绿；验收门 = D-1 五条指标（DEC-012） | 进行中——Step 0-8 完成（…/1f17ea8/12a8c71 + Step 8 本提交；R1-R11 审查链全通过含 Step 8 返工复审，EV-011~021；V-DSH-2 经宿主契约实读闭环）；下一步 Step 9（三级展示升级 + readWorkspaceFile service——V-DSH-3 前置核验；R9-F-11 image-consume 漂移顺带复核）；P3 遗留：R10×8+R11×4 记录项 + R4 F-1/F-2（顺延） | P0 |
