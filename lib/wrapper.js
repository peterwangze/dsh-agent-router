...
import { MAIN_MODEL_AGENT_ID, WRAP_ROUTE_SUFFIX } from './stats.js'
// EVO-021（ARCH-004 B3）：llm 适配器注册面形状检查收敛至 host-abi
// llm-selection 域 llmFaceOf（§4.3 三方法并集：registerAdapter/registration/
// listModels——宿主 LlmRuntime 0.1.5-rc.2 三方法齐备，与原两方法检查零回退）。
import { llmFaceOf, subscribeEvents } from './host-abi/index.js'

/** 包装路由后缀（内部 id 命名：`<provider>-router`）。
 *  FIX-031：唯一权威定义在统计归因单点 lib/stats.js（WRAP_ROUTE_SUFFIX）——
 *  路由层注册键与归因层剥离键必须同源，禁止两处字面量各自漂移（DEC-029②）。
 *  本导出面保持不变（prestep.js 与各测试按 WRAP_SUFFIX 消费）。 */
export const WRAP_SUFFIX = WRAP_ROUTE_SUFFIX

/**
 * FIX-009（P0）：image-solo（用户消息只有图片、无任何文本）经深改写后
 * content 为空数组时注入的最小非空文本。空 user content 会被下游适配器
 * 序列化为 `{role:'user', content:''}` 并遭端点拒绝（GLM 400/1213
 * 「未正常接收到prompt参数」实证；pi-ai/deepseek 适配器同型）。注入文本为
 * 行为指令形态（§5.3 通道①，与 prestep reminder 同型：行为指令，不含图片
 * 内容——防复述污染 T-1）。导出供判别测试断言精确注入形态。
 */
export const IMAGE_SOLO_PLACEHOLDER = '[图片已上传，请按系统提示操作]'
 ...
 *
 *  FIX-018 缺陷 2 核查结论（2026-08-31，EVO-009 chatgpt-oauth 实案——「原模型
 *  已多模态时 twin 直传图片」判定链成立，本模块无需行为修改，判别断言锁定）：
 *  ①探测可达：本函数经 `llm.registration(provider).adapter` **直调**原适配器
 *  resolveModel（不经宿主 seam 二次路由）；宿主 dsh-llm `registration()` 即
 *  `adapters.get(provider)`（宿主 dsh-llm 的 `registration()` 实现），插件经
 *  `llm.registerAdapter` 注册的适配器与内置适配器同面可达。EVO-009 适配器
 *  显式实现 resolveModel 并声明 `inputModalities: ['text','image']`
 *  （lib/oauth-llm.js）→ accepts=true → `stream()` 的图片块保真直传分支成立，
 *  图片块保真委托原 provider。宿主 seam 侧同证（宿主 dsh-llm 的
 *  `resolveModelInfo → resolveModelInfoFor → adapter.resolveModel` 链），能力判定链
 *  （service.decideImagePrecheck ①宿主声明路径）同源可达。
 *  ②60s TTL 缓存不缓存「注册前的失败」：twin 的注册/注销与原 provider 同事件
 *  链耦合（llm/adapters-updated → installAdmissionWrapper.sync 与 EVO-009
 *  installOauthLlmAdapters.sync 同步增删），twin stream 存续期间原适配器必在
 *  册——探测不可能观察到「未注册」态；prestep 逃生组对未注册 provider 的探测
 *  仅发生于本就无法分发（NO_ADAPTER）的会话，不构成用户可见路径。残余窗口：
 *  流中途注销原 provider（账号禁用）时本次探测回落改写——该流本身已无法直传
 *  完成，降级为标记文本不是行为退化（fail-safe 方向正确）。
 *  ③用户实证的 route_agent 中转另有来源：主模型系统提示的目录段引导
 *  （service.promptText——FIX-005 中性句为软约束，多模态大脑仍可能主动路由）
 *  与 FIX-018 缺陷 1 的多余 twin 切换，非本模块 marker/reminder 注入（prestep
 *  探测同链路 accepts=true → 不注入，判别断言 tests/routing-paths.mjs [G7]；
 *  twin 直传判定断言 tests/adapter-parity.mjs EVO-009 形状直传组）。
 */
 ...
 * EVO-017 R2：可选 onCall 回调（twin 调用统计——预设会话主/subagent 的
 * 实际 LLM 流经 twin，账号级用量此前零记录）：每次 stream 以
 * { provider(=twin 路由), model, ok, ms, inputTokens?, outputTokens?, error? }
 * 上报一次（usage chunk 提取；统计失败零影响流）。 */
export function createWrapAdapter(llm, provider, active, onCall) {
  ...
  const adapter = {
    ...
    /**
     * FIX-033：宿主 0.1.5-rc.2 LlmAdapter 原型新增契约方法（宿主 dsh-llm 的
     * `imageRequestPricing(_provider, _model)` 空体默认）——签名
     * imageRequestPricing(_provider, _model)，**同步零 I/O**（宿主 JSDoc
     * "must answer synchronously without I/O"），默认返回 undefined = "本路由
     * 未声明图片定价，消费者回落自身中性估算"；运行时消费点
     * LlmRuntime.imageRequestPricing 经 adapters.get(provider).adapter 逐路由
     * 解析，token meter 按次测量取价。
     *
     * twin 镜像语义（与 providerRetryPolicy 同构）：定价归属原适配器——twin
     * 路由是同一上游模型的包装路由（直传分支最终委托原 provider），原适配器
     * 声明了该路由的图片定价则原样透传（传参用原 provider id，与
     * providerInfo/providerRetryPolicy/listModels/resolveModel 全部镜像方法
     * 一致）；未实现/未声明 → undefined（宿主基类默认语义，消费者回落
     * 中性估算，非行为破坏）。同步契约：纯元数据直查，不 await、不走能力
     * 探测缓存路径；原适配器抛错按"未声明"回落（undefined，fail-safe 方向
     * 与 providerRetryPolicy 相同——计量元数据缺陷不击穿 token meter）。
     */
    imageRequestPricing(_route, model) {
      const base = original()
      try {
        return base && typeof base.imageRequestPricing === 'function'
          ? base.imageRequestPricing(provider, model)
          : undefined
      } catch {
        return undefined
      }
    ...
    async *stream(options) {
      // EVO-017 R2 twin 调用统计（onCall 回调）：usage/finish chunk 途经本
      // 生成器时提取（tokens 来自 usage chunk）；流抛错 = ok:false 上报后
      // 原样上抛；onCall 自身抛错零影响（统计旁路）。
      ...
      const report = (ok, error) => {
        ...
        try {
          onCall({
            // FIX-031 F-1：call 行归因到**请求起点**（与 scope 行同源时刻——遥测在
            // 请求解析时即记）。跨 UTC 午夜的流若按终点落日，会与 day1 的 scope 行
            // 分居两格 → 无 scope 覆盖的 day2 走兜底分支再计一次（同请求账号级
            // 合计 2）。record 已支持 at 注入（stats.js record 缺省 now）。
            at: startedAt,
            provider: wrapRoute,
            model: options.model,
            ok,
            ms: Date.now() - startedAt,
            ...(usage.seen ? { inputTokens: usage.inputTokens, outputTokens: usage.outputTokens } : {}),
      ...
      const trackChunk = (chunk) => {
        try {
          if (chunk && typeof chunk === 'object' && chunk.type === 'usage' && chunk.usage && typeof chunk.usage === 'object') {
            usage.seen = true
      ...
      const trackAndYield = function* (chunk) {
        trackChunk(chunk)
        yield chunk
      }
      try {
        ...
        if (present.length > 0) {
          ...
          if (accepts.every(Boolean)) {
            for await (const chunk of llm.stream({ ...options, provider })) yield* trackAndYield(chunk)
            report(true)
            return
          }
        }
        ...
        const rewritten = (options.messages ?? []).map((message) => {
          ...
          // FIX-009（P0）：改写后 content 为空 = image-solo（唯一块是图片、
          // 被整体移除）——空 user content 被下游适配器序列化为
          // `{role:'user', content:''}` 并遭端点拒绝（GLM 400/1213
          // 「未正常接收到prompt参数」实证）。注入最小非空行为指令文本
          // （§5.3 通道①形态，与 prestep reminder 同型；不含图片内容，
          // 防复述污染 T-1）。非空消息（含既有文本块）原样保留零注入。
          return result.changed
            ? { ...message, content: result.content.length > 0 ? result.content : [{ type: 'text', text: IMAGE_SOLO_PLACEHOLDER }] }
            : message
        })
        ...
        const system = systemParts.length > 0
          ...
          : options.system
        for await (const chunk of llm.stream({ ...options, provider, ...(system === options.system ? {} : { system }), messages: rewritten })) yield* trackAndYield(chunk)
        report(true)
      } catch (error) {
        report(false, error && error.message ? error.message : String(error))
        throw error
      }
    },
  }
...
export function installAdmissionWrapper(ctx, service) {
  ...
  const sync = () => {
    ...
    if (active.length > 0) {
      for (const provider of wanted) {
        ...
        try {
          // FIX-031（DEC-029①②）：twin 调用的统计上报按**主模型**语义 agent 键
          // 记账（旧 EVO-017 R2 形态 agentId='twin' 把路由中间构件引入专业 agent
          // 实体空间，用户实证归属不可达——已废弃）；provider 仍上报实际服务路由
          // `<account>-router`（传输事实），账号归一由 lib/stats.js 归因单点完成
          // （折叠侧剥 WRAP_ROUTE_SUFFIX → 用量并入真实账号，不产生伪实体账号卡）。
          const onCall = typeof service.record === 'function'
            ? (event) => service.record({ agentId: MAIN_MODEL_AGENT_ID, ...event })
            : undefined
          const handle = llm.registerAdapter([`${provider}${WRAP_SUFFIX}`], createWrapAdapter(llm, provider, active, onCall))
  ...
  sync()
  // ARCH-004 B6（§4.3 域 4 / D1-9 订阅归口收口）：llm/adapters-updated 与
  // settings/updated 两处独立 ctx.on 收敛 events 域——与本模块同根 ctx 上的
  // index.js 统计持久化 / service.js 宿主路由维护 / oauth-llm 状态同步共享
  // **同一宿主 listener**（同目标同事件名恒 1 listener，订阅 census 4→1；
  // 消费者按名分发、卸载按 consumer 摘除——零消费者才真正 dispose）。
  // ns 过滤语义逐字保持：监听 settings/updated 而非 settings/document-updated
  // ——settings 服务在 document-updated 发出时尚未提交 resolved 值
  // （bumpRevision 先于 commit），彼时 scope.get() 读到的还是旧配置 → 关闭动作
  // 不会卸载包装组。settings/updated 在 resolved 提交后发出，读到的一定是新值。
  const disposeEvents = subscribeEvents(ctx, [
    { event: 'llm/adapters-updated', consumer: 'wrapper-adapters-sync', handler: sync },
    {
      event: 'settings/updated',
      consumer: 'wrapper-settings-sync',
      handler: (ns) => {
