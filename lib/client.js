...
`
    ...
    function ModelTakeover(props) {
      const { sessionId, input, useInput, api, conversation } = props
      ...
      useEffect(() => {
        ...
        const trace = (phase, detail) => {
          ...
          if (phase === 'switched' || phase === 'skip-armed-wrapped') console.info(line)
      ...
      }, [sessionId, api, takeoverArmed, imageCount, imageConditional, capabilitySig])
      return null
    }
    ...
    function apply(ctx) {
      ...
      ctx.slots.inject('conversation.input.right', () => safeRegister({
        ...
        order: 40,
        inject: () => ({ api, conversation: conversationFace }),
