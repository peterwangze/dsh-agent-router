 ...
 * remote-events.js 的 `API_REMOTE_FORWARDED_EVENTS` 实读 19 项；B6 批静态
 * 比对测试锁定同步——镜像与宿主声明任一侧漂移即红）。Analyst D-2 锚：
 * credentials/reference-updated 在该表条目内；credentials/updated 不在表
 * （lib/client.js 死订阅根因，B5 修复承载）。
 */
 ...
 *
 * attach 失败降级（FIX-035 / §4.3 域 4「订阅失败 → 该事件消费者进降级名单」）：
 * 宿主注册抛错（ctx 已 dispose / 非法事件名等）时该事件条目**不入表** + 记
 * event-subscribe-failed（code=attach-threw，含事件名/consumer 标签/错误摘要），
 * 本调用消费者零 host listener（不记正向 event-subscribed）；其 dispose 为
 * no-op 且不悬挂，后续订阅再次 attach（重试自愈）。
 *
  ...
  for (const [event, consumers] of perEvent) {
    ...
    if (!entry) {
      ...
      try {
        ...
        // FIX-035（EVO-023 R0 P2-1）：attach 失败条目**绝不入表**。缓存死条目
        // 会让后续订阅复用（不再 attach）却记正向 event-subscribed = 静默死
        // 订阅 + 观测误导（违反 P8 / §4.3 域 4 降级行为）；失败消费者本次零
        // host listener，dispose 降级为 no-op，下次订阅自然重试 attach。
        noteHostDiag({ kind: 'event-subscribe-failed', face: event, consumer: consumers.map((item) => item.consumer).filter(Boolean).join(',').slice(0, 64), code: 'attach-threw', detail: String(error?.message ?? error).slice(0, 120) })
      }
      if (!attached) {
        disposers.push(() => {})
        continue
