...
| --- | --- | --- | --- | --- | --- | --- | --- |
| dsh-agent-router | development (6/11) | 31（29 任务 + EVO-006 + REL-004） | 31 终态（30 已完成 + 1 关闭 DEV-001）——**v0.3.1 已发布（2026-08-30，tag v0.3.1 = 5ca8b87）**：EVO-006 转正 + 收尾段承载 | 0 | 1（RISK-001 活跃——主轨道 = CI 面缺） | G4 待评（v0.3.0/v0.3.1 已发布；CI 面仍缺——RISK-001） | — |
...
| P0 | FIX-009 | 多模态视觉链路 P0：image-solo 经 vision agent（deepseek-official/deepseek-v4-flash-vision-exp）上游 400/1213「未接收到prompt」整轮失败 + 4xx 重试链疑（227s/0token）+ 并案「实验界面残留」（GUI 层旧 client 疑载 + settings 遗留键在案） | — | 未规划版本（热修候选） | 已完成（代码/审查闭环）——RCA 三案全破（**真凶 = wrapper.js 主 agent image-solo 空 content**，非 vision 路径；227s=用户中止非重试链；实验界面=存储账号名数据）+ 扩界批准（EV-088 落盘）+ 修复 8877365（IMAGE_SOLO_PLACEHOLDER 注入 + 判别测试 9/9，TDD 先红后绿）+ R0 APPROVED_WITH_NOTES/0（P2-1 补录闭环 / P2-2 台账）；十套件零回退；判别测试并入规范门控（consultant 采纳）；EV-088；**用户重启复验待做** |
| P0 | FIX-010 | GUI 显示层 P0 回归：用户消息图片气泡不渲染——**用户实证同主模型 glm-5.3 之前可显示**（v0.3.0 时代）→ v0.3.1+FIX-009 后图片消失仅剩文本标记；窗口锁定 v0.3.0→v0.3.1 产品变更（EVO-006 client/FIX-009 wrapper 已核零渲染触碰；剩 bf667b3 服务端/REL-004 telemetry/宿主交互） | — | 未规划版本 | 已完成（代码/审查闭环）——RCA 全实证：**根因 = prestep 读 options 快照非实际路由（MIG-001 起潜在缺陷），引爆 = GUI 写回 agent-default-model + 重启**（FIX-009/EVO-006/宿主全排除，会话实录 L9450/9454/9456 铁证）+ 修复 020909b（判定序 header 优先 + 判别 9/9 先红后绿）+ R0 APPROVED_WITH_NOTES/0（P1-1 回落层窗口入 v0.3.2 台账；直连纯文本 marker = T-2 设计语义如实评估）；**DEC-027 三不变量立版**（模态保真直传/原始呈现不可侵犯/全链路无感）；门控 11 套件零回退；EV-089；**用户重启复验待做** |
