...
- **ChatGPT / Claude 能 OAuth 登录吗？** ChatGPT **订阅**账号：v0.3.0 起可在设置开启 `oauthExperimental`（实验）后一键登录。官方 API 不提供 OAuth：请用官方 API Key；消费级 Web token 面向官方站后端，仅适用于兼容网关，可用「粘贴 token」方式保存。Gemini 需自建 Google Cloud OAuth Client（内置公开 Client 已被 Google 禁用：授权页报 invalid_request / invalid_scope）。
