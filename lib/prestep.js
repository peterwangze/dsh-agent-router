...
/**
 * 会话当前 provider（下一请求将使用的路由）：**优先会话日志的 request/header
 * 折叠头**（FIX-010——宿主 api-proxy selectionFor 同序的持久证据层），回落
 * **宿主 live default**（`agentDefaultModel.currentSelection()`，每次读取——
 * 与宿主 selectionFor 第三层同源），最后回落 agent.options（创建/恢复时的
 * 默认选择快照，仅无 header 且 live default 不可读时的终回退）。
 *
 * REL-005 P1-1（review-FIX-010-R0 F-1）：原实现回落链为 header → options
 * 快照，与宿主 selectionFor（picked → 日志 header → live default）偏离——
 * 新会话无 header + 创建后 default 被改写（GUI selectModel 写
 * saveDefaultModelSelection）时，options 是漂移前快照而宿主实际路由已是
 * live default → 判定分裂（误改写 → 日志污染 / 漏改写 → C-3 击穿）。
 * 补 live default 层后与宿主实际路由一致；picked 层为宿主进程内选择
 * （WeakMap，插件无访问 API——固有盲区，P3）；options 仅作终回退（宿主
 * buildRequest 中 options 只是 seedConfig，最终请求 provider 由 agent/request
 * 瀑布 selectionFor 覆盖，不作为路由判定层）。
 *
 * FIX-010 RCA（会话实录 ec6b01b0）：会话实际路由 = glm-router（request/header
 * 实证），但 agent.options = agent-default-model 恢复时快照——默认模型被切到
 * 非 wrapper 纯文本 provider（settings.yaml 18:16:20 → deepseek-official/
 * deepseek-v4-flash）后，旧实现读 options 误判 onWrapperRoute=false → 逃生组
 * 改写把 image 块替换为 route_agent 标记文本 → 宿主持久化 decision.messages
 * （dsh-agent-loop:554）→ GUI 气泡显示标记而非图片（用户实证回归）。header
 * 优先恢复 v0.3.0 解耦：wrapper 路由会话不改写日志层，模型面适配留在 wrapper
 * stream 输入层（F3）。
 */
...
/** 会话当前模型（与 sessionProvider 同源，同序：header → live default → options）。 */
