...
export class RouterService extends TypertRemoteService {
  ...
  async runChat(resolved, input) {
    ...
    const text = this.composeTask(input.task, input.extra) + this.composeChatFileText(chatFiles)
    const images = [
  ...
  async runAgentDelegation(resolved, input) {
    ...
    if (images.length > 0) contextLines.push(`本任务已附带 ${images.length} 张图片（作为图片内容直接可见），请直接查看图片内容，不要尝试按文件路径读取它们`)
  ...
  async runCli(resolved, input) {
    ...
    try {
      ...
      if (imagePaths.length > 0) contextLines.push(`本任务已附带 ${imagePaths.length} 张图片（已落盘为工作区文件，请用你的读图工具查看这些路径，不要假设图片内容）：\n${imagePaths.join('\n')}`)
  ...
  // ── 对话框图片通路（纯插件机制，不触碰主模型可见的图片块）────────────
  //
  // 主模型为纯文本（如 deepseek 系）时，harness 的 prompt 准入会拒绝图片
  // 内容，且图片块一旦进入会话历史，文本模型适配器每次请求都会报
  // UNSUPPORTED_CONTENT。因此插件自建一条通路：
  // - 客户端把对话框图片经 router/imagePrompt 交给宿主（保存为内容寻址
  //   附件），宿主把「图片标记」以纯文本注入用户消息——标记携带完整
  //   附件引用（attachmentId 内容寻址，readImage 会校验元数据，不构成
  //   信任边界问题）；
  // - 主模型按提示调用 route_agent（attachmentIds 填标记原文）交给视觉
  //   agent；视觉 agent 直接收到图片块（不经主模型）；
  // - route_agent 的结果渲染为纯文本标记（绝不把图片块写入工具结果，
  //   避免文本模型的历史被图片块击穿），浏览器侧 toolview 解析标记后
  //   经 router/imageData 取字节渲染缩略图——发送的图片与生成的图片
  //   都在对话卡片中可见。
  ...
  /**
   * 把 route_agent 的 attachmentIds 入参解析为附件引用（按 attachmentId
   * 去重）。接受两种形态：`[router:image:…]` 标记原文与 ref JSON 字符串；
   * 非法条目明确报错（绝不静默丢弃）。
   */
  parseImageRefs(entries) {
    const out = []
    const seen = new Set()
    for (const raw of Array.isArray(entries) ? entries : []) {
      if (typeof raw !== 'string' || !raw.trim()) continue
      let text = raw.trim()
      if (text.startsWith(RouterService.IMAGE_MARKER_PREFIX) && text.endsWith(']')) text = text.slice(RouterService.IMAGE_MARKER_PREFIX.length, -1)
      let ref
      try {
        ref = JSON.parse(text)
      } catch {
        throw new Error(`attachmentIds 条目不是有效的图片引用（应原样复制 "[router:image:…]" 标记或 ref JSON）：${raw.slice(0, 120)}`)
      }
      if (!ref || typeof ref !== 'object' || typeof ref.attachmentId !== 'string' || !ref.attachmentId) {
        throw new Error(`attachmentIds 条目缺少 attachmentId：${raw.slice(0, 120)}`)
      }
  ...
  /** 组装注入会话的用户消息文本：需求 + 图片标记 + 对主模型的 route_agent 引导。 */
  composeImagePromptText(text, refs, agentId) {
    const lines = []
    if (typeof text === 'string' && text.trim()) lines.push(text.trim())
    lines.push('', '[图片附件]')
    for (const ref of refs) lines.push(`${this.imageMarkerOf(ref)} ${RouterService.sanitizeImageRefName(ref.name ?? '图片')}`)
    lines.push('', [
      `用户通过对话框上传了 ${refs.length} 张图片。你（主模型）无法直接读取图片内容：`,
      `请调用 route_agent 工具把图片交给视觉 agent 分析——agent 填 "${agentId}"，attachmentIds 填每条图片标记的原文（逐个原样复制上方 "[router:image:…]" 整串，含方括号），task 写清用户需求（自包含，写明全部上下文）。`,
      '分析结果返回后，请结合图片分析回答用户；返回结果原样呈现，不要自行重复分析或改写结论。',
    ].join('\n'))
    return lines.join('\n')
  }

  /**
   * router/imagePrompt：对话框图片发送通路。
   * - 校验总开关、视觉 agent、图片数量/类型/大小（与 api-proxy 准入同源
   *   的 attachments.imageLimits）；
   * - 经附件服务保存（内容寻址、可去重；失败即中止，不触碰会话）；
   * - 把文本 + 标记注入会话（agent.followup），主模型随后调用 route_agent。
   * 失败一律返回 { ok: false, message }，绝不半写入会话。
   */
  async imagePrompt(request) {
    if (!this.isEnabled()) return { ok: false, message: '多模型路由未启用：请在 设置 → Agent 路由 中打开总开关' }
    const sessionId = String(request?.sessionId ?? '')
    if (!sessionId) return { ok: false, message: '缺少 sessionId' }
    const targets = this.listImageVisionAgents()
    if (targets.length === 0) {
      return { ok: false, message: '没有可接收图片的视觉 agent：请在 设置 → Agent 路由 中启用带 image 能力的专业 agent（chat/agent/cli 类型）' }
    }
    const requestedId = String(request?.agentId ?? '').trim()
    const target = requestedId ? targets.find(([id]) => id === requestedId) : undefined
    if (requestedId && !target) {
      return { ok: false, message: `agent "${requestedId}" 不存在或不可接收图片（可用：${targets.map(([id]) => id).join(', ')}）` }
    }
    const [finalId] = target ?? targets[0]
    const images = Array.isArray(request?.images) ? request.images : []
    if (images.length === 0) return { ok: false, message: '缺少图片附件' }
    const attachments = this.ctx.get('attachments')
    if (!attachments || typeof attachments.saveImage !== 'function') return { ok: false, message: '附件服务不可用，无法保存图片' }
    ...
    if (images.length > limits.maxImagesPerMessage) return { ok: false, message: `一条消息最多 ${limits.maxImagesPerMessage} 张图片（本次 ${images.length} 张）` }
    const decoded = []
    let totalBytes = 0
    for (let index = 0; index < images.length; index++) {
      const image = images[index]
      if (!image || typeof image.data !== 'string' || !image.data) return { ok: false, message: `第 ${index + 1} 张图片缺少数据` }
      let data
      try {
        data = decodeBase64(image.data)
      } catch {
        return { ok: false, message: `第 ${index + 1} 张图片不是有效的 base64 数据` }
      }
      if (data.length === 0) return { ok: false, message: `第 ${index + 1} 张图片为空` }
      totalBytes += data.length
      if (data.length > limits.maxImageBytes) return { ok: false, message: `第 ${index + 1} 张图片超过单图上限（${data.length} 字节 > ${limits.maxImageBytes} 字节）` }
      if (totalBytes > limits.maxMessageImageBytes) return { ok: false, message: `图片总大小超过消息上限（${limits.maxMessageImageBytes} 字节）` }
      const declared = typeof image.mediaType === 'string' ? image.mediaType : ''
      // 声明 MIME 在部署白名单内则用之；否则按魔数嗅探（saveImage 仍会校验声明与字节一致）。
      const sniffed = detectImageMediaType(data) ?? 'image/png'
      const mediaType = limits.mediaTypes.includes(declared) ? declared : sniffed
      decoded.push({ data, mediaType, name: typeof image.name === 'string' ? image.name : '' })
    }
    const refs = []
    for (const image of decoded) {
      try {
        const saved = await attachments.saveImage({ data: image.data, mediaType: image.mediaType, ...(image.name ? { name: image.name } : {}) })
        refs.push(saved)
      } catch (error) {
        // 已保存的引用是内容寻址孤儿对象，无副作用（与 api-proxy 行为一致）。
        return { ok: false, message: `第 ${refs.length + 1} 张图片无法保存：${errorMessage(error)}` }
      }
    }
    const agents = this.ctx.get('agents')
    const agent = typeof agents?.get === 'function' ? agents.get(sessionId) : undefined
    if (!agent || typeof agent.followup !== 'function') return { ok: false, message: `会话 "${sessionId}" 不存在或尚未就绪` }
    const text = this.composeImagePromptText(typeof request?.text === 'string' ? request.text : '', refs, finalId)
    try {
      agent.followup(createUserMessage({ content: [{ type: 'text', text }], source: { kind: 'user' } }))
    } catch (error) {
      return { ok: false, message: `消息注入会话失败：${errorMessage(error)}` }
    }
    return { ok: true, message: `已发送 ${refs.length} 张图片：将由视觉 agent "${finalId}" 分析`, agentId: finalId }
  }
  ...
  async runOauthChat(resolved, input) {
    ...
    const images = await this.readImagesAsDataUrls([
      ...(Array.isArray(input.images) ? input.images : []),
      ...(Array.isArray(chatFiles?.images) ? chatFiles.images : []),
    ])
  ...
  promptText() {
    ...
    return [
      ...
      '- 用户消息中带 `[router:image:…]` 图片标记（用户从对话框上传、由插件保存的图片）时：调用 route_agent，把每个标记原文（含方括号整串）放进 `attachmentIds` 参数，agent 填带 `image` 能力的视觉 agent。',
