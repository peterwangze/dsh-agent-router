...
/** argv 中 --sandbox / -s 的取值（含 --sandbox=<mode> 形态）；未指定返回 ''。 */
function sandboxFlagValue(argv) {
...
export class RouterService extends TypertRemoteService {
  ...
  resolveCliSpec(agent, platform = globalThis.process?.platform ?? '') {
    ...
    return { command, base, args: finalArgs, format, parse: preset?.parse, sandbox: sandboxFlagValue(finalArgs) }
  }
  ...
  async runCli(resolved, input) {
    ...
    try {
      ...
      // 沙箱提示与生效配置保持一致：启用 CLI 沙箱（workspace-write/read-only）
      // 时提示路径受限；未启用（danger-full-access 或 claude/gemini 旁路模式）
      // 时要求把读写收敛在工作区内。
      const sandboxed = spec.sandbox !== '' && spec.sandbox !== 'danger-full-access'
      const contextLines = [
        sandboxed
  ...
  async cliLogin(request) {
    ...
    const win = globalThis.process?.platform === 'win32'
    const label = `${agent.name || spec.command}`
