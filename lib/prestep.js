...
/** 宿主 live default 读取（REL-005 P1-1）：`agentDefaultModel.currentSelection()`
 *  每次读取（创建后 default 漂移可被感知——与宿主 api-proxy selectionFor 第三层
 *  同源，host:5532 `defaultModelSelection: () => ctx.agentDefaultModel.currentSelection()`）；
 *  服务缺失/无 currentSelection/抛错/空值 → null（调用方回落 options 终回退）。
 *  P-v2 原则 8：能力判定缺失可观测——此处缺失不静默吞错，回落链语义确定。 */
