...
`
    ...
    /** EVO-012 批二 A：ref → 同源直达 url（标记 url 字段优先；否则从
     *  attachmentId 推导——内容寻址 id 全局可达，无 url 的旧产物/旧会话标记
     *  自愈；经 encodeURIComponent 防御路径注入，宿主路由白名单兜底）。
     *  RouteImage 与本会话集合视图共用（P5 单点）。 */
    function directAssetUrlOf(ref) {
      if (typeof ref?.url === 'string' && ref.url) return ref.url
      if (typeof ref?.attachmentId === 'string' && ref.attachmentId) return `/router-assets/${encodeURIComponent(ref.attachmentId)}`
      return ''
    }
