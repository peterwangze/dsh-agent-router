...
| MIG-001 | development | v3 迁移实施 Step 0-10（DEC-012） | 按架构 v3 §8 迁移路径逐步实施：Step 0 基线测试 → Step 1 移除整轮路由 → … → Step 10；每步独立提交+测试全绿；验收门 = D-1 五条指标（DEC-012） | 进行中——Step 0-4 完成（7cb2024 + b7261d5 + a23b338 + Step 4 imageMemory 本提交；R1-R4 审查链全通过——R4 APPROVED_WITH_NOTES unresolved_blockers=0，EV-011/012/013/014）；下一步 Step 5a-5c（附件统一编址层）；P2 遗留 F-1/F-2（R4，Step 5 顺带） | P0 |
