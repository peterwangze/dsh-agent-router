...
export function probeImageDimensions(data) {
  ...
  if (u8[0] === 0x52 && u8[1] === 0x49 && u8[2] === 0x46 && u8[3] === 0x46 && u8[8] === 0x57 && u8[9] === 0x45 && u8[10] === 0x42 && u8[11] === 0x50 && u8[12] === 0x56 && u8[13] === 0x50) {
    ...
    if (u8[14] === 0x38 && u8[15] === 0x4c) {
      // VP8L（无损）：签名 u8[20]=0x2f；其后 LSB-first 位流，前 14 位 = 宽-1、
      // 次 14 位 = 高-1（共 28 位，跨 u8[21..24]）。
      // FIX-007 F-2：旧公式只拼 6+4 位（宽）与 4+2+4 位（高）——对真实
      // sharp VP8L 产物读出错误尺寸（实测 320x200 读成 128x29），宿主严格
      // 校验（readImageFile 宽高全等）下自取证整链失效。按位流语义重写。
      if (u8[20] !== 0x2f) return undefined
      const bits = ((u8[21] | (u8[22] << 8) | (u8[23] << 16) | (u8[24] << 24)) >>> 0)
      const w = (bits & 0x3fff) + 1
      const h = ((bits >>> 14) & 0x3fff) + 1
      return w > 0 && h > 0 ? { width: w, height: h } : undefined
    }
    // VP8（有损，FourCC "VP8 "＝0x56 0x50 0x38 0x20——注意第 4 字符是空格）：
    // RIFF/WEBP 布局为 0-3 "RIFF"、4-7 文件大小、8-11 "WEBP"、12-15 FourCC、
    // 16-19 chunk 大小（小端）、20-22 3 字节帧标签（位 0 = 帧类型，0 = 关键帧）、
    // 23-25 起始码（0x9d 0x01 0x2a）、26-27 宽-1、28-29 高-1（小端，14 位有效）。
    // FIX-007 F-2：旧代码把 u8[14..17]（"8 "+chunk 大小首字节）当帧标签比对，
    // 对真实 VP8 WebP（sharp 有损编码产物）恒不命中 → undefined → 宿主严格
    // 校验下自取证整链失效（attachmentIds 不可解析）。按规范偏移重写。
    if (u8[14] === 0x38 && u8[15] === 0x20) {
      const u16le = (i) => u8[i] | (u8[i + 1] << 8)
      // 帧标签位 0 = 帧类型（0 = 关键帧；非关键帧不含尺寸字段）。
      if ((u8[20] & 0x01) !== 0) return undefined
      // 14 位像素代码直接存实际值（RFC 6386 keyframe 头；与 VP8X/VP8L 的
      // "-1 编码" 不同——实测 sharp 640x480 产物 offset26-27 = 0x0280 = 640）。
...
export class AttachmentRegistry {
  ...
  /**
   * FIX-007 F-1：宿主图片读取单点（完整 ref 构造 + 自取证降级）。
   *
   * 宿主演进防御（P9 原则 9 / FIX-003 同型）：宿主 readImageFile 对 ref 执行
   * 元数据全等校验（mediaType/bytes/width/height），裸 id ref 恒 ATTACHMENT_CORRUPT。
   * 本方法是 materialize/read/imageData 等一切"按 id 取存储图片字节"路径的唯一
   * 单点（P5 泛化——按形状适配而非版本判定）：
   * 1. 已注册条目 → 用条目元数据构造完整 ref 调宿主 readImage（条目可能由
   *    saveImage/selectAttachments/懒注册建立，字段齐全）；
   * 2. 失败（含条目字段缺失导致的校验拒绝）→ lazyImageFromObjectFile 自取证
   *    （对象文件 → 魔数/尺寸探测 → 完整 ref 重试 → 哈希兜底）；
   * 3. 成功后以宿主返回的权威 ref 回填注册条目（字段漂移自愈——归一化改写、
   *    尺寸修正等宿主演进下的元数据纠偏）。
   *
   * @param {string} id - 内容寻址附件 id。
   * @param {object} [options]
   * @param {AbortSignal} [options.signal] - 传递给宿主读取的取消信号。
   * @returns {Promise<{ ref: object, data: Uint8Array } | undefined} - 宿主权威
   *   ref + 字节；不可解析返回 undefined（调用方映射 ATTACHMENT_UNKNOWN）。
   */
  async readStoredImage(id, options = {}) {
    const attachments = this.attachmentsService
    if (!attachments) return undefined
    const entry = this.peek(id)
    if (entry && typeof entry.mediaType === 'string' && entry.mediaType && typeof entry.bytes === 'number'
      ...
      try {
        ...
        if (stored && stored.data) {
          this.registerEntry({ ...entry, ...(stored.ref && typeof stored.ref === 'object' ? {
            ...(typeof stored.ref.mediaType === 'string' && stored.ref.mediaType ? { mediaType: stored.ref.mediaType } : {}),
            ...(typeof stored.ref.bytes === 'number' ? { bytes: stored.ref.bytes } : {}),
          ...
          return { ref: stored.ref && typeof stored.ref === 'object' ? stored.ref : ref, data: stored.data }
        }
      } catch { /* 条目元数据与宿主不一致（演进漂移）：走自取证降级 */ }
    }
    // 自取证降级（含未注册 id / 条目字段缺失 / 宿主拒绝）：读对象文件构造
    // 完整 ref 重试，仍失败时哈希兜底直接使用对象字节。
    const forensic = await this.lazyImageFromObjectFile(id, attachments)
    if (forensic && forensic.data) {
      const ref = forensic.ref && typeof forensic.ref === 'object' ? forensic.ref : { attachmentId: id }
      this.registerEntry({
        id,
        mediaType: typeof ref.mediaType === 'string' && ref.mediaType ? ref.mediaType : 'image/png',
        bytes: typeof ref.bytes === 'number' ? ref.bytes : forensic.data.length,
        ...(typeof ref.width === 'number' ? { width: ref.width } : {}),
        ...(typeof ref.height === 'number' ? { height: ref.height } : {}),
        ...(typeof ref.name === 'string' && ref.name ? { name: ref.name } : {}),
        source: 'image-block',
      })
      return { ref, data: forensic.data }
    }
    return undefined
  }
  ...
  async materialize(id, options = {}) {
    ...
    if (typeof entry.workspacePath === 'string' && entry.workspacePath) {
      ...
      if (!bytes) {
        // FIX-007 F-1：读取经完整 ref 单点（宿主元数据严格校验下裸 id 恒拒——
        // 旧内联 readImage({attachmentId}) 在条目已注册（peek 命中、bytes=null）
        // 时必败 ATTACHMENT_CORRUPT → 物化整链断）。
        const stored = await this.readStoredImage(id)
        if (!stored || !stored.data) throw attachmentError(ATTACHMENT_ERROR_CODES.ATTACHMENT_UNKNOWN, `附件读取失败（${id}）：未注册且宿主无法读取`)
        bytes = stored.data
        // 宿主权威元数据回填条目（extension 判定用最新 mediaType）。
        if (stored.ref && typeof stored.ref.mediaType === 'string' && stored.ref.mediaType) entry.mediaType = stored.ref.mediaType
      }
  ...
  async read(id, options = {}) {
    ...
    // FIX-007 F-1：读取经完整 ref 单点（宿主元数据严格校验下裸 id 恒拒——
    // 旧 readImage({attachmentId}) 必败 ATTACHMENT_CORRUPT）。
    const stored = await this.readStoredImage(id, { signal })
    if (!stored || !stored.data) throw attachmentError(ATTACHMENT_ERROR_CODES.ATTACHMENT_UNKNOWN, `附件读取失败（${id}）：未注册且宿主无法读取`)
