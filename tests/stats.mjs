...
export async function runStatsTests(check) {
  ...
  console.log('unified group derivations (EVO-017 R2):')
  {
    ...
    // FIX-031 对齐（旧断言锁定的是缺陷形态）：包装路由键 'glm-local-router' 不
    // 得成为独立账号实体，账号维度按规范键 'glm-local' 聚合（D1 根治）。
    const acctDay = (snap.accountDays ?? []).find((entry) => entry.key === 'glm-local')
    check('R2-2: accountDays 按账号聚合（FIX-031：包装路由键归并规范账号键）', !!acctDay && acctDay.days[0].calls === 1 && acctDay.days[0].models[0].model === 'glm-5.3')
    const agentRecent = (snap.recentByAgent ?? []).find((entry) => entry.key === 'vision')
    const acctRecent = (snap.recentByAccount ?? []).find((entry) => entry.key === 'glm-local')
    ...
    check('R2-3b: 账号级明细行自带规范 provider（FIX-031 D2——旧实现不带 provider 且内部 agentId 外显）', !!acctRecent && acctRecent.recent[0].provider === 'glm-local' && acctRecent.recent[0].agentId === 'main-model')
    await store.flush()
    ...
    const reAcctRecent = (reSnap.recentByAccount ?? []).find((entry) => entry.key === 'glm-local')
