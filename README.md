...
[![version](https://img.shields.io/badge/version-v0.1.1-blue)](https://github.com/peterwangze/dsh-agent-router/releases)
...
- 🔑 **多模态账号**：任意服务商 API Key 配置式添加（官方/中转/本地部署同一条路径，无预设无登录）；OAuth 官方授权（Gemini 内置公开 Client 零配置一键授权）；账号池按健康/用量/轮询策略自动选号与失败切换
...
固定版本：把命令中的 `main` 换成版本号，如 `v0.1.1`。

### 离线安装

1. 下载发行包：[dsh-agent-router-v0.1.1.tar.gz](https://github.com/peterwangze/dsh-agent-router/releases/download/v0.1.1/dsh-agent-router-v0.1.1.tar.gz)
2. 解压并进入包目录：

```powershell
# Windows
tar -xzf dsh-agent-router-v0.1.1.tar.gz
cd dsh-agent-router-v0.1.1
