...
`
    ...
    const zh = {
      ...
      collapse: '收起',
      imageAttach: '添加图片',
      imageUnavailable: '当前输入框不可用：无法添加图片',
      imagePickTitle: '选择图片',
    ...
    const en = {
      ...
      imageUnavailable: 'Cannot add images: the input is unavailable right now',
    ...
    function ModelTakeover(props) {
      ...
      const takeoverArmed = visionAgentsOf(catalog).length > 0
    ...
    /** composer 工具行附件按钮：选图 → 原生草稿图片栏（预览/移除都走原生机制；
     *  发送走原生回车，由准入包装放行）。 */
