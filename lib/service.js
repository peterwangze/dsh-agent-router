...
export class RouterService extends TypertRemoteService {
   ...
   * 返回 { id, agent, provider, model, source, error? }。
   */
  async resolveAgent(id) {
    const agent = this.getAgent(id)
    if (!agent) return { id, agent: null, provider: '', model: '', source: 'unknown', error: `未知 agent "${id}"（可用：${this.listEnabledAgents().map(([key]) => key).join(', ') || '无'}）` }
    ...
    if (!provider) return { id, agent, provider, model, source, error: '未解析到服务商：请为该 agent 配置服务商，或先在 设置 → 模型 中配置主模型' }
    if (!model) return { id, agent, provider, model, source, error: `服务商 "${provider}" 没有可用模型（未注册或未配置）；请先在 设置 → 模型 中完成该服务商的配置` }
    return { id, agent, provider, model, source }
  }
  ...
  promptText() {
    ...
    const lines = entries.map(([id, agent]) => {
      ...
      const meta = provider && model ? `${provider}/${model}` : provider ? `${provider}/*` : '跟随主模型'
  ...
  async test(request) {
    ...
    try {
      ...
      const result = await this.runChat({ ...resolved, agent }, { agentId: id, task: '连通性测试：请只回复 OK', extra: '', images: [] })
      return {
        ok: true,
