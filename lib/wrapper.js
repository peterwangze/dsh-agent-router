 ...
 *  直传判定必须覆盖全部进入模型输入的模态块）。 */
function requestHasModality(messages, modality) {
  const walk = (content) => {
    for (const block of content ?? []) {
      if (!block) continue
 ...
 *  （安全回落改写——宁可改写不可漏图击穿端点，§5.2.3）。 */
async function sourceAcceptsModality(originalAdapter, provider, model, modality) {
