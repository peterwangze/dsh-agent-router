...
import { llmOf, credentialsOf, attachmentsOf, subagentsOf, agentDefaultModelOf, hostVersionsOf, faceHealthSnapshot, hostDiagnostics } from './host-abi/index.js'
...
export class RouterService extends TypertRemoteService {
   ...
   * 或**本模块自身写入**——llm 触发（F-1 回环 gate）；undefined = 全量广播
   * ——user 触发）；③ 周期 tick（token 临期刷新注入 + parity 守卫自愈——
   * **走同一串行队列**（F-2：tick 不再旁路，登出→复活窗口 ≤30s 语义保持））。
   * 返回卸载器（清监听 + 清定时器）。
   */
  startHostRouteMaintenance() {
    const offSettings = this.ctx.on('settings/updated', (ns) => {
      if (ns === ROUTER_NS || ns === HOST_ROUTE_NS || ns === undefined) {
        this.queueHostRouteSync(ns === HOST_ROUTE_NS ? 'llm' : 'user')
      }
    })
   ...
   * 感知）：hostVersions（域 6 版本遥测）+ faces（FaceHealth 缓存快照纯读，
   * 不触发 probe——§7.1 惰性纪律）+ diag（noteHostDiag 环形最近 64 条）。
   */
  hostFaceDiagnostics() {
    return {
