...
[![version](https://img.shields.io/badge/version-v0.1.8-blue)](https://github.com/peterwangze/dsh-agent-router/releases)
...
- 💬 **对话框图片能力**：启用视觉类专业 agent（能力标签含 `image`）后，输入框出现「添加图片」按钮——附件图片直接进原生草稿栏，经插件发送通路交给视觉 agent 分析；发送的图片与生成的图片都以缩略图显示在对话的 route_agent 工具卡片中（纯插件机制：图片以内容寻址附件保存、以文本标记进入会话，纯文本主模型全程不受影响）
...
固定版本：把命令中的 `main` 换成版本号，如 `v0.1.8`。

### 离线安装

1. 下载发行包：[dsh-agent-router-v0.1.8.tar.gz](https://github.com/peterwangze/dsh-agent-router/releases/download/v0.1.8/dsh-agent-router-v0.1.8.tar.gz)
2. 解压并进入包目录：

```powershell
# Windows
tar -xzf dsh-agent-router-v0.1.8.tar.gz
cd dsh-agent-router-v0.1.8
...
- **对话框图片**：启用带 `image` 能力标签的视觉类专业 agent（chat / agent / cli 类型）后，对话输入框出现「添加图片」按钮——选中图片进入原生附件栏，输入框上方出现发送条（可下拉选择视觉 agent）；点「发送给专业 agent」后，插件把图片保存为内容寻址附件、以文本标记注入消息，主 agent 调用 route_agent 交给视觉 agent 分析。发送的图片与生成的图片都以缩略图显示在对话中的 route_agent 工具卡片里（点击查看原图），纯文本主模型全程不接触图片字节
...
- **纯文本主模型怎么发送对话框图片？** 主模型不支持图片输入时，harness 默认拒绝带图片的消息（且图片块进入历史会让纯文本模型的每次请求报 UNSUPPORTED_CONTENT）。启用带 `image` 能力的视觉类专业 agent 后，插件接管图片发送：输入框「添加图片」→ 发送条交给视觉 agent。图片以内容寻址附件保存、以 `[router:image:…]` 文本标记进入会话历史，因此既不触发准入拒绝也不污染模型上下文；route_agent 的结果同样以文本标记渲染，由浏览器侧工具卡片经插件接口取字节显示缩略图（点击查看原图）。主模型本身支持图片时，原生粘贴 / 拖拽发送仍照常可用。
