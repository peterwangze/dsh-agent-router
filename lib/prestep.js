 ...
 *    参考实现 dsh-vision-router 的 `activateDeepTools()` 自动挂载 reminder
 *    同款约束（其注释逐字写 "session validation requires an `id`"）；文本按
 *    §5.3 通道①：行为指令，
...
// EVO-021（ARCH-004 B3）：live 全局默认读取迁 host-abi llm-selection 域
// （唯一实现——本模块原本地副本删除，P5 双份去重；签名/语义不变）。
import { liveDefaultSelection } from './host-abi/index.js'
 ...
 *   参考实现 dsh-vision-router 的 `adapterHandlesImages` 同款取舍）。
...
/**
 * 会话当前 provider（下一请求将使用的路由）：**durable pending 选择优先**
 * （FIX-029-A）→ 会话日志 request/header 折叠头（FIX-010）→ 宿主 live
 * default（`agentDefaultModel.currentSelection()`，每次读取——与宿主
 * selectionFor 第三层同源）→ agent.options（创建/恢复时快照，终回退）。
 *
 * FIX-029-A 根因（会话 94adf09f 实证，2026-09-05）：宿主 0.1.2-rc.1 把
 * selectModel 持久化为 durable `model/selection` 会话事件，selectionFor 的
 * picked 首层由投影 pending 初始化（dsh-api-session-controller
 * types/agent.js:297-318：`stateOf(session,'modelSelection').pending`）。
 *
 * **FIX-043 批 C 判定（保留原文，不单侧去行号）**：上行锚**承载判据**——
 * `tests/host-contract.mjs` 的 S2 `extra.signatures` 以本串作跨文件逐字在位
 * 断言（该断言的对象文件 = `tests/fix-029-host-contract.mjs`，同串亦在其内）。
 * 按「真正承载判据的锚 MUST 先改写判据再改锚」，而判据与对象侧均在 tests 面、
 * 不在本批锁面 ⇒ 本批不改锚、不单侧去行号（单侧去行号会造成三处形态不一），
 * 登记移交三处同步（判据声明 + 对象文件 + 本处）。宿主对象实读复核（批 C 只读
 * 取证）：`selectionFor` 的 picked 首层确由 `projectionState.pending` 初始化、
 * `selection.current` 回退链（logged header → `agentDefaultModel` 默认）逐字在位
 * ⇒ 锚现值与对象语义相符，仅形态待批 F 统一。
 * pre-step 时**当前请求的 header 尚未写入**——日志末条 header 是旧 provider
 * （stale）；用户手切 twin（或插件接管）后的首个图片轮：旧链读 stale header
 * → 误判 onWrapperRoute=false → 逃生组改写把 image 块替换为标记文本并持久化
 * （[94184 手切 twin]→[94190 图片被改写]→[94192 实际路由=twin] 三点实证）。
 * 修复 = pending 优先层与宿主 selectionFor 同序（picked(pending) → header →
 * live default → options）；投影服务缺失/未注册/抛错（旧宿主形态）→ 回落
 * 既有链，行为不变（双向兼容）。
 */
function sessionProvider(agent, ctx) {
  const pending = pendingModelSelection(agent, ctx)
  if (pending && typeof pending.provider === 'string' && pending.provider) return pending.provider
  try {
    const header = agent?.session?.requestHeader?.()
    if (header && typeof header.config?.provider === 'string' && header.config.provider) return header.config.provider
  } catch { /* requestHeader 抛错：按未落盘处理，回落 live default / options 终回退 */ }
  const live = liveDefaultSelection(ctx)
  if (live && typeof live.provider === 'string' && live.provider) return live.provider
  const options = agent?.options
  if (options && typeof options.provider === 'string' && options.provider) return options.provider
  return ''
}

/** 会话当前模型（与 sessionProvider 同源，同序：pending → header → live default → options）。 */
function sessionModel(agent, ctx) {
  const pending = pendingModelSelection(agent, ctx)
  if (pending && typeof pending.model === 'string' && pending.model) return pending.model
  try {
    const header = agent?.session?.requestHeader?.()
    if (header && typeof header.config?.model === 'string' && header.config.model) return header.config.model
  } catch { /* 同上 */ }
  const live = liveDefaultSelection(ctx)
  if (live && typeof live.model === 'string' && live.model) return live.model
  const options = agent?.options
  if (options && typeof options.model === 'string' && options.model) return options.model
  return ''
}

/** 宿主 durable 模型选择投影读取（FIX-029-A 单点）：`sessionProjections.stateOf
 *  (session, 'modelSelection')` → `{lastUsed, pending}`；pending = 未被匹配
 *  request/header 消费的下一次选择（宿主 selectionFor picked 首层的持久源，
 *  宿主 selectModel 即经 `agent.session.append('model/selection')` 写入）。
 *  服务缺失/键未注册/会话形态漂移抛错 → null（调用方回落既有判定链——旧宿主
 *  无此投影，行为不变；P-v2 原则 8：判定缺失回落方向确定，不静默吞错）。 */
function pendingModelSelection(agent, ctx) {
  try {
    const projections = ctx && typeof ctx.get === 'function' ? ctx.get('sessionProjections') : undefined
...
export function installPreStep(ctx, service) {
  const handler = async ({ agent, messages, turn, step, signal }, next) => {
    ...
    try {
      ...
      // T-1）。
      const provider = sessionProvider(agent, ctx)
      const model = sessionModel(agent, ctx)
