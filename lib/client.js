      ...
      for (const [event, consumers] of perEvent) {
        ...
        if (!entry) {
          ...
          try {
            ...
            noteHostFaceDiag({ at: Date.now(), kind: 'event-subscribe-failed', face: event, consumer: consumers.map((item) => item.consumer).filter(Boolean).join(',').slice(0, 64), code: 'attach-threw', detail: String(error && error.message ? error.message : error).slice(0, 120) })
          }
          if (!attached) {
            disposeAll.push(() => {})
            continue
