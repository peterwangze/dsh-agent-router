...
export const wireCodecs = {
  ...
  /**
   * router/imagePrompt 请求：把对话框附件图片与文本交给插件保存并注入
   * 会话（视觉 agent 随后经 route_agent 分析）。images 为 base64 图片
   * 载荷（与 api-proxy 的 prompt 图片载荷同构）。
   */
  imagePromptRequest: v.object({
    sessionId: v.string(),
    text: v.string(),
    agentId: v.string(true),
  ...
  /** router/imagePrompt 结果。 */
  imagePromptResult: v.object({
    ok: v.boolean(),
    message: v.string(),
    agentId: v.string(true),
  }),
