...
/** 宿主官方路由键（lib/host-abi/version.js 的 HOST_ROUTE_PROVIDER 镜像——
 *  权威单点自 EVO-022 B4 ①翻转至 version.js，host-route.js 侧仅 re-export；
 *  配置归「设置 → 模型」，账号管理区不渲染，但用量是真实账号用量，保持独立实体）。 */
const HOST_ROUTE_ACCOUNT_KEY = 'openai-codex'

/** 插件自注册**路由键** → 真实账号键别名表：路由层造的键不是账号实体，读侧
 *  折回它所服务的账号。唯一条目 `chatgpt-oauth`（权威源 lib/oauth-llm.js 的
 ...
 *
 * D6（宿主官方路由归并，返工批 3）：`openai-codex` 是通路构件而非账号——
 * 宿主路由与 oauth 通路共用同一份 ChatGPT 订阅凭据（host-route.js
 * selectHostAccount 选首个启用账号注入同一凭据库）。hostRouteAccountKeyOf
 * resolver（service 注入，读路由 state.accountId——动态映射，随配置可变）
 * 返回账号身份键时，host-route 用量归并到该键（accountKind 继承 'oauth'，
 * 与 oauth 通路行同一聚合实体、同一 roster 名）；resolver 不可用/路由未
 * 激活/账号未知 → 保持独立实体（kind 'host-route'，显示层出诚实标签，不
 * 伪装成已知账号）。盘面行不改（读侧归一化，与既有架构一致）。
 *
 * 纯函数、无副作用、永不抛（非法形态一律落到清洁缺省值；resolver 抛错由
 * 调用方兜底——#normalize 包装 try/catch，本函数对非法 resolver 返回值
 * （非字符串/缺前缀/空 id）一律回落独立实体）。
 * @param {{provider?: unknown, agentId?: unknown}} event
 * @param {(() => string | null | undefined) | null} [hostRouteAccountKeyOf]
 *   宿主路由当前选中账号身份键 resolver（期望 'oauth:<accountId>' | null）
 * @returns {{accountKey: string, accountKind: string, accountLabel: string,
 *   agentKey: string, mainModel: boolean}}
 */
export function normalizeAttribution(event = {}, hostRouteAccountKeyOf = null) {
  ...
  if (!accountKey) {
    ...
    let merged = null
    if (typeof hostRouteAccountKeyOf === 'function') {
      try {
        merged = hostRouteAccountKeyOf()
      } catch { merged = null }
    }
    if (typeof merged === 'string' && merged.startsWith(OAUTH_ACCOUNT_PREFIX) && merged.length > OAUTH_ACCOUNT_PREFIX.length) {
      accountKey = merged
      accountKind = 'oauth'
    } else {
...
export class StatsStore {
  ...
  constructor(options = {}) {
    ...
    /**
     * D6（返工批 3）：宿主官方路由当前选中账号身份键 resolver（getAgentName
     * 先例）——service 注入 `() => 'oauth:<accountId>' | null`（读 host-route
     * state.accountId，动态映射）。null/缺省 = 路由未激活或账号未知 →
     * host-route 用量保持独立实体（显示层诚实标签）。
     */
    this.hostRouteAccountKeyOf = typeof options.hostRouteAccountKeyOf === 'function' ? options.hostRouteAccountKeyOf : null
   ...
   * D7（返工批 3）：preset 允许空串——无预设会话（如直用宿主官方
   * deepseek-official 默认模型）的请求同样产 scope 行进账号视图（账号级计数
   * 覆盖全部路由形态）；空串行不进预设视图（#foldScope 内分流，预设维度
   * 对无预设请求不适用）。其余校验（provider/model/at）保持。
   */
  recordScope(event) {
    let rec
    try {
   ...
   *  D7（返工批 3）视图分流：preset='' 的无预设行只进账号视图
   *  （#foldAccountScope 全量计）不进预设视图——预设维度对无预设请求不
   *  适用，不生成空名预设卡。 */
  #foldScope(raw) {
    const { accountKey, accountKind, accountLabel } = this.#normalize(raw)
  ...
  /**
   * 实例级归一化入口（D6）：归一化单点 + 本实例的宿主路由账号 resolver。
   * 全部内部折叠/派生调用经此（模块级导出 normalizeAttribution 保持纯函数
   * 形态供外部直测；resolver 抛错在此兜底 → 回落独立实体，绝不影响记录路径）。
   */
  #normalize(raw) {
    try {
      return normalizeAttribution(raw, this.hostRouteAccountKeyOf)
    } catch {
      return normalizeAttribution(raw, null)
    }
  }
  ...
  #fold(raw) {
    const attribution = this.#normalize(raw)
  ...
  /**
   * FIX-031 D8（微批 5）：安全重载——清空内存聚合后从盘面全量重放。
   *
   * 动机（启动时序竞态）：setPersist(true) 启动即 load()，此刻宿主路由维护
   * 未完成（syncHostRoute 异步等待宿主 settings/oauthAccounts 就绪）→
   * hostRouteAccountKeyOf resolver 返回 null → 历史 host-route 行全部折叠进
   * 回落独立实体；内存聚合一次成型不再重折 → 回落卡固化。维护完成
   * （accountId 就绪）后由 service 触发本方法重放，resolver 已就绪 → 归并
   * 正确（读侧归一化语义保持，盘面行零改写）。
   *
   * 清空语义：仅清聚合态（totals/accountTotals/series/accountSeries/recent/
   * days/presetStats/accountScope/agentDays/accountDays/recentByAgent 与
   * recentByAccount/detail）；selfReport 计数保留；盘面与队列不动。
   *
   * 并发防护（P7 永不抛）：与 setPersist 同经 #persistTransition 串行链；
   * flush 在途队列先落盘；**清空前快照在途队列**——flush 后仍残留的行
   * （记录于 flush 之后、清空之前：不在盘上、内存折叠即将被清）在重放后
   * 按身份回补恰一次；重放窗口内新到达的行（内存折叠发生在清空之后）不
   * 在快照内、只计内存一次——三类行互斥，不丢不双。
   */
  async reload() {
    if (!this.persist) return this.statsSelfReport()
    ...
    try {
      ...
      // N2（R3 保留项）：等待在途转换结束后复检——镜像 setPersist 自有纪律
      //（「等待在途转换结束后再比较」）。前驱为 setPersist(false) 时：其完成
      // 后 persist 已为 false，此处必须早退，否则清空内存聚合而 load() 因
      // persist=false 早退不重放 → persist-off 会话期统计归零。
      if (!this.persist) return this.statsSelfReport()
      if (this.#flushing) {
        try { await this.#flushing } catch { /* in-flight flush 收尾失败不阻断 */ }
      }
      try { await this.flush() } catch { /* flush 失败不阻断——盘面已有序，残留批次按快照回补 */ }
      ...
      // 在途回补（快照 = 记录于 flush 后、清空前的行——不在盘上，load 不会
      // 重放它们；内存折叠已被清空 → 按快照身份重折恰一次）。
      for (const line of pending) {
        const shaped = this.#shapeOf(line)
        if (!shaped) continue
        if (shaped.kind === 'scope') this.#foldScope(shaped)
        else this.#fold(shaped)
      }
      if (pending.length > 0) this.#rebuildRecent()
      return report
    } catch {
      // 重载任一环失败：保持已重放状态（或原状态），绝不抛（P7）。
      return this.statsSelfReport()
    } finally {
      release()
    }
  ...
  #shapeOf(parsed) {
    ...
    if (parsed.kind === 'scope') {
      ...
      // D7（返工批 3）：preset 允许空串（无预设会话的 scope 行——账号视图
      // 计数权威覆盖全部路由形态）；历史行全部有值，读侧零影响。
  ...
  snapshot() {
    ...
    for (const [accountKey, buckets] of this.accountSeries) {
      ...
      if (kept.length > 0) {
        ...
        accountSeries.push({ provider: row?.provider ?? accountDisplayLabel(accountKey), accountKind: row?.accountKind ?? this.#normalize({ provider: accountKey }).accountKind, buckets: kept })
      }
    }
    const days = {}
    ...
    const recentByAccount = [...this.recentByAccount.entries()].map(([accountKey, entries]) => {
      const attribution = this.#normalize({ provider: accountKey })
      return {
