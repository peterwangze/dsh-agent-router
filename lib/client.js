 ...
 *
 * 单页签「Agent 路由」（settings.section list slot），按分类组织为可展开卡片：
 * - 总开关（唯一不折叠的配置）；
 * - 多模态账号管理：ChatGPT/Claude/Grok/Gemini 预设 + API Key 登录 +
 *   可选 Base URL + 已配置账号状态列表（OAuth 说明见卡片内文案）；
 * - 统计信息：Agent 级与账号级（服务商）两级明细，行内可展开
 *   分钟级 tokens 分布与模型细分；
 * - 专业 Agent 卡片列表：每卡默认折叠显示摘要（名称/类型/生效模型/
 *   简要用量），点击展开配置与明细统计；列表末尾「+」添加新 agent
...
`
    ...
    function SectionCard(props) {
      const { title, summary, open, onToggle, children } = props
      return el('div', { className: 'dshrouter-card' },
        el('button', { type: 'button', className: 'dshrouter-card-head', onClick: onToggle, 'aria-expanded': open },
          ...
          el('span', { className: 'dshrouter-chevron' }, open ? '▾' : '▸')),
        open ? children : null)
    }
    ...
    function AgentsPage(props) {
      ...
      const [openSections, setOpenSections] = useState({ account: false, stats: true })
      ...
      const load = useCallback(async () => {
        ...
        let configResponse, providersResponse, catalogResponse
        try {
          ;[configResponse, providersResponse, catalogResponse] = await Promise.all([
            routerRemote.config({}),
            api.llm.providers({}),
      ...
      const toggleStatRow = (key) => setOpenStats((current) => ({ ...current, [key]: !current[key] }))
      ...
      for (const entry of providers) {
        ...
        accountRows.push({
          ...
          configured: entry.active === true || accountTotalsById.has(entry.provider),
      ...
      for (const total of stats ? stats.accountTotals ?? [] : []) {
        if (accountRows.some((row) => row.provider === total.provider)) continue
        accountRows.push({
      ...
      accountRows.sort((a, b) => (a.provider < b.provider ? -1 : 1))
      ...
      return el('section', { className: 'dshrouter-section', 'aria-label': t('title') },
        ...
        el(SectionCard, {
          ...
          summary: t('accountSummary')(accountRows.filter((row) => row.configured).length),
          ...
          el('div', { className: 'dshrouter-row' },
            ...
            el('button', {
              ...
              disabled: account.busy || !account.key.trim() || !state.writable,
              onClick: doLogin,
            }, account.busy ? t('saving') : t('accountLogin'))),
          ...
          account.failure ? el('p', { className: 'dshrouter-error' }, account.failure) : null,
          el('hr', { className: 'dshrouter-divider' }),
          el('div', { className: 'dshrouter-head' }, el('span', { className: 'dshrouter-subtitle' }, t('accountList'))),
          accountRows.length === 0 ? el('p', { className: 'dshrouter-hint' }, t('accountMissing')) : null,
          accountRows.map((row) => el('div', { className: 'dshrouter-row', key: row.provider },
            row.configured ? el('span', { className: 'dshrouter-dot ok', title: t('accountDone') }) : el('span', { className: 'dshrouter-dot bad', title: t('accountMissing') }),
            el('span', { className: 'dshrouter-meta' }, `${row.displayName} (${row.provider})`),
            row.active ? el('span', { className: 'dshrouter-tag' }, t('accountActive')) : el('span', { className: 'dshrouter-tag' }, t('accountDormant')),
            el('span', { className: 'dshrouter-spacer' }),
            el('span', { className: 'dshrouter-meta' }, `${t('statsCalls')}: ${row.calls} · ${t('statsErrors')}: ${row.errors} · ${fmtTokens(row.inputTokens)} / ${fmtTokens(row.outputTokens)}`)))),
        // ── 统计信息（Agent 级 + 账号级两级明细）────────────────────────
        el(SectionCard, {
          title: t('statsTitle'),
          summary: t('statsSummary')(sumAll.calls, sumAll.errors),
          ...
          agentIds.length === 0 ? el('p', { className: 'dshrouter-hint' }, t('statsNoCalls')) : el('table', { className: 'dshrouter-table' },
            el('thead', null, el('tr', null,
              el('th', null, t('statsAgent')), el('th', null, t('statsProvider')), el('th', null, t('statsCalls')), el('th', null, t('statsErrors')), el('th', null, t('statsTokens')), el('th', null, t('statsAvg')), el('th', null, t('statsLast')))),
            el('tbody', null, ...agentIds.map((id) => {
              const total = statsTotals.get(id)
              const rowKey = `agent:${id}`
              const open = openStats[rowKey]
              return [
                el('tr', {
                  key: rowKey, className: 'dshrouter-table-row',
                  onClick: () => toggleStatRow(rowKey),
                  'aria-expanded': open,
                },
                  el('td', null, total ? total.name : id),
                  el('td', null, `${total ? total.provider : '—'}/${total ? total.model : '—'}`),
                  el('td', null, total ? total.calls : 0),
                  el('td', null, total && total.errors > 0 ? el('span', { className: 'dshrouter-error' }, total.errors) : total ? total.errors : 0),
                  el('td', null, total ? `${fmtTokens(total.inputTokens)} / ${fmtTokens(total.outputTokens)}` : '0 / 0'),
                  el('td', null, total && total.calls > 0 ? fmtMs(total.totalMs / total.calls) : '—'),
                  el('td', null, timeOf(total && total.lastAt))),
                open ? el('tr', { key: `${rowKey}-detail` }, el('td', { colSpan: 7 },
                  (statsSeries.get(id) ?? []).length > 0 ? el(BarChart, { buckets: statsSeries.get(id) ?? [], title: t('statsSeries') }) : el('p', { className: 'dshrouter-hint' }, t('statsNoCalls')))) : null,
              ]
            }))),
          // 账号级明细
          el('div', { className: 'dshrouter-head', style: { marginTop: 8 } }, el('span', { className: 'dshrouter-subtitle' }, t('statsAccountLevel'))),
          accountRows.length === 0 ? el('p', { className: 'dshrouter-hint' }, t('statsNoCalls')) : el('table', { className: 'dshrouter-table' },
            el('thead', null, el('tr', null,
              el('th', null, t('accountProvider')), el('th', null, t('statsCalls')), el('th', null, t('statsErrors')), el('th', null, t('statsTokens')), el('th', null, t('statsAvg')), el('th', null, t('statsLast')))),
            el('tbody', null, ...accountRows.map((row) => {
              const rowKey = `acct:${row.provider}`
              const open = openStats[rowKey]
              const accountTotal = accountTotalsById.get(row.provider)
              let detail = null
              if (open) {
                const models = accountTotal && accountTotal.models ? accountTotal.models : []
                const modelRows = models.map((modelTotal) => el('tr', { key: modelTotal.model },
                ...
                const seriesBuckets = accountSeriesById.get(row.provider) ?? []
                detail = el('tr', { key: `${rowKey}-detail` }, el('td', { colSpan: 6 },
                  ...
                  seriesBuckets.length > 0 ? el(BarChart, { buckets: seriesBuckets, title: t('statsSeries') }) : null))
              }
              return [
                el('tr', {
                  key: rowKey, className: 'dshrouter-table-row',
                  ...
                  el('td', null, `${row.displayName} (${row.provider})`),
                  el('td', null, row.calls),
                  el('td', null, row.errors > 0 ? el('span', { className: 'dshrouter-error' }, row.errors) : row.errors),
                  el('td', null, `${fmtTokens(row.inputTokens)} / ${fmtTokens(row.outputTokens)}`),
                  el('td', null, row.calls > 0 ? fmtMs(row.totalMs / row.calls) : '—'),
                  el('td', null, timeOf(row.lastAt))),
                detail,
              ]
            }))),
