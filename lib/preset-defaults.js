 ...
 *   liveDefaultSelection / sessionNeverProduced → llm-selection 域唯一
 *   实现；agentsRegistryOf → ctx-services 域 B1 落地）——EVO-022 B4 F-1
 *   归属勘正：agentPresetsServiceOf 自 llm-selection 迁 ctx-services 域
 *   （§4.3 域 3 为准，与 agentPresetsOf 归并单点；本模块经桶 import
 *   零改动）
 ...
 *
 * - FIX-052（EV-217 / 用户报障 2026-09-18——预设作用域统计归属错乱，归属键重构）：
 *   归属键 MUST 绑定**工作流身份不变量** = 会话自身的预设身份（由该会话自己的
 *   agent-preset/selected 事件链维护），禁止依赖 live 挂载 / 部署默认 / 全局
 *   选择等可变量。旧链（live 罗盘 `composedPreset(agent.ctx)` 优先 → header 冻结
 *   快照兜底）两源皆可变量：宿主 `standingMountFor`（dsh-agent-presets L787-793）
 *   按 `livePresetMounts()` **运行时挂载状态**匹配（L1550-1552）→ 预设重装/部署
 *   默认变更后改指或落空（novel 会话被记入 governance 桶、governance 会话被记入
 *   standard 桶）；子代理 header.agentPreset 亦由宿主 `childSessionMeta`
 *   （dsh-subagent L502-504）从父 **live 罗盘**现写——同源漂移（novel 子代理行
 *   全入 governance|subagent）。新链（单点 = 下面的会话身份注册表，由本模块
 *   installPresetDefaults 的两个**既有**预设事件订阅维护，不新增任何宿主订阅面）：
 *   ① 会话身份映射命中 → ② header.agentPreset 兜底 → ③ live 罗盘仅最后兜底 +
 *   诊断披露（绝不再优先）。解析结果与罗盘不一致 → notePresetDiag
 *   'preset-attribution-divergence'；罗盘兜底（无身份可用）→
 *   'preset-attribution-live-fallback'（P8 降级可观测；两者均去重有界）。
 *   子代理身份优先继承**父会话身份**（宿主语义 = 子 join 父预设，而子头
 *   agentPreset 是父 live 罗盘现值——不可采信）。映射有界：LRU 上限
 *   SESSION_PRESET_LIMIT（超限淘汰最旧，淘汰后回落创建头兜底，归属不猜）。
 *
...
import { agentsRegistryOf, agentPresetsServiceOf, inheritedRouteOf, liveDefaultSelection, sessionNeverProduced, sessionSelectFaceOf, subscribeEvents } from './host-abi/index.js'
...
/**
 * FIX-052：会话预设身份映射容量上限（有界内存——LRU，超限淘汰最旧条目；
 * 淘汰后解析侧回落创建头快照，归属不猜、不漂移到 live 可变量）。
 */
export const SESSION_PRESET_LIMIT = 1024

/**
 * FIX-052：归属漂移诊断去重上限（同 (kind, 会话, 归属, 罗盘) 签名只记一条——
 * 每请求都记会把 64 条诊断环形冲掉，观测面反而失效；有界去重是 P8 与可读性的
 * 交点，退化为「每会话每漂移对恰一次」）。
 */
const ATTRIBUTION_DIAG_LIMIT = 256

/**
 * FIX-052：会话预设身份映射（**归属键单点**，模块级有界 LRU）——归属键 = 工作流
 * 身份不变量 = 会话自身预设身份，触发来源恰两个（同一动作单一路径，P5）：
 * ① `agent/created` 创建播种（主会话取创建头快照；子代理继承父会话身份）；
 * ② `agent-preset/selected(sessionId, agentPreset)` 会话自身事件链更新
 * （宿主把该会话追加的 agent-preset/selected 事实重发为双参 cordis 事件，
 * dsh-agent-presets L1325-1328）。两个触发点都在 installPresetDefaults 的既有
 * 订阅内维护——不新增宿主订阅面，遥测站点只读（消费单点 sessionPresetIdentityOf）。
 */
const sessionPresetIdentity = new Map()

/**
 * 写入/刷新一条会话预设身份（FIX-052 归属键权威入口）。语义：
 * - 有界 LRU：命中/重写刷新序，超限淘汰最旧；
 * - 空值忽略（不留空条目、不覆盖已建立身份——宿主载荷按 schema 必为预设 id
 *   字符串，空值属形态防御；「无法解析」由解析链兜底表达，不靠空条目）。
 */
export function noteSessionPresetIdentity(sessionId, preset) {
  try {
    const id = typeof sessionId === 'string' ? sessionId : ''
    const value = typeof preset === 'string' ? preset.slice(0, 64) : ''
    if (!id || !value) return
    sessionPresetIdentity.delete(id)
    sessionPresetIdentity.set(id, value)
    while (sessionPresetIdentity.size > SESSION_PRESET_LIMIT) {
      const oldest = sessionPresetIdentity.keys().next().value
      if (oldest === undefined) break
      sessionPresetIdentity.delete(oldest)
    }
  } catch { /* 归属记账失败绝不影响主链（解析侧回落创建头兜底） */ }
}

/** 读取一条会话预设身份（命中即刷新 LRU 序）；未命中 → ''（调用方走兜底链）。 */
export function sessionPresetIdentityOf(sessionId) {
  const id = typeof sessionId === 'string' ? sessionId : ''
  if (!id) return ''
  try {
    const hit = sessionPresetIdentity.get(id)
    if (hit === undefined) return ''
    sessionPresetIdentity.delete(id)
    sessionPresetIdentity.set(id, hit)
    return hit
  } catch { return '' }
}

/**
 * 创建期身份播种取值（FIX-052，纯函数面）：子代理会话优先继承**父会话身份**
 * ——宿主语义 = 子代理 join 父预设（dsh-subagent composeChild），而子头
 * `agentPreset` 由 `childSessionMeta`（dsh-subagent L502-504）从父 **live 罗盘**
 * 现写（`parent.ctx.get("agentPresets")?.composedPreset(parent.ctx)`）——同源
 * 可变量，采信即复发报障①（novel 子代理行入 governance 桶）。主会话取创建头
 * 快照 `header.agentPreset`（宿主 resolveSessionPreset 语义：加入预设时写入）。
 * 父身份缺失（本插件热装、父早于插件创建）→ 回落子自身头。返回 '' = 无身份
 * 可播（不落空条目）。
 */
function presetIdentityAtCreation(header) {
  const parentSession = typeof header?.parentSession === 'string' ? header.parentSession : ''
  if (parentSession) {
    const inherited = sessionPresetIdentityOf(parentSession)
    if (inherited) return inherited
  }
  const own = header?.agentPreset
  return typeof own === 'string' ? own : ''
}

/**
 * 宿主预设罗盘的 live 解析（**全模块唯一读取点**——FIX-052 起遥测归属站点不再
 * 内联第二份）：`composedPreset(agent.ctx)` = 宿主 `standingMountFor` 对
 * `livePresetMounts()` 的运行时挂载匹配（dsh-agent-presets L787-793 / L1550-1552）
 * ——预设重装 / 部署默认变更后可改指或落空，**不具身份不变量性质**，故降级为
 * 统计归属的最后兜底 + 诊断披露面（消费方 = 播种解析链与遥测归属链）。返回 '' =
 * 不可解析（服务缺失/抛错/未挂载——消费方自决兜底）。
 */
function liveCompassPresetOf(agent, ctx) {
  try {
  ...
  } catch { /* 罗盘服务缺失/抛错 → 不可解析 */ }
  return ''
}

/**
 * 主 agent 的预设解析（**播种路径**，EVO-013 语义不动：罗盘 live → 创建头快照
 * 兜底——决定播种哪个预设的默认模型，非统计归属，FIX-052 不改其链）。返回
 * '' = 无法解析。
 */
function livePresetOf(agent, ctx, header) {
  const live = liveCompassPresetOf(agent, ctx)
  if (live) return live
...
export function installPresetDefaults(ctx, service) {
   ...
   * **README「预设 Agent 默认模型」节「留空 = 继承主 Agent 模型」句**落地为
   * **跟随主 agent 当前（会话实际）模型**——宿主继承基线 parentAgentOptionsForDelegation
  ...
  const onAgentCreated = async (payload) => {
    try {
      ...
      // FIX-052：归属键身份播种——先于一切早退（总开关/配置/已产出判定）：
      // 归属是会话事实，不随播种是否执行、预设是否配置而变（遥测侧零漂移前提）。
      noteSessionPresetIdentity(sessionKeyOf(agent), presetIdentityAtCreation(header))
  ...
  const onPresetSelected = async (sessionId, agentPreset) => {
    try {
      // FIX-052：会话身份更新（该会话自己的预设事件链 = 归属键唯一权威更新源；
      // 先于总开关/agent 查找早退——身份是事实，且遥测归属与播种执行解耦）。
      noteSessionPresetIdentity(sessionId, agentPreset)
      if (!service.isEnabled()) return
  ...
  // EVO-023 B5（§4.3 域 4 ① / W-3）：agent-preset/selected 是域管事件（跨面
  // 环境事件——服务端播种 + 客户端目录刷新双消费者），订阅收敛 events 域
  // （共享 listener + 卸载聚合；转发白名单清单内）。agent/created 为 Scoped
  // 生命周期钩子（announce 同步抛错具 veto 语义）按 W-3 保留直订不经域。
  const disposeSelected = subscribeEvents(ctx, [{ event: 'agent-preset/selected', consumer: 'preset-seed', handler: onPresetSelected }])
  return () => {
    disposeCreated()
    disposeSelected()
  }
 ...
 * 返回 next() 结果——纯旁路观测，零介入）；预设归属 = 会话预设身份不变量
 * （FIX-052 解析链，见下），origin = header.origin ===
 ...
 * FIX-052（归属键重构，用户裁决 2026-09-18）：预设归属 = **会话预设身份不变量**
 * （`noteSessionPresetIdentity`/`sessionPresetIdentityOf` 单点，由会话自己的
 * agent-preset/selected 事件链维护）→ header.agentPreset 快照兜底 → live 罗盘
 * （`liveCompassPresetOf`）**仅最后兜底 + 诊断披露**（旧链「罗盘优先」依赖宿主
 * 运行时挂载匹配/部署默认，预设重装即漂移——报障①②根因）。解析与罗盘不一致 →
 * `preset-attribution-divergence`；罗盘兜底 → `preset-attribution-live-fallback`
 * （P8 降级可观测，去重有界）。
...
export function installRequestTelemetry(ctx, service) {
  // FIX-052：归属诊断去重（有界——见 ATTRIBUTION_DIAG_LIMIT 注释；安装闭包持有，
  // 卸载即弃，测试间互不污染）。
  const attributionLogged = new Set()
  const noteAttribution = (record) => {
    try {
      const key = `${record.kind}\u0000${record.session}\u0000${record.preset}\u0000${record.live}`
      if (attributionLogged.has(key)) return
      if (attributionLogged.size >= ATTRIBUTION_DIAG_LIMIT) attributionLogged.clear()
      attributionLogged.add(key)
      notePresetDiag({ kind: record.kind, applied: false, preset: record.preset, session: record.session, detail: record.detail })
    } catch { /* 诊断失败绝不影响归属与请求链 */ }
  }
  const handler = async (payload, next) => {
    ...
    try {
      ...
      // FIX-052 归属解析链（用户裁决：归属键绑定工作流身份不变量）：
      //   ① 会话身份映射（该会话自己的预设事件链）——权威源；
      //   ② header.agentPreset 创建期快照——兜底（会话早于本插件/身份被 LRU 淘汰）；
      //   ③ live 罗盘——仅最后兜底 + 诊断披露，**绝不再优先**（旧链把宿主
      //      standingMountFor 的运行时挂载匹配/部署默认当权威源 → 预设重装即
      //      改指：novel 会话记入 governance 桶、governance 会话记入 standard 桶）。
      const sessionId = sessionKeyOf(agent)
      const identity = sessionPresetIdentityOf(sessionId)
      const fromHeader = typeof header.agentPreset === 'string' ? header.agentPreset : ''
      let preset = identity || fromHeader
      let source = identity ? 'session-identity' : (fromHeader ? 'creation-header' : '')
      const live = liveCompassPresetOf(agent, ctx)
      if (!preset && live) {
        preset = live
        source = 'live-compass-fallback'
      }
      if (live && live !== preset) {
        // 解析结果与 live 罗盘不一致 = 可变量（挂载/部署默认）与身份分叉——
        // 归属取身份侧，分叉本身 MUST 可观测（P8：不无观测吞错/静默降级）。
        noteAttribution({
          kind: 'preset-attribution-divergence',
          session: sessionId,
          preset,
          live,
          detail: `attributed "${preset}" (source=${source || 'none'}) but live compass reports "${live}" (live mount is not authoritative since FIX-052)`,
        })
      } else if (!identity && !fromHeader && preset) {
        // 降级路径可观测：无会话身份可用（映射未命中 + 创建头为空）→ 罗盘兜底。
        noteAttribution({
          kind: 'preset-attribution-live-fallback',
          session: sessionId,
          preset,
          live,
          detail: `no session preset identity (map miss + empty creation header); live compass "${live}" used as last resort`,
        })
      }
      // D7（返工批 3）：解除 preset 门控——无预设会话（如直用宿主官方
      // deepseek-official 默认模型）同样记录 scope 行（解析链三段皆不可用时
      // preset=''，不猜）；账号级计数权威覆盖全部路由形态（与口径披露文案一致）。
      // 空串行在 stats.js #foldScope 分流：只进账号视图，不进预设卡。无 header
      // agent 零记录（上方早退）。
