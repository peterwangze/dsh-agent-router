 ...
 * 超时为协作式：signal 传入 fetch init（生产 undici 尊重 signal；注入实现
 * 需监听 init.signal 才能被超时中止）。消息只含状态码与原因，不含任何
 * token 值（P7；超时消息为固定格式，不内联底层中止错误文本）。
...
async function refreshCredential(cred, fetchImpl, timeoutMs) {
  ...
  // fetch 落定后即清理计时器，不留唤醒源。
  const controller = typeof AbortController !== 'undefined' ? new AbortController() : undefined
  const timer = controller ? setTimeout(() => controller.abort(), ms) : undefined
  let response
  try {
    ...
    if (controller && controller.signal.aborted) {
      const timeoutError = credentialError(CREDENTIAL_ERROR_CODES.REFRESH_FAILED, `刷新请求超时（timeout after ${ms}ms，未在时限内得到响应）`)
