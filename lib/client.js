...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 确认截图 sha256:9fa0908e 实证第三幽灵卡：已激活 · 7 个模型 · 调用 0）。
    // 两镜像值变更必须跟随权威单点。
    const HOST_ROUTE_ID = 'openai-codex'
    // FIX-015 + FIX-019：插件自注册 provider 判据——两语义面分开（host 路由
    // 不是 oauth 账号，各自独立演进）：
    //  · oauth 面（isPluginRouteProvider）：旧统计聚合键 `oauth:` 前缀（服务端
    //    record provider 形态，如 oauth:chatgpt，FIX-015）+ EVO-010 起无冒号
    //    插件路由 id（OAUTH_PROVIDER，前缀判据失效，用户实证「又出现了」红字
    //    假编辑器）；
    //  · 宿主路由面（isHostManagedRoute）：插件自动维护进 llm-pi-ai settings
    //    的 HOST_ROUTE_PROVIDER 条目（真 provider 形态，出现在 llm-pi-ai
    //    providers 目录，无法靠「不在 providers 里」过滤，须显式排除）。
    // isPluginSelfRegisteredProvider 组合谓词 = 渲染过滤**单点**：addedAccounts
    // 与 statsAccountRows 的全部来源（llm-pi-ai providers 目录 + 统计
    // accountTotals）只经它判定；新增路由形态仅改此地，禁止在过滤点重复
    // 展开条件（P5 单点化）。订阅登录真卡的「路由已维护」状态行（EVO-010
    // openaiCodexRoute）已承载宿主路由可见性——账号管理区零重复。
    ...
    const wCatalog = wv.object({
      ...
      // FIX-018：主模型 image 能力判定（服务端单点下发，客户端贴图接管门控
      // 消费；可选——旧服务端缺省 = 客户端按「不判定」保守回落既有接管）。
      mainModelImage: wv.object({
        acceptsImage: wv.boolean(), source: wv.string(), provider: wv.string(), model: wv.string(),
      }, true),
    ...
    // FIX-046 返工（根因确证，EV-217）：观测面两条结果的客户端 codec——
    // 此前 $mount 列表缺这两条 descriptor ⇒ `remote.router.presetDiagnostics` /
    // `remote.router.hostFaceDiagnostics` **不存在** ⇒ 设置页宿主面健康面板的
    // 版本/诊断面被静默跳过（真机 `host-face-shape: hostFaceDiagnostics`）。
    // 形状权威 = lib/schemas.js 的 `presetDiagnosticsResult`（FIX-030-C）与
    // `hostFaceDiagnosticsResult`（ARCH-004 B1；face 条目 `faceHealthCodec`、
    // diag 条目 `hostDiagEntryCodec`）——本处为浏览器镜像（本包经
    // __ModuleLoader__ 仅 require('react')，无 ESM 模块面，无法 import）；形状
    // 漂移由 tests/rpc-shadow-guard.mjs 的双侧 descriptor id 集合 + codec
    // schema 深度等价守卫看护。**FIX-047（N-5）限定语（禁超宣示）**：该深等判据
    // 只覆盖两侧**手写 spec 的文本等价**（本包 `wv` 家族 vs lib/schemas.js `v`
    // 家族），**不覆盖**两侧校验器实现（本包 `wireCheck` vs lib/schemas.js
    // `check`）的行为等价，也不覆盖宿主 `$mount` 的 codec 消费面（本仓不可核验）。
    const wPresetDiagEntry = wv.object({
      at: wv.number(), kind: wv.string(), applied: wv.boolean(),
      preset: wv.string(), session: wv.string(),
      target: wv.object({ provider: wv.string(), model: wv.string() }, true),
      skip: wv.string(true), detail: wv.string(true),
    })
    const wPresetDiagnosticsResult = wv.object({ entries: wv.array(wPresetDiagEntry) })
    const wFaceHealth = wv.object({ name: wv.string(), state: wv.string(), detail: wv.string(true) })
    const wHostDiagEntry = wv.object({
      at: wv.number(), kind: wv.string(),
      face: wv.string(true), consumer: wv.string(true), code: wv.string(true), detail: wv.string(true),
    })
    const wHostFaceDiagnosticsResult = wv.object({
      // 版本三键为**必填**（lib/schemas.js 同形）：结果缺 hostVersions 时
      // 客户端按 `host-face-result-shape` 显式诊断，不再静默回落为 '?'。
      hostVersions: wv.object({ llm: wv.string(), tools: wv.string(), typertProtocol: wv.string() }),
      faces: wv.array(wFaceHealth),
      diag: wv.array(wHostDiagEntry),
    })
    ...
    const ROUTER_REMOTE = {
      ...
      descriptors: [
        ...
        // FIX-046 返工（EV-217 根因）：descriptor 集合曾为**两份事实源**——服务端
        // lib/rpc.js 的 ROUTER_DESCRIPTORS 19 条、本 $mount 列表 17 条，缺此两条
        // ⇒ 浏览器侧 remote.router 少注册两个方法（宿主的 $mount 按本列表注册，
        // 与宿主 ROUTER_DESCRIPTORS 无关）。同一动作必须只有一处事实源，故本列表
        // 与服务端逐条对齐（id/method/parameters/result 形状同构：emptyRequest
        // 请求 + presetDiagnosticsResult / hostFaceDiagnosticsResult 结果）。
        // 防再分叉：tests/rpc-shadow-guard.mjs 的双侧 id 集合 + codec 等价守卫
        // （任一单侧增删即判红）。
        { id: 'dsh-agent-router#router/presetDiagnostics', service: 'router', namespace: 'router', method: 'presetDiagnostics', invocation: { kind: 'direct' }, parameters: [parameter('request', wEmpty)], result: resultOf('PresetDiagnosticsResult', wPresetDiagnosticsResult) },
        { id: 'dsh-agent-router#router/hostFaceDiagnostics', service: 'router', namespace: 'router', method: 'hostFaceDiagnostics', invocation: { kind: 'direct' }, parameters: [parameter('request', wEmpty)], result: resultOf('HostFaceDiagnosticsResult', wHostFaceDiagnosticsResult) },
...
/* EVO-012 批二 C：会话产物集合视图（输入区按钮 → 网格 → 单图放大）。 */
.dshrouter-gallery{display:inline-flex;align-items:center}
.dshrouter-gallery-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(120px,1fr));gap:8px}
.dshrouter-gallery-cell{display:block;padding:0;border:none;background:none;cursor:zoom-in;border-radius:8px}
.dshrouter-gallery-image{display:block;width:100%;height:120px;object-fit:cover;border:1px solid var(--dsw-alias-border-l3);border-radius:8px;background:var(--dsw-alias-bg-base,transparent)}
.dshrouter-gallery-image:hover{opacity:.9}
...
`
    ...
    const zh = {
      nav: 'Agent 路由',
      title: '多模型路由',
      intro: '为不同能力配置专业 agent（图片识别、图片生成、翻译、语音识别等），每个 agent 可选独立服务商与模型；未配置时自动复用主 agent 模型。四张分类卡片（专业 Agent / 预设 Agent / 多模态账号 / 统计信息）均默认折叠——点击分类卡片标题展开或收起。',
      ...
      // F-1（EVO-007 R0 返工）：池行/孤儿列表的账号删除入口——非 preset
      // OAuth 账号删除路径恢复（unset 条目 + 凭据清理 + 清全部池引用）。
      poolDeleteAccount: '删除账号',
      poolOneClick: '一键授权登录',
      ...
      // F-4（EVO-010 R0）：transport 是维护开关非请求路由决策——实际走向由
      // 模型选择器所选分组决定（host=openai-codex 官方组 / plugin=ChatGPT
      // 订阅插件组）；切换不迁移既有会话绑定，如实披露。
      presetTransportHint: '实际调用走向以模型选择器所选分组为准（宿主官方 = openai-codex 组；插件内置 = ChatGPT 订阅组）；切换通路后既有会话请在模型选择器手动选组。',
      oauthTokenBack: 'access token 已自动保存。',
      oauthNeedConfig: '请先完善账号配置（Base URL 等）再登录',
      // F-1（EVO-007 R0 返工）：非 preset OAuth 账号删除路径恢复——
      // 删除确认/成功文案 + 未入池孤儿账号的极简管理列表文案。
      oauthDeleteConfirm: '确认删除该 OAuth 账号？将同时删除本机凭据，并从引用它的账号池中移除。',
      oauthDeleted: 'OAuth 账号已删除（凭据与池引用已清理）。',
      orphanOauthTitle: '未入池的 OAuth 账号',
      orphanOauthSummary: (n) => `${n} 个未入池账号`,
      ...
      toolRunning: '正在处理…',
      toolImageLabel: '图片',
      galleryButton: '本会话图片产物',
      galleryTitle: '本会话图片产物',
      galleryEmpty: '本会话暂无图片产物',
      openFile: '打开文件',
      fileOpening: '正在读取…',
    ...
    const en = {
      ...
      intro: 'Configure specialist agents (vision, image generation, translation, speech recognition, …) with their own provider and model; unset values inherit the main agent model. All four category cards (Specialist / Preset Agents, accounts, statistics) are collapsed by default — click a category header to expand or collapse.',
      ...
      // F-1 (EVO-007 R0 rework): per-account delete entry on pool rows and the
      // orphan list — restores the non-preset OAuth account delete path.
      poolDeleteAccount: 'Delete account',
      ...
      presetTransportHint: 'Actual calls follow the group picked in the model selector (Host official = the openai-codex group; Plugin built-in = the ChatGPT subscription group). After switching, pick the group manually in the model selector for existing sessions.',
      ...
      // account list copy (non-preset OAuth accounts outside every pool).
      oauthDeleteConfirm: 'Delete this OAuth account? Its local credential will also be deleted, and references removed from all account pools.',
      oauthDeleted: 'OAuth account deleted (credential and pool references cleaned up).',
      orphanOauthTitle: 'Orphan OAuth accounts',
      orphanOauthSummary: (n) => `${n} orphan account(s)`,
      ...
      galleryButton: 'Session images',
      galleryTitle: 'Session images',
    ...
    /**
     * EVO-017 R2 统一分组统计体（用户裁决：所有 2/3 级分组复用相同逻辑——
     * 总用量 / 每日用量 / 实时调用 三段）。props：totals 行（calls/errors/
     * inputTokens/outputTokens/totalMs/lastAt）+ days 数组（date/calls/
    ...
    function GroupStatsBody(props) {
      ...
      return el('div', { className: 'dshrouter-stats' },
        el('div', { className: 'dshrouter-row' },
          el('span', { className: 'dshrouter-meta' }, `${t('statsCalls')}: ${call.calls}`),
          el('span', { className: 'dshrouter-meta', style: call.errors > 0 ? { color: 'var(--dsw-alias-state-error-primary)' } : undefined }, `${t('statsErrors')}: ${call.errors}`),
          el('span', { className: 'dshrouter-meta' }, `${t('statsTokens')}: ${fmtTokens(call.inputTokens)} / ${fmtTokens(call.outputTokens)}`),
          call.totalMs > 0 && call.calls > 0 ? el('span', { className: 'dshrouter-meta' }, `${t('statsAvg')}: ${fmtMs(call.totalMs / call.calls)}`) : null,
          totals && totals.lastAt ? el('span', { className: 'dshrouter-meta' }, `${t('statsLast')}: ${timeOf(totals.lastAt)}`) : null),
        el('div', { className: 'dshrouter-head' }, el('span', { className: 'dshrouter-meta' }, t('statsPresetTotals'))),
        Array.isArray(models) && models.length > 0 ? el('table', { className: 'dshrouter-table' },
          el('thead', null, el('tr', null, el('th', null, t('fieldModel')), el('th', null, t('statsCalls')), el('th', null, t('statsErrors')), el('th', null, t('statsTokens')))),
          el('tbody', null, ...models.map((modelTotal) => el('tr', { key: modelTotal.model },
            el('td', null, modelTotal.model),
            el('td', null, modelTotal.calls),
            el('td', null, modelTotal.errors > 0 ? el('span', { className: 'dshrouter-error' }, modelTotal.errors) : modelTotal.errors),
            el('td', null, `${fmtTokens(modelTotal.inputTokens)} / ${fmtTokens(modelTotal.outputTokens)}`))))) : null,
        el('div', { className: 'dshrouter-head' }, el('span', { className: 'dshrouter-meta' }, t('statsPresetDaily'))),
        (!days || days.length === 0) ? el('p', { className: 'dshrouter-hint' }, t('statsNoCalls')) : el('table', { className: 'dshrouter-table' },
          el('thead', null, el('tr', null,
            el('th', null, t('statsDate')), el('th', null, t('statsCalls')), el('th', null, t('statsErrors')), el('th', null, t('statsTokens')), el('th', null, t('statsDayMs')), el('th', null, t('statsCost')), ...(days.some((day) => Array.isArray(day.models) && day.models.length > 0) ? [el('th', null, t('fieldModel'))] : []))),
          el('tbody', null, ...[...days].reverse().map((day) => el('tr', { key: day.date },
            el('td', null, day.date),
            el('td', null, day.calls),
            ...
            ...(days.some((d) => Array.isArray(d.models) && d.models.length > 0)
              ? [el('td', null, Array.isArray(day.models) ? day.models.map((modelCount) => `${modelCount.model}×${modelCount.calls}`).join(', ') : '')]
              : []))))),
        el('div', { className: 'dshrouter-head' }, el('span', { className: 'dshrouter-meta' }, t('statsPresetRecent'))),
        (!recent || recent.length === 0) ? el('p', { className: 'dshrouter-hint' }, t('statsNoCalls')) : el('table', { className: 'dshrouter-table' },
          el('thead', null, el('tr', null,
    ...
    /**
     * EVO-017 R2 统一分组卡（三级卡壳：折叠头 + GroupStatsBody）——专业
    ...
    // FIX-015（c/d）：preset 默认模型三件套单一来源（P5——禁止复制字面量）：
    // placeholder、新建账号预填、存量已登录空 models 账号的编辑态实预填共用。
    const PRESET_MODEL_DEFAULTS = ['gpt-5.6-sol', 'gpt-5.6-terra', 'gpt-5.6-luna']
    const PRESET_MODELS_PLACEHOLDER = PRESET_MODEL_DEFAULTS.join(', ')

    /** FIX-015 防御面：发现模型合并（discoverOauth 用）——**并集语义**：已存
     *  列表 ∪ 发现结果，保序去重。任何发现结果（含空/失败）都不可能缩小已有
     *  非空列表（写 oauthAccounts.<id>.models 的防缩小证明之一；判别见
     *  client-render FIX-015 块）。经 exports 暴露供判别测试直驱（FIX-012
     *  测试钩子先例——浏览器包内不可用 export 语句）。 */
    function mergePresetModels(existing, discovered) {
    ...
    function PresetAccountCard(props) {
      ...
      return el('div', { className: 'dshrouter-card' },
        ...
        el('p', { className: 'dshrouter-hint' }, t('presetTransportHint')),
        el('div', { className: 'dshrouter-row' },
          el('div', { className: 'dshrouter-field', style: { flex: '1 1 320px' } },
            el('span', { className: 'dshrouter-field-label' }, t('oauthModels')),
            el('input', {
              className: 'dshrouter-input', value: modelsValue, placeholder: PRESET_MODELS_PLACEHOLDER,
    ...
    function PoolCard(props) {
      const { id, pool, health, oauthEntries, tokenStates, expanded, draft, busy, notice, t, writable, onToggle, onField, onSave, onDelete, onAddAccount, onRemoveAccount, onOneClickAccount, onDiscoverAccount, onOauthAddToPool, onDeleteAccount, accountBusy, accountNotice } = props
      ...
      return el('div', { className: 'dshrouter-card' + (pool.enabled === false ? ' disabled' : '') },
        ...
        el('div', { className: 'dshrouter-stats' },
          ...accounts.map((accountId) => {
            ...
            const deleteNotice = accountNotice && accountNotice[accountId]
            return [
              el('div', { className: 'dshrouter-row', key: accountId },
                ...
                // F-1（EVO-007 R0 返工）：整账号删除——unset 条目 + 凭据清理 +
                // 清全部池引用（preset 成员自动分流 W-5）。区别于「移除」（仅移出本池）。
                el('button', {
                  type: 'button', className: 'dshrouter-button danger',
                  disabled: !writable || !!((accountBusy ?? {})[accountId]),
                  title: t('oauthDeleteConfirm'),
                  onClick: () => onDeleteAccount(accountId),
                }, t('poolDeleteAccount'))),
              deleteNotice ? el('p', { className: 'dshrouter-hint', key: `notice-${accountId}` }, deleteNotice) : null,
            ]
          })),
        el('div', { className: 'dshrouter-row' },
          el('select', {
    ...
    function AgentsPage(props) {
      ...
      // 分类卡片统一默认折叠（用户按需展开；原 agents:true 默认展开取消）。
      const [expandedSection, setExpandedSection] = useState({ presets: false, agents: false, accounts: false, stats: false })
      ...
      useEffect(() => {
        if (!ready) return
        let alive = true
        // FIX-047（F-16）：stats 轮询失败的**边沿触发**机录状态——持续失败期只在
        // 首拍记一条（见下方 rejection 分支注释），成功一拍即复位。
        let statsFailureRecorded = false
        const poll = () => {
          const routerRemote = remote()
          if (!routerRemote) return
          routerRemote.stats({}).then((response) => {
            statsFailureRecorded = false
            if (alive && response.ok) setStats(response.value)
          }, (error) => {
            // FIX-047（F-16，R0 P3／P8 同族）：旧实现此处 `() => undefined`
            // 静默吞错——stats 轮询失败在上屏面与诊断环均无痕。改为与
            // host-face-* 同一体系机录（noteHostFaceDiag 镜像环，不新增第二
            // 观测通道）：环条目由 health() 上行（createClientRemotes
            // health().diag），健康面板「最近宿主诊断事件」行即其消费面。
            // **边沿触发**（如实登记口径）：一个失败周期只记首拍一条——逐拍
            // 追加会在 64 条环上限（HOST_FACE_DIAG_LIMIT）内挤掉既有诊断事件
            // （apply 期 inject-face-missing 等），即观测面自伤；成功一拍复位，
            // 故「失败 → 恢复 → 再失败」仍各有一条。持续性由「无新条目 + 无恢复
            // 复位」推断（与 host-face 成功路径只清 notice、不记恢复条目同口径）。
            if (!alive || statsFailureRecorded) return
            statsFailureRecorded = true
            noteHostFaceDiag({
              kind: 'host-face-rpc',
              face: 'remote.router',
              code: 'host-face-stats-rejected',
              detail: String(error?.message ?? error).slice(0, 160),
            })
          })
          // FIX-030-C：预设诊断随轮询刷新（方法缺失 = 旧服务端，静默跳过）。
          if (typeof routerRemote.presetDiagnostics === 'function') {
            routerRemote.presetDiagnostics({}).then((response) => {
              if (alive && response.ok && Array.isArray(response.value?.entries)) setPresetDiag(response.value.entries.slice(0, 8))
            }, () => undefined)
          }
        }
      ...
      useEffect(() => {
        ...
        const reportFailure = (code, detail) => {
          ...
          // FIX-046 R0 F-4／R1 更正：本行**不是**自激重渲染防线。它只在
          // code+detail 均相同时保留原对象（notice 身份稳定）；同 effect 内
          // setFaceHealth(localFaces) 与成功路径 setHostHealth(response.value)
          // 每次重跑写新身份，该守卫不覆盖它们。R0 已证伪的「宿主 props.remote
          // 每次渲染新函数 ⇒ effect 重跑 ⇒ 自激」前提在判别夹具中不可复现
          // （tests/client-render.mjs 单次构造 root element、settle 复用同一
          // props 身份 ⇒ deps 稳定 ⇒ effect 不重跑），故此处不作防线声称。
          // 影响面为零：notice 写入本身幂等，无重复渲染放大。
          setHostFaceNotice((current) => (current && current.code === notice.code && current.detail === notice.detail ? current : notice))
          // FIX-047（F-12，R0 P3）：失败即清陈旧 RPC 快照——effect 重跑（deps
          // 变化）后失败时，上一次成功的 hostHealth 仍被面板合并（faces 行 /
          // hostVersions），与失败行并存 ⇒「旧成功值 + 新失败态」不可区分。
          // 清空后失败态只剩本地 face 快照（面数据源语义不变：本地面仍由
          // health() 一次性探测上行）。
          setHostHealth(null)
        ...
        if (typeof routerRemote.hostFaceDiagnostics !== 'function') {
          // FIX-047（N-6，R1 P3）：detail 中性化——原文案只指宿主侧
          // （「宿主服务端未注册该方法——插件与宿主服务端版本不一致？」），
          // 而真机该短码的实际成因是**浏览器侧 $mount 挂载列表**缺
          // descriptor（EV-217），本批已修复。缺失面两侧皆可能，故不预设
          // 因果（禁未证断言）：只如实列出「谁没提供该方法」。
          reportFailure('host-face-shape: hostFaceDiagnostics', '宿主面版本/诊断 RPC 方法缺失（浏览器侧挂载列表或宿主命名空间未提供该方法）')
          return () => { alive = false }
        }
      ...
      // FIX-015 + FIX-019：插件侧 provider（oauth 面统计键 + 宿主路由面真目录
      // 条目）不得渲染成通用 API Key 编辑器（幽灵卡）——目录来源同样过滤，
      // 判据经 isPluginSelfRegisteredProvider 单点（与统计 accountTotals 来源
      // 同判）。
      ...
      for (const entry of providers) {
        ...
        // FIX-019 边界澄清（EVO-017 R2 修订）：插件自注册 provider 不出现在
      ...
      // openOauthAuthorize/exchangeOauthCode）已随区块移除；以下保留项服务
      // 账号池（池内一键授权/发现模型/按预设建号入池）与 ChatGPT 预设账号。
      // F-1（EVO-007 R0 返工）：deleteOauthAccount 以池行/孤儿列表入口恢复
      // （非 preset 账号删除路径——unset 条目 + 凭据清理 + 清全部池引用）。
      ...
      /** 删除非 preset OAuth 账号（F-1，EVO-007 R0 返工恢复删除路径）：
       *  ① credentials.unset(tokenRef) 凭据清理（幂等，失败容忍）→
       *  ② 从所有引用它的账号池移除（含当前池）→ ③ unset oauthAccounts 条目。
       *  preset 成员账号（chatgpt-codex）分流 W-5 联动删除（deletePresetAccount：
       *  删凭据文件 + 清池引用 + unset 三步，§3.6）。入口：账号池行「删除账号」
       *  按钮与高级扩展区「未入池的 OAuth 账号」列表行（孤儿兜底）。 */
      const deleteOauthAccount = async (id) => {
        const entry = oauthById.get(id)
        if (entry && isPresetAccount(entry)) return deletePresetAccount(id)
        if (!window.confirm(t('oauthDeleteConfirm'))) return
        ...
        try {
          ...
          setOauthNotice((current) => ({ ...current, [id]: outcome.ok ? t('oauthDeleted') : outcome.message }))
          if (outcome.ok) await load()
        } catch (error) {
          setOauthNotice((current) => ({ ...current, [id]: messageOf(error) }))
        } finally {
          setOauthBusy((current) => ({ ...current, [id]: false }))
        }
      }

      // F-1 孤儿兜底（派生在账号池区，依赖 poolEntries）——非 preset 且未加入
      // 任何账号池的账号（历史自定义/粘贴 token/未知 preset 值）：池行删除覆盖
      // 池内账号，此处覆盖池外残留，保证每条 oauthAccounts 条目都有删除入口
      // （P7 凭据可清理）。
       ...
       *  并立即发起一键登录；模型预置 gpt-5.6 系（用户指令 2026-08-30）。
       *  FIX-015 防缩小证明：恒为**新建**（accountId 去重循环保证不覆盖已有
       *  账号）。 */
      const addPresetAccount = async () => {
        ...
        const value = {
          ...
          publicClient: false, authUrl: '', tokenUrl: '', scope: '', models: [...PRESET_MODEL_DEFAULTS], tokenRef: '',
        }
      ...
      /** preset 模型编辑态解析（FIX-015 c/d + 根因未定防御）：显式 draft 优先
       *  （含用户清空的 ''）；已存 models 非空 → join；已登录且空 → 默认三件套
       *  **实预填**（用户保存即写入真实值）；未登录空 → ''（placeholder 保留）。
       *  显示与保存共用（P5 单点）——**防缩小守卫**：无 draft 时保存回退已存值
       *  （未编辑保存 = 幂等原值，绝不空覆盖）。RCA 注记（2026-08-31 修正）：
       *  用户数据丢失根因**未定**——已证 ①磁盘 models:[] ②本缺陷能造成空覆盖
       *  （需保存点击触发）③用户唯一保存点击在丢值之后（不构成因果）④
       *  persistPresetLogin 只写凭据文件不写 settings；真凶未明，防御性全覆盖。 */
      const presetModelsDraftOf = (id) => {
        if (presetModels[id] !== undefined) return presetModels[id]
        const entry = oauthById.get(id)
        const saved = entry && Array.isArray(entry.models) ? entry.models : []
        if (saved.length > 0) return saved.join(', ')
        if (entry && entry.presetLoggedIn === true) return PRESET_MODEL_DEFAULTS.join(', ')
        return ''
      }

      /** preset 模型列表保存（逗号分隔编辑 → 数组落盘）。FIX-015（c 兜底）：
       *  已登录账号解析后仍为空 → 提示而非「已保存」（配合实预填该路径自然
       *  消失，防呆兜底）。 */
      const savePresetModels = async (id) => {
        const raw = presetModelsDraftOf(id)
        ...
        const entry = oauthById.get(id)
        if (outcome.ok && models.length === 0 && entry && entry.presetLoggedIn === true) {
          setOauthNotice((current) => ({ ...current, [id]: t('presetModelsEmptyNotice') }))
          return
        }
      ...
      const discoverOauth = async (id) => {
        ...
        // FIX-015 防缩小证明：并集合并（mergePresetModels）——已存非空列表在
        // 任何发现结果（含空/端点失效）下都不可能被缩小；空发现 = 幂等保存原值。
        const entry = oauthById.get(id)
        const merged = mergePresetModels(entry ? entry.models ?? [] : [], response.value.models)
      ...
      // F-1 孤儿兜底派生：非 preset 且未加入任何账号池的账号（历史自定义/
      // 粘贴 token/未知 preset 值）——池行删除覆盖池内账号，此处覆盖池外
      // 残留，保证每条 oauthAccounts 条目都有删除入口（P7 凭据可清理）。
      const poolMemberIds = new Set(poolEntries.flatMap((pool) => pool.accounts ?? []))
      const orphanOauthIds = oauthEntries
        .filter((entry) => !isPresetAccount(entry) && !poolMemberIds.has(entry.id))
      ...
      const oauthAddToPool = async (poolId, presetId, clientId) => {
        ...
        // FIX-015 防缩小证明：oauthAddToPool 恒为**新建**（accountId 去重循环
        // 保证永不写已存在账号）——models 空模板不会覆盖任何已有非空列表。
        const outcome = await mutate([{ op: 'set', path: ['oauthAccounts', accountId], value }])
      ...
      const accountsBody = [
        ...
        el('div', { className: 'dshrouter-card' },
          ...
          ...presetOauthEntries.map((entry) => el(PresetAccountCard, {
            ...
            modelsValue: presetModelsDraftOf(entry.id),
        ...
        // ── 高级扩展：账号池 + 未入池 OAuth 账号（收进折叠卡片，默认不展开。
        //    EVO-007：OAuth 官方登录区整块移除、ChatGPT 订阅登录上移后，折叠
        //    区仅余账号池与孤儿账号删除兜底——折叠壳保留，避免一级区空置噪音）──
        el(CategoryCard, {
          ...
          children: [
            ...
            ...poolIds.map((id) => {
              ...
              return el(PoolCard, {
                ...
                // F-1（EVO-007 R0 返工）：池行整账号删除入口 + 结果提示。
                onDeleteAccount: (accountId) => deleteOauthAccount(accountId),
                accountBusy: oauthBusy,
                accountNotice: oauthNotice,
              })
            }),
            el(AddPoolCard, {
            ...
            // 未入池的 OAuth 账号（F-1 孤儿兜底：池行删除覆盖池内账号，
            // 此处覆盖池外历史残留——极简列表行，仅删除入口）
            ...(orphanOauthIds.length > 0 ? [
              el('hr', { className: 'dshrouter-divider' }),
              el('div', { className: 'dshrouter-head' },
                el('span', { className: 'dshrouter-subtitle' }, t('orphanOauthTitle')),
                el('span', { className: 'dshrouter-spacer' }),
                el('span', { className: 'dshrouter-meta' }, t('orphanOauthSummary')(orphanOauthIds.length))),
              el('p', { className: 'dshrouter-hint' }, t('orphanOauthIntro')),
              ...orphanOauthIds.map((id) => {
                const entry = oauthById.get(id)
                return el('div', { className: 'dshrouter-card', key: id },
                  el('div', { className: 'dshrouter-row' },
                    el('span', { className: 'dshrouter-dot bad', title: t('oauthNotLoggedIn') }),
                    el('span', { className: 'dshrouter-name' }, entry ? entry.name || id : id),
                    el('span', { className: 'dshrouter-id' }, id),
                    el('span', { className: 'dshrouter-spacer' }),
                    el('button', {
                      type: 'button', className: 'dshrouter-button danger',
                      disabled: !state.writable || !!oauthBusy[id],
                      title: t('oauthDeleteConfirm'),
                      onClick: () => deleteOauthAccount(id),
                    }, t('poolDeleteAccount'))),
                  oauthNotice[id] ? el('p', { className: 'dshrouter-hint' }, oauthNotice[id]) : null)
              }),
            ] : []),
          ],
        }),
      ...
      const statsBody = [
        ...
        // 操作行（清空 + 导出）：工具行不是统计数据——留在卡顶（用户布局
        // 裁决 2026-09-06：统计内容全部归入二级卡，操作行除外）。
        el('div', { className: 'dshrouter-row' },
        ...
        // 全局统计二级卡（用户布局裁决 2026-09-06：散落的全局元素收编；
        // EVO-017 R2 复用统一分组体——总用量/每日/实时三段同构）。
        el(CategoryCard, {
          title: t('statsGlobalLevel'), summary: `${t('statsCalls')} ${sumAll.calls} · ${t('statsErrors')} ${sumAll.errors}`,
          expanded: expandedSection.statsGlobal === true, t, onToggle: () => toggleSection('statsGlobal'),
          children: [
            el('div', { className: 'dshrouter-stats' },
              el(GroupStatsBody, {
                totals: { calls: sumAll.calls, errors: sumAll.errors, inputTokens: sumAll.inTokens, outputTokens: sumAll.outTokens, totalMs: 0 },
      ...
      const presetsBody = [
        ...
        // FIX-030-C：预设生效诊断（最近事件——applied/skip 原因/target；
        // 观测面：配置是否生效一眼可判，排障不再依赖宿主终端日志）。
        presetDiag.length > 0 ? el('details', { className: 'dshrouter-notice', style: { margin: '8px 0' } },
          el('summary', { style: { cursor: 'pointer', fontSize: '12px' } }, t('presetDiagTitle')),
          el('div', { style: { fontSize: '12px', lineHeight: 1.7, marginTop: '6px', wordBreak: 'break-all' } },
            ...presetDiag.map((entry, index) => el('div', { key: `diag-${index}-${entry.at}` },
              `${new Date(entry.at).toLocaleTimeString()} ${entry.applied ? '✓' : '✗'} ${entry.preset || '—'} → ${entry.target ? `${entry.target.provider}/${entry.target.model}` : '—'}${entry.skip ? ` (${entry.skip})` : ''}`)),
            el('div', { style: { opacity: 0.6, marginTop: '4px' } }, t('presetDiagHint')))) : null,
    ...
    function HostHealthCard(props) {
      ...
      // FIX-047（F-11，R0 P3）：取数失败通知本身即「必须上屏的信息」——双源
      // 皆缺（health 面不可用 ⇒ faceHealth 恒 null，且取数失败 ⇒ hostHealth 为
      // null）时旧早退条件会把通知一并吞掉（静默）。故 failure 判定前移并纳入
      // 早退条件：失败态恒渲染，其余情况行为不变。
      const failure = hostFaceNotice && typeof hostFaceNotice === 'object' ? hostFaceNotice : null
      if ((!hostHealth || typeof hostHealth !== 'object') && (!faceHealth || typeof faceHealth !== 'object') && !failure) return null
      const mergedFaces = [
      ...
      // router 自身 RPC 失败不与任一探针面混同（同批：失败态由失败徽章 + 失败行
      // 承载，不借面数篡改语义）。
      // R1 更正（R0 F-3）：R0 指出原注释「『恒 ? 而全绿』的误导性已由该行消除」
      // 不成立——`degraded` 只由面数派生，失败态曾与绿色徽章同排。现改为失败态
      // 徽章位直接渲染失败文案（见下方 summary），原声称已由行为消除。
      // FIX-047（N-2，R1 P3）：失败 ∧ 降级**并存**时折叠态同时给出失败短码与
      // 降级面计数（summary 文本单点合成，见下）——旧形态只渲染失败文案，
      // 降级计数仅存在于展开体 faces 行。`degraded` 口径不变（仍只由 faces 派生，
      // 失败不计入面数）。
      ...
      // FIX-046（P8）→ 返工（R0 F-1/F-2）：失败原因**并列**于「最近宿主诊断事件」
      // 区首行（R0 F-1：原实现以失败行**替换** diag 全集 ⇒ 失败时本地镜像环条目
      // face-degraded / event-subscribe-* / inject-face-missing 被静默隐藏）；且
      // 短码上抬进 summary（R0 F-2：原实现把短码放进默认折叠的 details 体内 ⇒
      // 「直接可见」不成立——V-9 验收要求失败原因直接可见）。
      const diagRows = failure
        ? [{ at: Date.now(), kind: 'host-face-rpc', face: 'remote.router', code: failure.code, detail: failure.detail }, ...diag]
        : diag
      return el('details', { className: 'dshrouter-notice', style: { margin: '0' } },
        el('summary', { style: { cursor: 'pointer', fontSize: '12px' } },
          el('span', { className: failure || degraded > 0 ? 'dshrouter-error' : 'dshrouter-ok' },
            failure
              ? `${t('hostHealthRpcFailed')(failure.code)}${degraded > 0 ? ` · ${t('hostHealthWarn')(degraded)}` : ''}`
        ...
        el('div', { style: { fontSize: '12px', lineHeight: 1.7, marginTop: '6px', wordBreak: 'break-all' } },
          failure ? el('div', { className: 'dshrouter-error' }, failure.detail || '') : null,
          faces.length === 0
    ...
    /**
     * FIX-018：会话当前模型是否被「正向判定」为支持 image（贴图接管门控）。
     *
     * 判定来源（P5 单点——服务端判定，客户端消费）：router/catalog 的
     * mainModelImage（服务端 RouterService.mainModelImageCapability 复用
     * decideImagePrecheck——宿主声明优先 + wrapper sourceAcceptsModality 能力
     * 自证，60s TTL）。客户端零探测：宿主不向客户端暴露模态信息（2026-08-31
     * 取证：buildModelCatalog group.models 仅 {id,name,description,reasoning}，
     * session.models current 仅 {provider,model}，llm.providers 仅 provider 视图）。
     *
     * 门控语义（保守侧不破图）：
     * - 仅当 mainModelImage.acceptsImage === true 且其 provider+model 与会话
     *   当前选择一致时，才认定「当前模型已多模态」→ 贴图不切 twin（图直传
     *   主模型，FIX-005 约定——消除多余 twin 中转面）；
     * - 判定缺失（旧服务端/llm 不可用/无默认选择/探测失败）或漂移（能力快照
     *   与当前选择不一致——目录 30s 轮询窗口内的模型切换）→ 不认定 → 保守
     *   回落既有贴图接管（图经 twin 必可达，宁多切不漏图——FIX-012 语义不受损）。
     *   已知边界（与旧代码一致，非本修复引入）：图片在草稿途中手动把会话切到
     *   另一个模型不触发重评（组件无选择变化订阅面），发送时由宿主准入拦截。
     * - FIX-049 漂移窗口消除：漂移/缺失不再是即刻保守切换——armed 且未 wrapped
     *   时先按需刷新新鲜能力再判（refreshMainModelImage——catalog RPC 实时重算
     *   + 防抖 + 失败/超时回落），命中即按本函数语义 suppress 直传；上面的
     *   保守回落仅剩刷新失败/超时/旧服务端/真不匹配形态。判定语义零改动。
     */
    function currentModelAcceptsImage(catalog, current) {
      const capability = catalog && catalog.mainModelImage
      if (!capability || capability.acceptsImage !== true) return false
      if (!capability.provider || !capability.model) return false
      return !!current && current.provider === capability.provider && current.model === capability.model
    }
    /** 能力快照签名（effect deps）：目录轮询（30s/settings 事件）带来判定变化
     *  时重评接管——否则「判定缺失窗口内误切/抑制」后不会随目录刷新纠偏。 */
    function mainModelImageSignature(catalog) {
      const capability = catalog && catalog.mainModelImage
      if (!capability) return ''
      return `${capability.acceptsImage === true ? '1' : '0'}|${capability.provider || ''}|${capability.model || ''}`
    }

    // ── FIX-049：能力快照按需刷新（漂移窗口消除）────────────────────────
    // mainModelImage 按默认选择计算、随 ~30s 轮询刷新——会话内切模型后立即
    // 贴图必落「快照 vs 当前」不匹配窗口（用户报障 2026-09-18：原生多模态
    // 模型贴图被误切 twin，FIX-012 永不还原使其停留永久化）。armed 时点先
    // 刷新新鲜能力再判定，FIX-018「正向判定命中即直传不切」的意图在时序上
    // 成立。
    /** 防抖记忆（sessionId → `${capabilitySig}|${provider}/${model}`）：同会话
     *  同快照同当前选择不重复刷（目录 30s 轮询兜底保留），失败不重试风暴。
     *  生命周期同 takeoverMemory（模块级，随浏览器包存活）。 */
    const capabilityRefreshKeys = new Map()
    /** 按需刷新竞速上限：catalog RPC 为本机 daemon 调用（常态 <100ms），2s
     *  上限覆盖慢机；超时按「刷新失败」保守回落既有接管（宁多切不漏图）。 */
    const CAPABILITY_REFRESH_TIMEOUT_MS = 2000

    /**
     * FIX-049：按需拉取新鲜能力快照。直调 router/catalog RPC（与装配层
     * refreshCatalog 同一消费面——P5 不新造第二刷新路径），新鲜值写回共享
     * 目录缓存（setRouterCatalog → useRouterCatalog 订阅全量跟随 →
     * capabilitySig 进 effect deps 重评）并原值返回供调用方即时判定。
     *
     * 服务端新鲜性（2026-09-19 只读实读 lib/service.js / lib/wrapper.js）：
     * mainModelImageCapability 每次 catalog RPC 实时重算——host-declared 路径
     * （宿主 resolveModelInfo 声明含 image）无缓存即时返回；self-certified
     * 路径的 60s TTL 按 `provider\0model\0modality` 分键（wrapper.js
     * sourceAcceptsModality），新选中模型必走新鲜探测——故按需 RPC 拿到的
     * 即是会话内新选择的当前判定。
     *
     * 失败语义（保守侧不破图）：router 面不可达 / RPC 拒绝 / 超时 / 防抖命中
     * → resolve null——调用方保守回落既有切换（宁多切不漏图，FIX-018/FIX-012
     * 语义零改动）；response.ok=false 同样按失败回落。
     * @param {unknown} router 槽位注入的 routerRemote（() => ctx.get('remote.router')，
     *   AttachButton 同款注入先例）
     * @param {string} sessionId 会话 id（防抖键之一）
     * @param {string} snapshotSig 触发时的能力快照签名（mainModelImageSignature）
     * @param {{provider: string, model: string}} current 会话当前选择（调用方已验证非空）
     * @returns {Promise<object|null>} 新鲜 catalog（response.value）或 null
     */
    function refreshMainModelImage(router, sessionId, snapshotSig, current) {
      const remote = typeof router === 'function' ? router() : router
      if (!remote || typeof remote.catalog !== 'function') return Promise.resolve(null)
      const key = `${snapshotSig}|${current.provider}/${current.model}`
      if (capabilityRefreshKeys.get(sessionId) === key) return Promise.resolve(null)
      capabilityRefreshKeys.set(sessionId, key)
      let timer = null
      const timedOut = new Promise((resolve) => { timer = setTimeout(() => resolve(null), CAPABILITY_REFRESH_TIMEOUT_MS) })
      return Promise.race([
        Promise.resolve(remote.catalog({})).then((response) => {
          const value = response && response.ok ? (response.value ?? null) : null
          if (value) setRouterCatalog(value)
          return value
        }, () => null),
        timedOut,
    ...
    /**
     * FIX-048：输入快照待发图片计数（双形态兼容——宿主 M2 附件统一编址跟随）。
     *
     * 权威宿主契约（2026-09-18 只读实读，宿主实跑 npx checkout 0.1.5-rc.1，包
     * @deepseek-ai/dsh-client-ui-conversation）：
     *  - `lib/types/client/contract/input.d.ts` 的 `export interface InputState`
     *    字段清单：draft / attachmentIds / draftRev / phase / claim /
     *    occurrences / queue——附件以不透明 `attachmentIds: readonly
     *    DraftAttachmentId[]` 承载（M2 image/file 统一编址），**无 imageIds
     *    字段**（全宿主树 grep 零命中）；宿主消费点 = 同包 lib/client.js 的
     *    InputBar：`resolveDraftAttachments(input.attachmentIds)`。
     *  - kind 解析面 = 该包 `lib/types/client/service.d.ts` 的
     ...
     *    'image' | 'file'`，见 contract/slots.d.ts 的 ComposerImageAttachment /
     *    ComposerFileAttachment）。
     *
     * 计数策略（兼容序：新形态 kind 解析 → 新形态保守回落 → 旧形态 imageIds）：
     *  1. attachmentIds（现行 InputState 形态）+ kind 解析面可达 → 仅统计
     *     kind === 'image'（方案 A：「贴图即切」精确语义，file-only 不武装）；
     *  2. attachmentIds + 解析面不可达/抛错（face 缺失等）→ 保守回落
     *     count = attachmentIds.length（方案 B：宁多切不漏图——file-only 轮也
     *     切 twin，wrapper 对无图轮零改写委托原生模型，代价 = 会话模型标签
     *     变化；与 FIX-012 门控语义同向的保守侧）。降级可观测（P8）：调用方
     *     （ModelTakeover）对 source === 'attachment-ids' 的活跃回落一次性 warn。
     *  3. imageIds（旧宿主 0.1.2-rc.x 前契约回落，旧夹具同源）→ 长度即图片数
     *     （FIX-012 语义不变）。
     * @param {{attachmentIds?: unknown, imageIds?: unknown}} inputSnapshot 宿主 InputState 快照（双形态）
     * @param {((ids: unknown) => unknown) | null} resolveDraftAttachments kind 解析面（无则 null）
     * @returns {{count: number, source: 'attachment-kind'|'attachment-ids'|'imageIds'|'none'}}
     */
    function inputImageCountOf(inputSnapshot, resolveDraftAttachments) {
      if (!inputSnapshot || typeof inputSnapshot !== 'object') return { count: 0, source: 'none' }
      if (Array.isArray(inputSnapshot.attachmentIds)) {
        if (typeof resolveDraftAttachments === 'function') {
          try {
            const attachments = resolveDraftAttachments(inputSnapshot.attachmentIds)
            if (Array.isArray(attachments)) {
              return { count: attachments.filter((attachment) => !!attachment && attachment.kind === 'image').length, source: 'attachment-kind' }
            }
          } catch { /* 解析面异常 → 与 face 缺失同走保守回落（P8 由调用方 warn 承担） */ }
        }
        return { count: inputSnapshot.attachmentIds.length, source: 'attachment-ids' }
      }
      if (Array.isArray(inputSnapshot.imageIds)) return { count: inputSnapshot.imageIds.length, source: 'imageIds' }
     ...
     *    既有还原）。原因（宿主核实 2026-08-30，dsh-api-session-controller
     *    的 session.selectModel 面）：session.selectModel 仅 resolveCallConfig +
     *    写 selectionFor.current，对会话历史/草稿图片零校验——旧注释「会话已
     *    含图时宿主拒绝切回纯文本」不成立（图片准入只在 prompt 时点
     *    dsh-api-session-controller 的 prompt 侧准入与 pi-ai stream 时点
     *    dsh-llm-pi-ai 的 streamWithSnapshot()）。
     ...
     *    store 变化重渲染，草稿附件实时（现行 InputState.attachmentIds，kind 经
     *    conversation.resolveDraftAttachments 解析——FIX-048）：视觉已开启而用户
     *    手动切回纯文本组后贴图，同样自动归位到包装组。
     */
    function ModelTakeover(props) {
      const { sessionId, input, useInput, api, conversation, router } = props
      ...
      // 原行号式锚已漂移到他对象 ⇒ 改代码串式）。输入快照改经 hook 读取；typeof
      ...
      // FIX-048：宿主 0.1.5-rc.1 M2 附件统一编址——InputState 以 attachmentIds
      // 承载附件（input.d.ts 的 export interface InputState，无 imageIds 字段），
      // 旧 imageIds 只读形态在现行宿主恒 0 → 贴图接管静默失效（用户报障
      // 2026-09-18）。计数双形态兼容（inputImageCountOf）：attachmentIds 经 kind
      // 解析面仅计 image（方案 A——「贴图即切」指图片），面不可达保守回落
      // attachmentIds.length（方案 B——宁多切不漏图）；imageIds 旧形态长度即
      // 图片数（语义不变）。解析面 = conversation 服务的 resolveDraftAttachments
      // （service.d.ts 面；conversation 形如装配层注入的 conversationFace =
      // () => ctx.get('conversation')，AttachButton 同款先例；以方法调用保住
      // 服务 this）。capabilitySuppressed（下方 effect）消费同一 imageCount，
      // kind 形态下语义等价（有图 + 当前模型原生多模态 → 直传不切）。
      const conversationSvc = typeof conversation === 'function' ? conversation() : conversation
      const resolveDraftFace = conversationSvc && typeof conversationSvc.resolveDraftAttachments === 'function'
        ? (ids) => conversationSvc.resolveDraftAttachments(ids)
        : null
      const imageInfo = inputImageCountOf(inputSnapshot, resolveDraftFace)
      const imageCount = imageInfo.count
      // FIX-048 P8 可观测：方案 B 保守回落活跃（attachmentIds 非空而 kind 解析
      // 面缺失/异常）→ 一次性 warn——「宁多切不漏图」的降级形态禁静默。
      const fallbackWarnedRef = useRef(false)
      if (imageInfo.source === 'attachment-ids' && imageInfo.count > 0 && !fallbackWarnedRef.current) {
        fallbackWarnedRef.current = true
        console.warn('dsh-agent-router[FIX-048] input.attachmentIds kind 解析面不可达（conversation.resolveDraftAttachments 缺失/异常）——imageCount 保守回落为 attachmentIds.length（file-only 轮亦武装，宁多切不漏图）')
      }
      ...
      // （F18-5：抑制 → 能力反转 → 补接管；缺该 deps 则抑制后永不重评）。
      const capabilitySig = mainModelImageSignature(catalog)
      useEffect(() => {
        const sessions = api && api.sessions
        // P8 可观测（FIX-029 复验取证）：接管链关键分叉结构化上行——F12 可直接
        // 区分「未武装（无图/无多模态 agent）/会话面缺失/models 读取失败/
        // current 缺失/能力门控抑制/切换失败/成功」。修复验收期开启；静默
        // 失败形态（FIX-029 真机首验：贴图不切且无任何提示）由此终结。
        const trace = (phase, detail) => {
          const line = `dsh-agent-router[FIX-029] takeover ${phase} (session ${sessionId}${detail ? `, ${detail}` : ''})`
          if (phase === 'switched' || phase === 'skip-armed-wrapped' || phase === 'capability-refresh') console.info(line)
          else if (phase !== 'skip-idle') console.warn(line)
        }
        if (!sessionId || !sessions || typeof sessions.models !== 'function' || typeof sessions.selectModel !== 'function') {
          trace('face-missing', 'api.sessions incomplete')
          return
        }
        let cancelled = false
        ;(async () => {
          try {
            ...
            if (!result || !result.ok || !result.value) {
              trace('models-failed', result && result.error && result.error.message ? String(result.error.message).slice(0, 120) : 'no result')
              return
            }
            if (!result.value.current) {
              trace('models-empty', 'current is null (directory loading?)')
              return
            }
            const current = result.value.current
            if (typeof current.provider !== 'string' || typeof current.model !== 'string' || current.model === '') {
              trace('models-invalid-current', JSON.stringify(current).slice(0, 120))
              return
            }
            const wrapped = current.provider.endsWith(WRAP_SUFFIX)
            if (!takeoverArmed) {
              if (!wrapped) trace('skip-idle', 'not armed')
              return
            }
            if (wrapped) {
              ...
              // P2-1 deps 含 imageConditional 的交错升级链）。
              const entry = takeoverMemory.get(sessionId)
              if (entry && !imageConditional && entry.armedBy !== 'switch') takeoverMemory.set(sessionId, { ...entry, armedBy: 'switch' })
              trace('skip-armed-wrapped', `already on ${current.provider}`)
              return
            }
            // FIX-018：image 来源接管先过「当前模型已多模态」门控——主模型
            // 原生支持图片时贴图直传主模型，不切 twin（FIX-005 约定；消除
            // 多余 twin 中转面）。仅 image 来源门控：开关驱动接管不经此判定
            // （imageConditional=false 时 armed 来自开关——用户显式要 twin
            // 路由面，FIX-002 语义）。判定缺失/与当前选择漂移 → 不认定 →
            // 保守回落既有接管（见 currentModelAcceptsImage 注释）。未切换
            // = 无记忆写入，解除武装还原逻辑零影响。
            const capabilitySuppressed = imageConditional && imageCount > 0 && currentModelAcceptsImage(catalog, current)
            if (capabilitySuppressed) {
              trace('skip-capability', `current ${current.provider}/${current.model} natively accepts image`)
              return
            }
            // FIX-049：漂移窗口——快照与当前选择不匹配/缺失时先按需拉取新鲜
            // 能力再判定（服务端 catalog RPC 实时重算，见 refreshMainModelImage）；
            // 新鲜判定命中 → suppress 直传主模型（FIX-018 正向判定意图在时序上
            // 成立，消除「切模型→立即贴图」误切 twin）。刷新不可达/失败/超时/
            // 防抖命中/仍不匹配 → 保守回落既有切换（宁多切不漏图不破坏，下方
            // 既有路径零改动）。防抖：同 sessionId+同快照签名+同当前选择不重复
            // 刷；新鲜值经 setRouterCatalog 写回共享缓存 → capabilitySig 变化
            // 驱动 effect 重评（下方 deps 闭环）。
            if (imageConditional && imageCount > 0) {
              const freshCatalog = await refreshMainModelImage(router, sessionId, capabilitySig, current)
              if (cancelled) return
              if (freshCatalog && currentModelAcceptsImage(freshCatalog, current)) {
                trace('skip-capability', `current ${current.provider}/${current.model} natively accepts image (on-demand refreshed)`)
                return
              }
              trace('capability-refresh', freshCatalog ? 'refreshed snapshot still mismatched — conservative switch' : 'refresh unavailable/failed/timed out/debounced — conservative switch')
            }
            ...
            if (taken && taken.result && taken.result.ok) {
              takeoverMemory.set(sessionId, { native: current.provider, armedBy: imageConditional ? 'image' : 'switch' })
              trace('switched', `${current.provider} -> ${current.provider}${WRAP_SUFFIX}`)
            } else {
              const message = taken && taken.result && taken.result.error && taken.result.error.message ? String(taken.result.error.message).slice(0, 120) : 'unknown'
              trace('switch-failed', message)
            }
            return
          } catch (error) {
            // 接管失败容忍：准入是最终防线，用户仍可手动切组（P8：结构化
            // 上行后不再静默）。
            trace('error', error && error.message ? String(error.message).slice(0, 120) : String(error).slice(0, 120))
          }
        })()
        // 未武装但持有接管记忆的还原路径（FIX-002-R7 F1 + FIX-012）：独立于
        // 上面的 armed 主链——仅 !takeoverArmed 时评估（armed 期间的还原会
        // 与接管主链互踩：开启开关的 armed 重跑被还原撤销 = P2-1 交错链破坏）。
        if (!takeoverArmed && takeoverMemory.has(sessionId)) {
          ;(async () => {
            try {
              const entry = takeoverMemory.get(sessionId)
              if (entry.armedBy === 'image') {
                ...
                // 尊重，下次贴图重新接管——需读当前选择判断是否仍停在我们的
                // twin 上，读不到时保守保持。
                const { result } = await sessions.models({ sessionId })
                const current = result && result.ok && result.value ? result.value.current : null
                if (current && (typeof current.provider !== 'string' || !current.provider.endsWith(WRAP_SUFFIX) || current.provider !== `${entry.native}${WRAP_SUFFIX}`)) takeoverMemory.delete(sessionId)
              } else {
                ...
                if (current && typeof current.provider === 'string' && current.provider === `${native}${WRAP_SUFFIX}`) {
                  const restored = await sessions.selectModel({ sessionId, provider: native, model: current.model })
                  if (restored && restored.result && restored.result.ok) {
                    takeoverMemory.delete(sessionId)
                    trace('restored', `${current.provider} -> ${native}`)
                  }
                } else if (current) {
                  takeoverMemory.delete(sessionId)
                }
              }
            } catch { /* 还原失败容忍：记忆保留，下次触发重试 */ }
          })()
        }
        return () => { cancelled = true }
        // R0 P2-1：deps 含 imageConditional——「图片在途时 takeoverDefaultModel
        // 开→关」不改 armed（imageCount 仍在）但改升级/保持判定，缺该变量时
        // effect 不重跑 → armedBy 滞留 'image' → 移除图片后不还原（组件头承诺
        // 的「开启后走既有还原」在交错下失效）。补入后交错升级链闭合。
        // FIX-018：deps 含 capabilitySig——能力快照变化（目录轮询纠偏/适配器
        // 注销反转判定）时重评门控（F18-5 看护）。
        // FIX-049：deps 含 router——按需刷新消费的注入面（宿主经槽位 inject
        // 同步提供，恒稳定；api 同款 deps 先例）。
      }, [sessionId, api, takeoverArmed, imageCount, imageConditional, capabilitySig, router])
      return null
    }
    ...
    /** EVO-012 批二 A / FIX-021：ref → 同源直达 url。兜底序 **url > attachmentId
     *  > id**（三者任一 string 非空即用）：
     * - url：标记 url 字段优先；
     * - attachmentId：标记（parseMarkersOf）产物 ref 的规范字段；
     * - id：宿主浏览器侧把 tool-result marker 结构化为 image content block 的
     *   attachment 形状（dsh-client-ui-conversation 实证 `attachment.id`——
     *   浏览器 store 的 block 形状与持久化 jsonl 格式不同，宿主编解码层归一
     *   化为 id 字段）。缺失 id 兜底 = FIX-021 根因：推导空 → RouteImage 走
     *   imageData RPC → 宿主网关 rejected → 用户见「图片加载失败」错误按钮
     *   （EVO-012 复验失败）。
     * 经 encodeURIComponent 防御路径注入，宿主路由白名单兜底。RouteImage 与
     * 本会话集合视图共用（P5 单点）。 */
    function directAssetUrlOf(ref) {
      if (typeof ref?.url === 'string' && ref.url) return ref.url
      const idField = typeof ref?.attachmentId === 'string' && ref.attachmentId
        ? ref.attachmentId
        : (typeof ref?.id === 'string' && ref.id ? ref.id : '')
      return idField ? `/router-assets/${encodeURIComponent(idField)}` : ''
    }

    /** 单张图片缩略图：优先同源 url 直达（EVO-012——directAssetUrlOf 单点；
     *  imageData RPC 降为 onError 最后回退（Electron file:// 面 / 宿主网关回归
     *  边界 / 非内容寻址 id）。thumb 态用小尺寸类（卡片折叠态）。 */
    function RouteImage(props) {
      // AUDIT-001 P0-A：宿主浏览器提供**真 React 18.3.1**（dsh-web-frontend
      // seed 表 `react` → react.production.min.js，createRoot 并发入口），
      // createElement 的 RESERVED_PROPS（key/ref/__self/__source）会把 `ref`
      // 从 props 中剥离——本组件曾以 `ref` 为 prop 名接收图片引用，props.ref
      // 恒 undefined → directAssetUrlOf 恒空 → 直达路径永不生效 → RPC 回退
      // （EVO-012/FIX-021 用户侧「三连败」根因；L3 诊断
      // `site=collapsed ref=undefined` 直接印证）。prop 更名 imageRef 彻底
      // 避开保留字；tests/audit-001-concurrency.mjs S1（宿主剥离语义
      // harness）守卫本回归。
      const { t, router, imageRef, onOpen } = props
      const [state, setState] = useState({ status: 'loading' })
      const directUrl = directAssetUrlOf(imageRef)
      const load = () => {
        const remote = typeof router === 'function' ? router() : router
        ...
        remote.imageData({ ref: imageRef }).then((response) => {
      ...
      if (directUrl) {
        const alt = (typeof imageRef.name === 'string' && imageRef.name) || t('toolImageLabel')
        return el('img', {
    ...
    // ── EVO-012 批二 C：会话产物集合视图（本会话生成图网格入口）────────────
    // 收集：route_agent 工具卡渲染时把标记产物 ref 写入会话产物表（sessionId
    // 由输入区 SessionGallery 在 render 期锚定——DSH web 单会话视图，活跃输入
    // 区即当前会话；render 期赋值先于工具卡 effect 回写，无时序竞态）；展示：
    // 输入区「本会话图片」按钮（N 张）→ 网格 → 单图放大。跨会话不串（键 =
    // sessionId），同一张图（attachmentId 内容寻址）按 id 去重。
    const sessionGallery = {
      map: new Map(),
      activeId: '',
      listeners: new Set(),
    }
    const SESSION_GALLERY_MAX = 200
    function sessionProductsOf(sessionId) {
      return sessionGallery.map.get(sessionId) ?? []
    }
    function addSessionProduct(sessionId, ref) {
      if (!sessionId || !ref || typeof ref.attachmentId !== 'string' || !ref.attachmentId) return
      const list = sessionGallery.map.get(sessionId) ?? []
      if (list.some((item) => item.attachmentId === ref.attachmentId)) return
      list.push({ ...ref })
      if (list.length > SESSION_GALLERY_MAX) list.splice(0, list.length - SESSION_GALLERY_MAX)
      sessionGallery.map.set(sessionId, list)
      for (const listener of sessionGallery.listeners) listener()
    }
    function useSessionProducts(sessionId) {
      const [version, setVersion] = useState(0)
      useEffect(() => {
        const listener = () => setVersion((value) => value + 1)
        sessionGallery.listeners.add(listener)
        return () => { sessionGallery.listeners.delete(listener) }
      }, [])
      void version
    ...
    function SessionGallery(props) {
      ...
      sessionGallery.activeId = typeof sessionId === 'string' && sessionId ? sessionId : ''
      const products = useSessionProducts(sessionId)
      const [open, setOpen] = useState(false)
      const [lightbox, setLightbox] = useState(null)
      if (products.length === 0) return null
      return el('div', { className: 'dshrouter-gallery' },
        el('button', {
          type: 'button',
          className: 'dshrouter-attach',
        ...
        open ? el('div', { className: 'dshrouter-modal', onClick: (event) => { if (event.target === event.currentTarget) setOpen(false) } },
          el('div', { className: 'dshrouter-modal-body' },
            ...
            el('div', { className: 'dshrouter-gallery-grid' },
              ...products.map((ref, index) => el('button', {
                type: 'button',
                className: 'dshrouter-gallery-cell',
                key: `${String(ref.attachmentId ?? '')}:${index}`,
                onClick: () => setLightbox(ref),
              }, el('img', {
                className: 'dshrouter-gallery-image',
                src: directAssetUrlOf(ref),
                alt: (typeof ref.name === 'string' && ref.name) || t('toolImageLabel'),
              })))),
            el('div', { className: 'dshrouter-row' },
              el('span', { className: 'dshrouter-spacer' }),
              el('button', { type: 'button', className: 'dshrouter-button ghost', onClick: () => setOpen(false) }, t('close'))))) : null,
        lightbox ? el('div', { className: 'dshrouter-modal', onClick: (event) => { if (event.target === event.currentTarget) setLightbox(null) } },
          el('div', { className: 'dshrouter-modal-body' },
            ...
            el('img', { src: directAssetUrlOf(lightbox), alt: (typeof lightbox.name === 'string' && lightbox.name) || t('toolImageLabel'), style: { maxWidth: '100%', borderRadius: 8 } }),
            el('div', { className: 'dshrouter-row' },
              el('span', { className: 'dshrouter-spacer' }),
     ...
     * 交互）+ 折叠态产物缩略图（directAssetUrlOf 直达；旧产物无 url 字段自愈）；
     * 点击展开完整结果（错误行 / 图片 gallery / L3 文件行 / 文本）。imageData
     * RPC 仅 onError 最后回退。标记产物同时回写本会话产物集合（批二 C）。 */
    function RouteAgentToolCard(props) {
      ...
      const productRefs = []
      const texts = []
      for (const contentBlock of settled ? block.content ?? [] : []) {
        if (!contentBlock) continue
        if (contentBlock.type === 'image') {
          // FIX-021 根因修复：attachment 单次访问存局部（宿主对象可能为惰性
          // getter——条件判断与 push 的两次属性访问可能返回不同值，push 进
          // undefined → directAssetUrlOf 空 → RPC 回退 → 错误按钮）。值为空
          // 或非对象时跳过（该图的显示由同块 text marker 解析的 ref 承载——
          // marker ref 携带 url，直达渲染）。
          const attachment = contentBlock.attachment
          if (attachment && typeof attachment === 'object') refs.push(attachment)
          continue
        }
        if (contentBlock.type === 'text' && typeof contentBlock.text === 'string') {
          for (const ref of parseMarkersOf(contentBlock.text)) {
            refs.push(ref)
            // 标记派生 ref = 产物（批二 B 起转发图不再进结果标记；旧会话的
            // 图片 content 块是输入回显遗留——不进集合，避免污染产物网格）。
      ...
      // EVO-012 批二 C：产物回写本会话集合（所有 hook 在运行态分支之前——
      // 节点 running→settled 转换时 hook 顺序稳定；addSessionProduct 按
      // attachmentId 去重，重渲染幂等）。
      useEffect(() => {
        for (const ref of productRefs) addSessionProduct(sessionGallery.activeId, ref)
      })
      if (!settled) {
      ...
      const duration = typeof block.callTime === 'number' && typeof block.time === 'number'
        ...
        : ''
      // 折叠态缩略图：directAssetUrlOf 直达（渲染零 RPC、零等待）。
      const directRefs = refs.filter((ref) => directAssetUrlOf(ref))
      return el('div', { className: 'dshrouter-toolcard' },
        el('button', {
          type: 'button',
        ...
        !expanded && directRefs.length > 0 ? el('div', { className: 'dshrouter-toolcard-thumbs' },
          // AUDIT-001 P0-A：图片引用经 imageRef prop 传递——`ref` 是 React
          // 保留 prop（RESERVED_PROPS），宿主真 React 18.3.1 会剥离致
          // props.ref 恒 undefined（组件头注释）。
          ...directRefs.map((imageRef, index) => el(RouteImage, {
            t, router, imageRef, onOpen: open, thumb: true,
            key: `${String(imageRef.attachmentId ?? '')}:${index}`,
          }))) : null,
        expanded ? el('div', { className: 'dshrouter-toolcard-body' },
          ...
          refs.length > 0 ? el('div', { className: 'dshrouter-toolimages dshrouter-toolgallery' },
            ...refs.map((imageRef, index) => el(RouteImage, {
              t, router, imageRef, onOpen: open,
              key: `${String(imageRef.attachmentId ?? '')}:${index}`,
            }))) : null,
     ...
     *  settings-models 的 joinProviderDirectory 同构镜像
    ...
    function noteHostFaceDiag(entry) {
      try {
        ...
        hostFaceDiagEntries.push(record)
     ...
     *  dsh-api-remotes/lib/types/remote-events.js 的 API_REMOTE_FORWARDED_EVENTS（19 项转发事件）；
     ...
     *  fail-safe 吞并记诊断（绝不外泄击穿宿主事件总线）；attach 抛错条目**不
     *  入表** + event-subscribe-failed 诊断（FIX-035 降级语义，与权威侧
     *  events.js 同型——失败消费者 dispose no-op，下次订阅重试 attach）。
    ...
    function subscribeClientEvents(on, subscriptions) {
      ...
      for (const sub of Array.isArray(subscriptions) ? subscriptions : []) {
        if (!sub || typeof sub.event !== 'string' || !sub.event || typeof sub.handler !== 'function') {
          noteHostFaceDiag({ kind: 'event-subscribe-rejected', code: 'invalid-shape' })
          continue
        }
        const consumer = typeof sub.consumer === 'string' && sub.consumer ? sub.consumer : ''
        if (!isForwardedEvent(sub.event)) {
          noteHostFaceDiag({ kind: 'event-subscribe-rejected', face: sub.event.slice(0, 64), consumer, code: 'not-forwarded', detail: 'event absent from API_REMOTE_FORWARDED_EVENTS mirror (dead subscription)' })
          continue
        }
      ...
      for (const [event, consumers] of perEvent) {
        ...
        if (!entry) {
          ...
          const sharedListener = (...args) => {
            ...
            for (const consumer of created.consumers.slice()) {
              try {
                ...
                noteHostFaceDiag({ kind: 'event-handler-error', face: event, consumer: consumer.consumer, detail: String(error?.message ?? error).slice(0, 120) })
              }
            }
          ...
          try {
            ...
            // FIX-035（EVO-023 R0 P2-1 镜像半边）：attach 失败条目**绝不入表**
            // （否则后续订阅复用死条目 + 静默死订阅）；失败消费者本次零 listener，
            // dispose 降级 no-op，下次订阅重试 attach。
            noteHostFaceDiag({ kind: 'event-subscribe-failed', face: event, consumer: consumers.map((item) => item.consumer).filter(Boolean).join(',').slice(0, 64), code: 'attach-threw', detail: String(error?.message ?? error).slice(0, 120) })
          }
          if (!attached) {
            disposeAll.push(() => {})
            continue
    ...
    function createClientRemotes(ctx) {
      const noteDegraded = (face, code, detail) => noteHostFaceDiag({ kind: 'face-degraded', face, code, ...(detail ? { detail } : {}) })
    ...
    // 隔离域可见性的规范保证面）。EV-134 真机反证：FIX-026 状态层/部署层全对
    // 而显示层断——裸 ctx.get('modelDirectories') 在客户端隔离域/注入门控下
    ...
    // + runner 自身 'remote.dynamicCordisRunner'（dsh-cordis-client-runner
    // 的 inject 数组声明）；
    ...
    function apply(ctx) {
      ...
      for (const name of inject) {
        ...
        if (resolved === undefined) noteHostFaceDiag({ kind: 'inject-face-missing', face: name, code: 'missing' })
      }
      ...
      // 的 ctx.remote.$on 订阅）加目录打开时 load()，预设切换三者皆不触发；
      ...
      // API_REMOTE_FORWARDED_EVENTS 首项；$on 即
      ...
      const refreshSessionDirectory = (sessionId, agentPreset) => {
        // FIX-027 遥测（P8）：handler 每次触发的结果结构化 console 上行——前缀
        // dsh-agent-router[FIX-027]，含结果/解析形态/preset/sessionId，用户 F12
        // 可见。服务端上行面待后续任务（现有 remote.router 方法均有语义，
        // 不为遥测新造 RPC 面拆分职责）。
        const telemetry = (result, detail) => {
          const line = `dsh-agent-router[FIX-027] preset switch model directory refresh: ${result} (preset ${agentPreset}, session ${sessionId}${detail ? `, ${detail}` : ''})`
          if (result === 'load') console.info(line)
        ...
        try {
          ...
          // 消费，model-selection 的 scope.modelDirectories 属性面）。
          const viaGet = ctx.get('modelDirectories')
      ...
      // EVO-012 批二 C：会话产物集合视图入口（输入区「本会话图片」按钮；空态
      // 不渲染）。同样注入 sessionId（宿主 input.right 域按会话传递）。
      ctx.slots.inject('conversation.input.right', () => safeRegister({
        name: 'conversation.input.right',
        id: 'router-session-gallery',
        order: 30,
        inject: () => ({}),
      }, (props) => el(SessionGallery, {
        t,
      ...
      // FIX-048：conversation 必须透传（组件 kind 解析面来源——conversationFace
      // = () => ctx.get('conversation')，宿主 ConversationController 以
      // `conversation` 名注册的 resolveDraftAttachments 面；AttachButton 槽位
      // 同款 inject 先例）。漏透传 = 组件恒走方案 B 保守回落（file-only 亦武装）。
      // FIX-049：router 必须透传（组件按需能力刷新面来源——routerRemote =
      // () => ctx.get('remote.router')，AttachButton/RouteAgentToolCard 同款
      // inject 先例）。漏透传 = 组件无刷新面 → 漂移窗口内恒走保守切换（误切
      // twin 复发，FIX-029-B 同型装配事故面；tests/fix-012 D49-0 源码级守卫）。
      ctx.slots.inject('conversation.input.right', () => safeRegister({
        name: 'conversation.input.right',
        id: 'router-model-takeover',
        order: 40,
        inject: () => ({ api, conversation: conversationFace, router: routerRemote }),
    ...
    // FIX-046 返工判别测试钩子：`$mount` 描述符集合直出（宿主运行时只消费
    // apply/inject；tests/rpc-shadow-guard.mjs 经本导出与服务端 lib/rpc.js 的
    // ROUTER_DESCRIPTORS 做**双侧 id 集合 + codec 等价**断言——单侧增删即红，
    // P5「同一动作单一事实源」的机器落地）。
    exports.ROUTER_REMOTE = ROUTER_REMOTE
    ...
    exports.resetSessionGallery = () => {
      sessionGallery.map.clear()
      sessionGallery.activeId = ''
    }
