...
`
    ...
    function AgentsPage(props) {
      ...
      for (const total of stats ? stats.accountTotals ?? [] : []) {
        // FIX-015：`oauth:` 前缀是统计聚合键（服务端 record provider 形态，
        // 如 oauth:chatgpt）而非可配置账号——由「ChatGPT 订阅登录」卡自管，
        // 不得并入配置区渲染成通用 API Key 编辑器（幽灵卡；用户实证指认）。
        if (typeof total.provider === 'string' && total.provider.startsWith('oauth:')) continue
      ...
      for (const total of stats ? stats.accountTotals ?? [] : []) {
        // FIX-015：`oauth:` 前缀是统计聚合键（服务端 record provider 形态，
        // 如 oauth:chatgpt）而非可配置账号——由「ChatGPT 订阅登录」卡自管，
        // 不得以账号行形态出现在统计账号级卡片（与 addedAccounts 同判据）。
        if (typeof total.provider === 'string' && total.provider.startsWith('oauth:')) continue
