...
export function collectReminder(attachmentIds, visionAgents) {
  ...
  return `本轮消息包含图片${idPart}。请调用 route_agent 工具，includeImages 传 true；${visionPart}task 写清用户需求；返回结果原样呈现。`
}
...
function modalityState(service) {
  try {
    return MODALITY_ENTRIES[0].stateOf(service) ?? null
  } catch {
    return null
  }
}
...
export function installPreStep(ctx, service) {
  const handler = async ({ agent, messages, turn, step, signal }, next) => {
    ...
    try {
      ...
      // 纯文本轮零开销（通道①/②都不触发）；requestHasModality 含 tool-result 嵌套。
      if (!claimed.some((message) => requestHasModality([message], 'image'))) return decision
      ...
      if (!onWrapperRoute) {
        const original = () => {
          try {
            ...
            return llm && typeof llm.registration === 'function' ? llm.registration(provider)?.adapter : undefined
          } catch {
            return undefined
          }
        }
        const accepts = await sourceAcceptsModality(original, provider, model, 'image')
        // 能力探测失败 → false → 安全回落改写（§5.2.3：宁可改写不可漏图击穿 C-3）。
        if (!accepts) rewritten = rewriteImageTurnsToMarkers(claimed, state)
      }
      ...
      const out = rewritten
        ...
        : decisionMessages
      return { ...decision, messages: [...out, reminder] }
    } catch (error) {
