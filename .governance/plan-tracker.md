...
| P1 | EVO-012 | 视觉产物画布架构——插件同源 HTTP 图片路由 + 详情侧板画布 + 对话流结果卡 + route_agent 默认折叠（用户提案） | EVO-011✅ + 用户裁决全套（2026-08-31） | 0.4.1 | **批一终态（待用户重启复验）+ 批二待派发**——批一：6d12289（/router-assets/ 前缀路由 + marker url——FIX-020 承载绕开宿主 RPC）+ 9b751a1（卡片默认折叠 + url 直达缩略图 + RPC 回退兼容）；判别 9+9 RED/GREEN；门控 16/16；R0 APPROVED_WITH_NOTES/0（机录——路由安全逐攻击形态核验全绿/暴露评估接受；P2-1 directUrl 白名单二次校验 + P3×4 台账）。批二设计校准（Codex 参考 sha256:d01fd025）：内联优先 + lightbox 点击放大 + 会话产物集合视图（非常驻侧板）。用户复验 = 重启后 draw 生图内联可见 + 折叠交互 |
