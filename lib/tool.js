/**
 * dsh-router 的模型面向行（`- id: tool-router; name: dsh-router/tool`）。
 *
...
export function apply(ctx) {
  ctx.tools.register(defineTool({
    ...
    description: [
      ...
      '`files`：工作区文件路径或 http(s) URL 列表，一次可传多个不同类型。chat 类型按内容分发（图片文件内联注入，需该 agent 声明 image 能力；文本文件内联进 task；其余二进制请改传 agent 类型）；agent 类型把路径注入子代理由其用 fs 工具读取；URL 由宿主下载到工作区 .router-files/ 后按同样规则分发。',
    ...
    async execute(args, exec) {
      ...
      // 附件按需显式派发：attachments 序号 + includeImages 快捷方式；都不给 = 不携带附件。
