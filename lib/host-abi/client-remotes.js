 ...
 * 单点，lib/client.js 内 createClientRemotes 为其行为镜像（lib/client.js 的
 * `OAUTH_ROUTE_PROVIDER` 镜像先例）；同步由 tests/host-abi-health.mjs §7
    ...
    api: {
      ...
      sessions: {
        ...
        models: guard('modelDirectories', async (payload) => {
          ...
          // :124-125 三错误码降级体系（原 FIX-028 failureOf 无短码无环形
          // 事件）。directoryFor 形状漂移 / sessionId 缺失保持 failureOf 原
          // 语义（非缺面形态不误标 missing）。
          if (!directoryService) {
            return degradedEnvelope('modelDirectories', HOST_FACE_ERROR_CODES.missing, 'dsh-agent-router: modelDirectories 服务不可用（宿主旧版本？）')
          }
 ...
 * settings-models 的 `joinProviderDirectory(registered, directory)` 同构镜像
