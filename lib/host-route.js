 ...
 * - settings.mutate：dsh-settings 的 `mutate(ns, ops, expectedRevision)` 契约
 *   `{op:'set'|'unset', path:[string]}` path-op；`write(ns, input, mode,
 *   expectedRevision)` 无跨 ns 属主限制——注册 ns
 *   均可写，写前按该 ns schema（llm-pi-ai Config：providers dict，apiKeyEnv
 *   role credential-ref）+ assertServiceable 校验，非法即 reject。
 * - credentials：宿主 dsh-credentials-local 的 `resolve`/`set`/`unset`；
...
// EVO-022 B4：ctx-services 域批消费者切换（credentialsOf/settingsOf/llmOf）
// + ①（EVO-019 P2-2）HOST_ROUTE_* 常量权威源翻转——经桶单向依赖
// host-abi/version.js 域（§4.2 图：消费者 → 桶 → 域，无循环 import）。
import { HOST_ROUTE_NS, HOST_ROUTE_PROVIDER, HOST_ROUTE_REF, HOST_ROUTE_TICK_MS, credentialsOf, settingsOf, llmOf } from './host-abi/index.js'

// HOST_ROUTE_NS/PROVIDER/REF/TICK_MS 权威定义已迁 lib/host-abi/version.js
// （B4 ①单点）；此处 re-export 维持既有消费面（service.js / tests）import
// 路径零改动。PoC ref 与任务级阈值常量仍为本模块自持（路由维护专属）。
 ...
 * @param trigger - 触发源：'boot'（启动/测试直驱，不 gate）/ 'tick'（周期
 *   重试，不 gate）/ 'user'（router ns 等用户操作事件，不 gate）/ 'llm'
 *   （llm-pi-ai ns 事件——**F-1 回环抑制：失败态下 gate 写入**）。
 * @returns 维护后状态快照（hostRouteStatusOf 同形）。
 */
export async function syncHostRoute(ctx, service, trigger = 'boot') {
  ...
  // F-1 回环抑制：llm-pi-ai ns 事件触发的 pass 在连续失败态（failures>0）下
  // **gate 条目写入**——parity 失败 pass 恰好产生两次值变化 mutate（写条目 +
  // 回滚），宿主 diff-gated commit 每 pass 2 事件 → 事件→写入→事件 自激热
  // 回环（hostRouteQueue 指数增长）。gate 后事件 pass 只读（状态/user-modified
  // 检查照常——宿主真变化仍响应），写入收敛到 tick 节奏（30s 重试）——环
  // 在首次失败后即断裂。tick/user/boot 触发不受 gate（用户操作即时生效、
  // tick 承载重试）。
  // EVO-023 B5 口径（§4.3 域 4 gate 纪律通用化）：本 gate 是**写级**部分
  // 抑制——域 gate（host-abi/events.js armEventGate）是**整 handler** 抑制，
  // 供「整个 handler 即写路径」的消费者使用；本订阅不声明域 gate，因事件
  // pass 的只读面（状态/user-modified 检查）必须照常（tests/oauth-main-model
  // F1-2 断言 lastAction=gated 契约）。两机制同源（F-1 先例），非并存双实现。
  const gateWrites = trigger === 'llm' && state.failures > 0
  ...
  if (maintain) {
    ...
    if (plan.ops.length > 0) {
      if (gateWrites) {
        // F-1：失败态下 llm 事件 pass 不写条目——只刷状态（写入收敛到 tick
        // 节奏；notice 已由 recordHostRouteFailure 承载）。
        state.maintained = false
        state.lastAction = 'gated'
        return hostRouteStatusOf(service)
      }
      try {
        await settings.mutate(HOST_ROUTE_NS, plan.ops)
      } catch (error) {
        // F-7（P8 口径统一）：mutate 被拒（含同 ns 其它 provider 条目非法的
        // 连带拒绝——resolveProfiles 遍历整个 dict）→ recordHostRouteFailure
        // 统一链路（计数/降级阈值/notice），不再静默外溢。
        state.maintained = false
        state.lastAction = 'mutate-rejected'
        recordHostRouteFailure(ctx, service, `条目写入被拒：${error && error.message ? error.message : String(error)}`)
        try { service.recordHostRouteEvent('host_route_maintain_fail', { reason: 'mutate_rejected' }) } catch { /* 尽力而为 */ }
        return hostRouteStatusOf(service)
      }
    ...
    if (gateWrites) {
      // F-1：失败态下 llm 事件 pass 全程零写入（idle 亦早退——probe 失败会
      // 触发回滚 mutate，同样构成事件源；PoC 清理交由 tick pass）。
      state.maintained = false
      state.lastAction = 'gated'
      return hostRouteStatusOf(service)
    }
  ...
  if (plan.ops.length > 0) {
    try {
      ...
      // F-7：清理 mutate 被拒 → recordHostRouteFailure 统一链路（与 maintain
      // 分支同口径）。
      state.maintained = false
      state.lastAction = 'mutate-rejected'
      recordHostRouteFailure(ctx, service, `条目清理被拒：${error && error.message ? error.message : String(error)}`)
      try { service.recordHostRouteEvent('host_route_maintain_fail', { reason: 'mutate_rejected' }) } catch { /* 尽力而为 */ }
      return hostRouteStatusOf(service)
    }
    state.lastAction = plan.action
  ...
  // F-3（数据主权）：HOST_ROUTE_REF 凭据清理——停用方向条目删除后 ref 即无
  // 消费方（宿主 pi-ai 每请求活读），access token 副本不得滞留宿主凭据文件；
  // idle（无条目）也清（残留自上次登出）；user-modified 分支已提前 return
  // （用户条目可能仍引用 ref，不清理）。尽力而为不阻断（与 PoC ref 同型）。
  const credentials = credentialsOf(ctx)
  if (credentials && typeof credentials.unset === 'function') {
    try { await credentials.unset(HOST_ROUTE_REF) } catch (error) {
      warn(`dsh-agent-router: 宿主凭据 ref 清理失败（${HOST_ROUTE_REF}）：${error && error.message ? error.message : String(error)}`)
    }
  }
  if (plan.poc === true) {
    if (credentials && typeof credentials.unset === 'function') {
...
export function hostRouteStatusOf(service) {
  ...
  let present = false
  try {
    const settings = settingsOf(service?.ctx)
