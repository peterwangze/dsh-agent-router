...
export function createWrapAdapter(llm, provider, active, onCall) {
  ...
  const adapter = {
    ...
    /**
     * FIX-033：宿主 0.1.5-rc.2 LlmAdapter 原型新增契约方法（宿主
     * lib/index.js:1645）——签名 imageRequestPricing(_provider, _model)，
     * **同步零 I/O**（宿主注释 :1639 "must answer synchronously without
     * I/O"），默认返回 undefined = "本路由未声明图片定价，消费者回落自身
     * 中性估算"；运行时消费点 LlmRuntime.imageRequestPricing（:1996-1998）
     * 经 adapters.get(provider).adapter 逐路由解析，token meter 按次测量取价。
     *
     ...
     * 一致）；未实现/未声明 → undefined（宿主基类默认语义 :1645，消费者回落
