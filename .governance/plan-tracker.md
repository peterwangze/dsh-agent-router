...
| MIG-001 | development | v3 迁移实施 Step 0-10（DEC-012） | 按架构 v3 §8 迁移路径逐步实施：Step 0 基线测试 → Step 1 移除整轮路由 → … → Step 10；每步独立提交+测试全绿；验收门 = D-1 五条指标（DEC-012） | 进行中——Step 0-5b 完成（7cb2024/b7261d5/a23b338/374edfa/f89b8bd + Step 5b 本提交；R1-R6 审查链全通过，EV-011~016）；**Step 5c/7 启动门：V-DSH-7 宿主 id 运行时验证（DEC-013，R6 P1）MUST 先闭环**；下一步 Step 5c（RPC wire codec）或先做 V-DSH-7 验证；P2 遗留：R6-F-2/F-3（测试判别力）/R6-F-4（匿名键缓存）+ R4 F-1/F-2（顺延） | P0 |
