...
/**
 * Gemini API 现行 OAuth scope（官方 OAuth quickstart 的组合）。旧 scope
 * `https://www.googleapis.com/auth/generativelanguage` 已被 Google 拒绝
 * （授权页报「禁止访问：发生了授权错误」，Error 400: invalid_scope）。
 */
export const GEMINI_OAUTH_SCOPES = 'https://www.googleapis.com/auth/cloud-platform https://www.googleapis.com/auth/generative-language.retriever'

/** 旧 Gemini scope 迁移：含旧名（且不含新名）时替换为现行组合。 */
export function migrateGeminiScope(scope) {
...
export class RouterService extends TypertRemoteService {
  ...
  async run(input) {
    ...
    if (inputFiles.length > 0) {
      ...
      // - agent 类型：路径注入子代理任务，由其用 fs 工具自行读取任意文件；
      // - chat 类型：按文件内容能力化分发——图片（需 capabilities 含 image）
      //   经附件服务内联注入、文本内联进 task、其余二进制/目录明确报错。
      if (type === 'agent') {
        if (resolved.mode !== 'route') throw new Error('files 文件附件仅支持普通 agent 类型专业 agent（OAuth 账号/账号池仅支持 chat 类型）')
  ...
  promptText() {
    ...
    const lines = entries.map(([id, agent]) => {
      ...
      return `- \`${id}\`（${agent.name || id}，${this.normalizeType(agent.type)}${capsText}）：${description} [${meta}]`
    })
    return [
  ...
  async oauthBegin(request) {
    ...
    const scope = migrateGeminiScope((typeof account.scope === 'string' && account.scope.trim()) ? account.scope.trim() : '')
