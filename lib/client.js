...
`
    ...
    function AgentsPage(props) {
      ...
      /** preset 模型编辑态解析（FIX-015 c/d）：显式 draft 优先（含用户清空的
       *  ''）；已存 models 非空 → join；已登录且空 → 默认三件套**实预填**（用户
       *  保存即写入真实值——placeholder 幻觉根因：视觉近似已填值但保存的是
       *  空值，实测 models=[]）；未登录空 → ''（placeholder 保留提示价值）。
       *  显示与保存共用（P5 单点，杜绝「显示有值、保存空值」分裂）。 */
