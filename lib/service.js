 ...
 * 安全默认：自动批准工具调用 + 仅允许写入工作目录的沙箱（workspace-write；
 * 子代理的产物——如图片——必须能落盘到工作区，read-only 会导致任务无法完成）；
...
export class RouterService extends TypertRemoteService {
   ...
   * - `args` 为空时使用该 CLI 的安全默认参数，否则整体覆盖；
   * - `model` 非空时经 modelFlag（-m / --model）传给 CLI。
   */
  resolveCliSpec(agent) {
    ...
    const args = userArgs.length > 0 ? userArgs : (preset?.args ?? [])
    ...
    return { command, base, args: finalArgs, format, parse: preset?.parse }
  }
