...
// llm-selection 域 llmFaceOf（§4.3 三方法并集——与原两方法检查零回退；
// 先例 wrapper.js installAdmissionWrapper 同批切换）。
...
/**
 * 错误映射（H3-14 同构，与 service.js `runCodexResponsesChat` 语义
 ...
 * parameters}——dsh-system-prompt 的 `result.schemas.map(({ name, description,
 * parameters }) => ({ … parameters: structuredClone(parameters) }))`（tool
 * provider schemas 解构面）+ dsh-llm-pi-ai 的 `toolsOf(options)`（官方 adapter
 * 同款读法）双向印证。两处原为行号式锚，FIX-043 批 C 实读判定均已漂移到他
 * 对象（前者当前行号为 `getContextOrder` JSDoc、后者为 context 模块注释）⇒
 * 改代码串/符号名式（对象逐字实读，P10-④ 零新增未实证符号）。
 ...
 *  imageRequestPricing（FIX-034 补齐，宿主 dsh-llm 的 LlmAdapter 原型新增）/ listModels /
 *  resolveModel / prepareCall / stream）——FIX-001 教训：手工对象字面量无基类默认可
 *  继承，契约方法必须显式实现；导出工厂供 adapter-parity 契约守卫直接构造（与
 *  wrapper createWrapAdapter 同构先例）。 */
export function createOauthAdapter(ctx, service) {
  const adapter = {
    providerInfo(route) {
    ...
    /**
     * FIX-034：宿主 0.1.5-rc.2 LlmAdapter 原型新增契约方法（宿主 dsh-llm 的
     * `imageRequestPricing(_provider, _model)` 空体默认，语义与 FIX-033 twin
     * wrapper.js 镜像实现同构）——签名 imageRequestPricing(_provider, _model)，
     * **同步零 I/O**（宿主 JSDoc "must answer synchronously without I/O; the
     * token meter resolves this per measurement"），默认返回 undefined = "本
     * 路由未声明图片定价，消费者回落自身中性估算"。运行时消费点
     * LlmRuntime.imageRequestPricing 经 adapters.get(provider).adapter.
     * imageRequestPricing **直调**——本适配器为手工对象字面量（无基类空体
     * 默认可继承），缺此方法即宿主 token meter 取价时
     * "adapter.imageRequestPricing is not a function" 全量断裂（twin 同型
     * 缺口的镜像形状，FIX-033 Developer 上报）。
     *
     * 本路由未声明图片定价：ChatGPT 订阅端点（chatgpt.com/backend-api/
     * codex/responses）无公开按图计价表——不虚构定价数字，恒 undefined =
     * 宿主基类默认语义（空体 → "declares none"，宿主 LlmImageRequest
     * Pricing schema 的 priceImages 实形由声明方适配器承载）。纯
     * 同步字面量返回：零 I/O 天然满足、无委托链天然不抛（fail-safe 平凡
     * 成立，方向与 twin/providerRetryPolicy 一致）。
     */
    ...
    async *stream(options) {
      ...
      try {
        ...
        service.record({
          ...
          model,
          ok: true,
          // FIX-031 F-1：call 行归因到请求起点（与 scope 行同源时刻）——跨 UTC
          // 午夜的流按终点落日会与 day1 scope 行分居两格，兜底分支再计一次。
          at: started,
          ms: Date.now() - started,
...
export function installOauthLlmAdapters(ctx, service) {
  ...
  // settings/updated 两处独立 ctx.on 收敛 events 域——与同根 ctx 上的
  // index.js 统计持久化 / service.js 宿主路由维护 / wrapper 热同步共享同一
  // 宿主 listener（订阅 census 4→1，BR-01② 只减不增）；ns 过滤语义逐字保持
  // （settings/updated 在 resolved 提交后发出——wrapper 同款理由；
  // llm/adapters-updated 兜底宿主侧拓扑变化）。
  const disposeEvents = subscribeEvents(ctx, [
    { event: 'llm/adapters-updated', consumer: 'oauth-llm-adapters-sync', handler: sync },
    {
      event: 'settings/updated',
      consumer: 'oauth-llm-settings-sync',
      handler: (ns) => {
