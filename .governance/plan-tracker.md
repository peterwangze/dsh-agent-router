...
| — | v0.4.2 候选 | EVO-013 台账（R1 复审裁入）：F-4 parent 查询失败时显式覆盖保护降级披露 / F-5(b)(c)(e) 测试缺口三处 / F-6 liveDefaultSelection 双份实现去重（prestep.js 与 preset-defaults.js 共享导入）/ F-7 composedPreset {id} 对象防御死代码 / N-1 判别断言头部索引未同步 P4b/P4c / 观察项：已配置条目卡不显示 broken 状态 | — | 0.4.2 | 已记录待排期 |
