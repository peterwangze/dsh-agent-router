 ...
 * pi-ai 的 `openai-codex.js` 中 `DEVICE_REDIRECT_URI` 一手事实）。
 */
 ...
 * `openai-codex.js` 中 `DEVICE_CODE_TIMEOUT_SECONDS` 一手事实）。
 */
...
/**
 * 传输类失败（网络错误）重试退避增量（毫秒，F-2 / REL-003）：poll 网络错误
 * 非终态——轮询循环 intervalMs 增加此值后重试至 expiresAt。与 slow_down
 * 增量分开常量（协议退避 vs 传输健壮性，出口可独立校准）；只影响客户端
 * 轮询节奏，不影响协议语义。
 */
export const DEVICE_TRANSPORT_RETRY_ADD_MS = 5_000

/**
 * 设备码轮询间隔下限（毫秒，F-7 / REL-003）：服务端通告 interval 异常小
 * （含 0）时不触发热轮询——地板值防打爆 token 端点；service.js 实例级
 * oauthDeviceMinIntervalMs 注入仅供测试提速。
 */
export const DEVICE_MIN_POLL_INTERVAL_MS = 1000
 ...
 * - 'failed'：其他 error 码 / 响应畸形——终态语义（附 message）。网络错误
 *   （fetch 抛错）同为 'failed' 但附 transport:true 判别标志（F-2 /
 *   REL-003）——轮询循环据此退避重试至超时而非终态；既有 failed 语义
 *   不变（协议拒绝路径无此标志）。
...
export async function pollDeviceAuthorizationToken(device, fetchImpl) {
  ...
  try {
    ...
    // transport:true（F-2 / REL-003）——网络错误非协议终态，轮询循环据此重试。
    return { status: 'failed', transport: true, message: `设备码轮询请求失败（网络错误）：${errorMessage(error)}` }
  }
...
export class OauthCredentialStore {
  ...
  async ensureFresh(cred, options = {}) {
    ...
    return this.withLock(timeoutMs, async () => {
      ...
      // AUDIT-001 P0-B（写回前盘上 CAS 守卫）：刷新在途（≤25s 网络）期间，盘上
      // 文档可能已被并发写/删路径改变（登出删除 / 新登录写入 / 跨进程操作）。
      // 无守卫时无条件 write(fresh) 会①把用户已显式删除的凭据文件**复活**
      // （合规删除 W-5 被击穿，违反项目原则 7——设备码流同类问题 F-1 已有
      // 落盘前复查，刷新路径此处补齐）②用旧 refresh 链产物覆盖新登录文档
      // （rotating 链丢失、accountId 回跳）。守卫语义：
      // - 盘上已删除 → 绝不写回，仅返回本次内存值（在途请求用完即弃，后续
      //   请求自然走重登语义）；
      // - 盘上 refresh 已变化 → 采用盘上最新文档并放弃写回；
      // - 盘上未变 → 原子写回（既有语义不变）。
      const latest = await this.read()
      if (!latest) return fresh
      if (latest.refresh !== base.refresh) return latest
      try {
