 ...
 *   oauthExperimental 关闭时零端口占用；1455 为 client 注册死值，与
 *   Codex CLI/dsh-codex/yoke233 生态共用）；被占用时静默降级走 E4
 *   降级链（设备码/手动粘贴，Step 4 接线）。
 *
 * 开关方式：
...
export function createCodexLoopback({ service, logger, port } = {}) {
  ...
  codexLoopbackActive = new Promise((resolve) => {
    ...
    loopback.on('error', (error) => {
      codexLoopbackActive = null // 失败不缓存：下次调用重试（E4 用户可重试语义）
...
export function apply(ctx) {
  ...
  // 多模态平台 L1：准入包装（twin 路由，随视觉 agent 开关热同步）。
